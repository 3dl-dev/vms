/*
 * ovmx_datalink.c - vms-838: raw-L2 datalink backends (Linux AF_PACKET,
 * NetBSD bpf(4)). See include/scs_datalink.h for the surface, the rationale
 * and why the header keeps its original name after FC-P3.9 relocated this
 * pair out of the retired src/vmsscs tree.
 *
 * WHAT IS PROVEN vs. WHAT IS A DESIGN CLAIM. The Linux backend is the
 * long-standing AF_PACKET behavior, unchanged by the relocation. The NetBSD backend closes SCSD.EXE's
 ...
 * a caller's own send-site gate applies to it -- can attribute
...
static ssize_t datalink_send_netbsd(int fd, const uint8_t *frame, size_t len)
{
     ...
     * destination MAC, set by the caller's builders. */
...
ssize_t scs_datalink_recv(int fd, uint8_t *buf, size_t buf_len)
{
    ...
    if (e->off >= e->used) {
        ...
        if (r == 0) {
             ...
             * back later" signal the caller handles. */
            errno = EAGAIN;
            return -1;
        }
        e->off = 0;
