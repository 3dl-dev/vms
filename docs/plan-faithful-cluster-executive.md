...
| FC-P4.3 | `dir_resolve` per the resolved ladder: `struct vms_ldwv` in the CLUB (entries per member = LOCKDIRWT, one each when all 0, contiguous, own entries 0, CSV-index order), resized at Phase 1 / filled at Phase 2 via the CNXMAN transition callbacks, all `rsb->dir_csid` invalidated on vector change; `rsb->hash16`/`hash_known` learned from every cat-02 frame carrying body[10:12]; `dir_resolve(rsb) = ldwv[hash16 mod n]` (0 ⇒ self) requiring `hash_known`; a lookup is never sent without a wire-learned hash (novel root ⇒ `SS$_UNSUPPORTED` + one `%CNXMAN` line until rung B/C resolves); received-lookup self-check counter `dir_lookup_misaddressed`; `exec_jhash` deleted. | R1: vector construction vs the p. 6-33 worked example and the all-0 case; index rule; hash-learn/never-guess tests; R2: simulated weighted set with OVMX as directory for its share; self-check fires on a deliberately mis-ordered vector. | `vms_lock.c` (`dlm_directory_csid` → `dir_resolve`, LDWV), `vms_cnxman.h` callbacks, `vms_dlm_scs.h`, `codec_dlm` (hash accessor) | P4.1, P3.5 | **O5** (high) — the Rule-8 boundary lives in this function | OP (rung C only, for the novel-name residual) |
