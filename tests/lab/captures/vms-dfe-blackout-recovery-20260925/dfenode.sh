#!/bin/bash
# dfenode.sh - boot ONE booted-OVMX node under QEMU/KVM on the isolated rd
...
if grep -qaF 'Username:' "$OUT"; then
    ...
    while [ "$p" -lt "$DUR" ]; do
        kill -0 "$QP" 2>/dev/null || break
        send "WRITE SYS\$OUTPUT \"DFE-POLL-$NAME-$p\""; sleep 1
        send 'SHOW CLUSTER'; sleep 6
        sleep 8; p=$((p+15))
    done
    send 'WRITE SYS$OUTPUT "DFE-DONE"'; sleep 3
else
