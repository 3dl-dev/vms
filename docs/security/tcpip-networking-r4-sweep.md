...
> **STATUS UPDATE (re-grounded @ `ed426269`).** Since the original sweep all three gaps have now **LANDED**: **G1 (per-service run-as persona + privilege drop before `execv`, fail-closed) is CLOSED** (vms-8bd, V0.6-15); **G2 (INETD `MAXCHILD` concurrency cap + accept back-pressure) is CLOSED** (vms-bb4, V0.6-15); **G3 (decoder fuzz) is CLOSED** (vms-deb3 / PR #1213 reaped, DECnet fuzz loan): ~400k adversarial inputs under ASan/UBSan across **both** SERVICE.DAT parsers (`tcpip_inetd_parse_db` + #878 `tcpip_svcdb_parse_rec`) plus the parse→format→re-parse round-trip — **zero over-reads/OOB/UB; no bug**, locked as a build-enforced ctest (#1213 reaped). The **HARD GATE** the posture placed before enabling a reading/privileged service (notably the SSH rung) — *G1 + G2 must land first* — is therefore **FULLY MET on the technical axis (G1+G2+G3 all landed)**; what remains before an SSH-enable is only the **enable-decision itself, a Baron/conductor-reserved posture call, not enacted here.** §§3–5 below are updated in place to reflect this.
...
### G3 — Fuzz coverage of the decoders — **CLOSED (vms-deb3 / PR #1213 reaped)** → rd vms-deb3
**DONE, no bug** (DECnet fuzz loan). Exact-sized-buffer ASan/UBSan mutation fuzzing of **both** SERVICE.DAT parsers — `tcpip_inetd_parse_db` (`tcpip_inetd.h`) and the #878 `tcpip_svcdb_parse_rec` (`tcpip_service_db.h`) — ~400k adversarial inputs + an edge corpus, with a `parse_rec → format_rec → inetd_parse_db` round-trip over-read assertion; the over-read bug class (missing/embedded NUL, over-length token, no-newline line, all-whitespace, bare `!`, out-of-range/overflow port, user-vs-image) is contract-agnostic. **RESULT: zero over-reads/OOB/UB/crashes — both parsers are well-bounded** (bounded `strncpy` into `memset` structs + pre-truncated `line[512]`), confirming §2 by execution; locked as a build-enforced host ctest `tcpip_svcdb_fuzz` (~3.6s under ASan). **Contract note:** both are `const char*` C-string APIs; **every production caller NUL-terminates** — `tcpip_inetd_parse_db` via `fread`+`buf[got]='\0'` (`vms_tcpip_inetd.c:86`), `tcpip_svcdb_parse_rec` via `rms_textfile_getline` (always NUL-terminated) — so the C-string contract is safe as called; a length-bounded API is deferred as unneeded (no caller feeds unterminated bytes; revisit only if a wire-fed variant appears). The `bgsock` recv path is a bounded 4096-byte stream copy (not a structured decoder), memory-safe by inspection (§2) — low fuzz value. **Original state (superseded):** no coverage-guided/mutation fuzzing existed (only `test_syssvc_tcpip_daytime.c`, a functional proof); DECnet shipped 5 sanitizer-fuzzed decoders. **Mitigant:** the TCP/IP byte path is far simpler (fixed 4096 buffers, no nested TLV), memory-safe by inspection at every layer (§2). Close (deb3 lands) before enabling any service that *reads* client bytes. **Higher-untrusted follow-on (flagged):** the client-tool *response* parsers that eat network bytes — `tcpip_client.h` (TELNET option negotiation, FTP reply/length) and `tcpip_ping.h` (ICMP echo-reply) — are a separate, arguably higher-value fuzz rung than the admin-config SERVICE.DAT parse.
...
- **(d) Decoder fuzz — CLOSED (G3, vms-deb3 / PR #1213 reaped)** — ~400k ASan/UBSan inputs across both SERVICE.DAT parsers + round-trip, zero over-reads/OOB/UB; the residual fuzz risk is retired and locked as a build-enforced ctest.

**Recommended framing (composes with the DECnet ruling; Baron chooses):**
1. **"OVMX's TCP/IP surface is a memory-safety + config-integrity boundary, with per-service identity isolation and DoS back-pressure on the aux server, that ships DORMANT-by-default; it is NOT a seccomp-sandboxed or transport-authenticating boundary (each service authenticates itself). Enabling any inbound service is an explicit operator posture decision."** — precise, defensible, matches the code + the CI guard. **Recommended for 1.0.** The original **HARD GATE — G1 (per-service run-as + privilege drop) and G2 (concurrency cap / back-pressure) MUST land before any *reading* or privileged service (notably the SSH rung) is enabled — is now MET (both landed, V0.6-15).** With G3 now CLOSED (vms-deb3 / PR #1213 reaped), **the technical axis is fully met (G1+G2+G3)** — what remains before an SSH-enable is **only the enable-decision itself**, the reserved posture call (conductor per the R4 Option-1 precedent, or Baron if it is a confidentiality call) — **not enacted by this sweep.**
2. **"OVMX's TCP/IP surface is a hardened security boundary"** — G1 + G2 remediated and G3 fuzzed-clean; the technical preconditions for a reading service are met, and only the enable-decision remains.
...
| vms-deb3 | G3 no fuzz coverage (SERVICE.DAT parsers: `tcpip_inetd_parse_db` + #878 `tcpip_svcdb_parse_rec`) | low-medium | before a client-reading service | **CLOSED** (PR #1213 reaped, ~400k ASan/UBSan, no bug; ctest) |
