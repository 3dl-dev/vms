...
shot 'SHOW CONNECTIONS' 'T-PRE ALLCONN'
shot "SHOW CONNECTIONS/NODE=$IDENT" "T-PRE $IDENT"
...
while [ $(( $(date +%s) - T0 )) -lt "$DUR" ]; do
  shot "SHOW CONNECTIONS/NODE=$IDENT" "T+$(( $(date +%s) - T0 ))s" || break
done
shot 'SHOW CONNECTIONS' 'T-END ALLCONN'
say 'EXIT' 4
