...
#
# WHAT THIS SCRIPT PROVES TODAY (the in-domain maximum for vms-b65.6):
#   1. The full six-library producer graph links VMS-native, INCLUDING the 32 libc
#      universals appended to DECC$SHR for DCL (access..utimes) — the graph still
#      builds STRICT-clean.
...
#   3. The exact toolchain gap that blocks the DCL.EXE LINK is characterized: the
#      executable emit path is single-object / _start-only / external-reloc-only,
#      but DCL is a 22-object, main()-entered program dominated by intra-image
#      relocs (ADRP/ADD/ABS64/PREL32/LDST) + its own TLS object.
#
# WHAT IS BLOCKED (escalated on the bead, category=architecture): the actual
# `LINK.EXE --executable` of DCL.EXE. `emit_executable` (src/vmslink/link.c) must be
# rebuilt on the emit_shareable multi-object machinery + gain a C-runtime entry
# (crt0 -> __init_libc/__init_array/main), and IMGACT.EXE must give an executable
# module process-startup (argc/argv/envp/auxv) + its own-module TLS. link.c and
# imgact.c are OUT of the Systems-Engineer file-domain — do NOT edit them here.
# Because DCL.EXE cannot yet link, this test is NOT wired into ci.yml (a red job is
# never wired). It exits non-zero at the blocker so it is never mistaken for DONE;
# flip DCL_EXPECT_LINK=1 once the executable emit path lands to run the full
# activate+scripted-session assertion.
#
...
echo
echo "== characterize the emit_executable gap (empirical, informative) =="
echo "-- entry symbol (emit_executable needs _start; DCL has main): --"
nm $DCLOBJS 2>/dev/null | awk '$2=="T" && ($3=="main"||$3=="_start"){print "   "$3}'
echo "-- reloc histogram (intra-image ADRP/ADD/ABS64/PREL32 dominate): --"
for o in $DCLOBJS; do readelf -rW "$o" 2>/dev/null; done \
  | awk '/R_AARCH64/{print $3}' | sort | uniq -c | sort -rn | sed 's/^/   /'
echo "-- TLS-defining objects (single-object TLS is within emit_shareable's limit): --"
for o in $DCLOBJS; do readelf -SW "$o" 2>/dev/null | grep -qE '\.tdata|\.tbss' && echo "   TLS: $(basename $o)"; done

echo
echo "== attempt the DCL.EXE executable link (EXPECTED BLOCKED, vms-b65.6) =="
set +e
...
if [ "$LRC" -ne 0 ]; then
    if [ "${DCL_EXPECT_LINK:-0}" = "1" ]; then
        echo "FAIL: DCL_EXPECT_LINK=1 but the executable link still fails"; exit 1
    fi
    echo
    echo "BLOCKED (vms-b65.6, escalated category=architecture): DCL.EXE cannot link"
    echo "  yet — emit_executable is single-object/_start-only/external-reloc-only."
    echo "  The producer graph + DCL compile are PROVEN ready. Once the multi-object"
    echo "  executable emit path + IMGACT process-startup land, re-run with"
    echo "  DCL_EXPECT_LINK=1 to add the activate + scripted-session assertion."
    exit 2
fi
