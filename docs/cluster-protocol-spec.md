...
#### 4(O.35) A DETERMINISTIC RETURN-SIDE RECLAIM-ENGAGEMENT FIX IS **NOT** OVMX-FORCEABLE — the win-vs-lose discriminator is grounded to the WIRE + the member CSB oracle (the member's `VMS$VAXcluster` joiner VC reaching **OPEN with BOTH members** — the Rule of Total Connectivity, Davis p.7-39), and the completing step is **member-driven**: on a return the member reclaims its departed CSB PROMPTLY and deterministically (DCL `SHOW CLUSTER` for the returner goes `BRK_NON` → `NEW` within ~4 s, a fresh CSB per Davis pp.7-24/7-25) but then STALLS at `NEW` unless BOTH surviving members' connection managers RE-ENGAGE the returner — each opening its OWN `SCS$DIRECTORY` connection to OVMX and then sending `ACCEPT_REQUEST` to bring OVMX's joiner VC to OPEN (`RCV_ACCEPT_REQ --> OPEN`); a WIN (`CLUSTER_NODES` 2→3) has BOTH members do this, a LOSE has only ONE, and OVMX **cannot force the missing member's engagement**: the direct OVMX lever — pre-emptively opening its joiner CONNECT-REQ to every member it has already brought to VC OPEN — was BUILT behind a kill-switch and MEASURED to LOSE **0/10 consecutive returns** (the member transport-accepts to `CONNECT ACK` but never issues the SYSAP-level `ACCEPT_REQUEST` for a connect it did not itself solicit, so both VCs stall), so the deciding factor is member-side residual-CSB reclaim state, NOT OVMX send-ordering (GROUNDED, `vms-694`/runs `o35dbg`+`o35rel`+`o35proof`, single-factor first-join-vs-return bracket on virgin `vaxlab-2`, VAX `F$GETSYI("CLUSTER_NODES")` + DCL `SHOW CLUSTER` oracles + OVMX daemon connection-FSM + tcpdump, current-HEAD Release `md5=f059ce0c` & Debug `md5=17c25b26` daemons plus a patched Debug `md5=7fbf5dd3` (this branch's `src/vmsscs`, comment-only edits since), 2026-08-12).

**Frame.** §4(O.34) refuted the leave-time deallocation fix and relocated the deterministic path to the RETURN side; the operator chose the deterministic path and asked THE question: is there an OVMX-side engagement ORDERING/HANDSHAKE that makes OVMX reliably COMPLETE the member's reclaim rather than race it (§4(O.33))? Every prior increment read OVMX's derived verdicts (`READMITMAP`/`XITABORT`), which #346 flagged as phantom-refusal artifacts. §4(O.35) reads only the oracles OVMX cannot colour — each member's `CLUSTER_NODES` and DCL `SHOW CLUSTER` state for the returner — and aligns them, per member, against OVMX's own wall-clock-stamped connection FSM and the wire.

**THE SINGLE-FACTOR BRACKET (GROUNDED, virgin `vaxlab-2`, identity `OVMXE0`/1815).** F1 fresh first-join → clean-leave depart → settle (departed CSB lingers `BRK_NON` on both members, `CLUSTER_NODES` back to 2) → J return the SAME identity; arms differ ONLY in first-join-vs-return, run with a slow (Debug) and a fast (Release) daemon:

| observation (VAX oracle + OVMX FSM) | F1 first-join (WIN) | J return, Debug (LOSE) | J return, Release (WIN) |
|---|---|---|---|
| members OVMX brings to VC OPEN (STARTDONE, `start_acked`) | 2 | **2** | 2 |
| members that open their OWN `SCS$DIRECTORY` to OVMX (`DIRCONN`) | 2 | **1** | 2 |
| OVMX joiner VCs reaching OPEN (`RCV_ACCEPT_REQ --> OPEN`, JOINBOUND) | 2 | **1** | 2 |
| member CSB for the returner (DCL `SHOW CLUSTER`) | → MEMBER | **`BRK_NON` → `NEW`, stalls (74 s)** | → MEMBER |
| `CLUSTER_NODES` (member oracle) | 2→3 | **stays 2** | 2→3 |

**Finding 1 — the member reclaims; it does NOT "fail to engage".** On the LOSE the coordinating member deallocates the old `BRK_NON` CSB and allocates a FRESH one PROMPTLY: `SHOW CLUSTER` for the returner goes `BRK_NON` → `NEW` (Davis pp.7-24/7-25's returning-CSB state) within ~4 s of the return, and holds `NEW` for the full 74 s window. §4(O.31)/§4(O.33)'s "member never re-opens / NO-ENGAGE" was OVMX's own classifier over-reading; the member DOES reclaim. The stall is `NEW` → MEMBER, not a failure to reclaim.

**Finding 2 — admission is gated on TOTAL CONNECTIVITY (Davis p.7-39), and reaching it is member-driven.** A return is admitted IFF OVMX's `VMS$VAXcluster` joiner VC reaches OPEN with BOTH members. A joiner VC reaches OPEN only when the member sends `ACCEPT_REQUEST` (`CONNECT ACK --RCV_ACCEPT_REQ--> OPEN`), which the member's CM does only AFTER it has opened its own `SCS$DIRECTORY` connection to OVMX (its re-engagement of the returner). WIN = both members do this; LOSE = only one. OVMX already completes START with BOTH members on the LOSE (both `STARTDONE`), so it has done its discovery part; the second member's non-engagement is its residual-CSB reclaim state, the §4(O.33)/§4(O.34) race, now localised to "does this member re-open `SCS$DIRECTORY` to the returner and `ACCEPT_REQUEST` its joiner VC."

**Finding 3 — the direct OVMX lever is REFUTED (0/10).** If total connectivity were merely OVMX opening a joiner CONNECT-REQ to both members, forcing it would win. It does not. A kill-switched build (`OVMX_JOIN_ALL_MEMBERS=1`, default OFF) opens OVMX's joiner VC to EVERY `start_acked` member off OVMX's own clock, independent of the member-driven `SCS$DIRECTORY` prompt. Measured on the patched Debug daemon, 10 consecutive returns each from a CONFIRMED `CLUSTER_NODES=2`: **0/10 wins**. The pre-emptive CONNECT-REQ gets a transport `CONNECT_RESPONSE` (VC → `CONNECT ACK`) but the member NEVER sends `ACCEPT_REQUEST` — because `ACCEPT_REQUEST` is the member CM's decision, gated on its reclaim, not OVMX's to force — so both VCs stall in `CONNECT ACK` (repeated `CONNECT_RESPONSE`, `ILLEGAL-EVENT-FOR-THIS-STATE`) and the return reliably LOSES, WORSE than the unpatched daemon, which at least lands the one member-prompted VC. This positively grounds that OVMX's send-ordering is not the lever.

**Verdict — deterministic rejoin is NOT cleanly OVMX-forceable; surface to operator.** The deciding factor between win and lose is whether BOTH surviving members' connection managers re-engage the returner (each opening `SCS$DIRECTORY` to OVMX and issuing `ACCEPT_REQUEST` to complete OVMX's joiner VC to OPEN, satisfying total connectivity, Davis p.7-39). That engagement is member-driven residual-CSB reclaim state; a fast (Release) daemon usually elicits both and wins, a slow (Debug) daemon elicits one and loses — the §4(O.33) nondeterminism, now shown to be MEMBER-side. OVMX cannot force the missing member's `ACCEPT_REQUEST`: the only OVMX-emittable signal that would drive a member's CM to complete removal/re-engagement earlier is the CM-layer op-0x0d self-departure, the RE'd crash class that bugchecks the real VAX (§4(O.29)). Per the operator's discipline (ship only a cleanly-isolated, validatable deterministic fix; else STOP at isolation and report the verdict), this increment ships the isolation + the refutation ONLY. The kill-switched `OVMX_JOIN_ALL_MEMBERS` path is retained as the bisectable NEGATIVE CONTROL that grounds the verdict, default OFF — **not** a fix; no default-path change.

**Explicit non-claims.** (1) This does NOT claim the return is UNwinnable — XITDONE 0→1 remains reachable on current HEAD (§4(O.33)), it is the DETERMINISM that is not OVMX-forceable. (2) It does NOT re-open the op 0x02 body — F1 and J present the identical first-join op 0x02 under the clean-leave default (verified: `CMREJOIN` count 0 on the return). (3) The Debug-vs-Release axis is a timing PROBE, not a claim one build is "correct"; both are the same source, and the axis shows the outcome sits on the member-side reclaim knife-edge. (4) `OVMX_JOIN_ALL_MEMBERS` is retained for reproducing the refutation only and must never be read as a shippable fix.

**Evidence** (host, tank volume). Single-factor bracket on virgin `vaxlab-2` (disks re-cloned from `.3node-golden.bak`, `CLUSTER_NODES=2` confirmed pre-run, golden preserved). Isolation runs (Release `md5=f059ce0c`, Debug `md5=17c25b26`, both `origin/main` `src/vmsscs` HEAD): `tests/lab/tools/o35run.sh`; timeline `/data/training/vax/cluster/work/o35dbg.timeline` (`BRK_NON`→`NEW`@J+4s, stays `NEW`, `CLUSTER_NODES` stays 2) & `o35rel.timeline` (`CLUSTER_NODES` 2→3 @J+4s); daemon logs `/data/training/vax/k8s-labs/vaxlab-2/logs/scsd-o35dbg-J.log` (1 JOINBOUND, 1 DIRCONN, member `NEW`) & `scsd-o35rel-J.log` (2 JOINBOUND via `RCV_ACCEPT_REQ`, 2 DIRCONN, op 0x03/0x05/0x06 commit, XITDONE). Refutation proof (patched Debug `md5=7fbf5dd3` (this branch's `src/vmsscs`, comment-only edits since)): `tests/lab/tools/o35proof.sh`, `o35proof-fix.status` (`RESULT mode=fix: 0/10 wins; sequence: L L L L L L L L L L`), `scsd-o35p-r1.log` (2× `4(O.35) all-members` CONNECT-REQ, 0 JOINBOUND, both VCs `CONNECT ACK ... ILLEGAL-EVENT`). Code: `src/vmsscs/scsd.c` (`ovmx_join_all_members`, negative control, default OFF); wire regression `tests/vmsscs/test_scsd_wire.c` green (default path unchanged). *VAXcluster Principles* pp. 7-23/7-24/7-25/7-39.

#### 4(O.36) THE RETURN-PATH DIRECTED-HELLO IS NOT THE GATE — the §4(O.36) directed-HELLO lead is REFUTED on the return wire: OVMX's NISCA channel-verify handshake (§4(a).1 abs-30 `b2→b3→b4`) and its dest-logical addressing (§4(a).0 abs-16) are BYTE-CORRECT and COMPLETE with BOTH members on a LOSE return — so §4(O.35)'s "member never `ACCEPT_REQUEST`s / joiner stalls at `NEW`" is NOT caused by an OVMX directed-HELLO defect; the channel is verified, START completes, and the stall sits DOWNSTREAM at the member-driven CM/SYSAP reclaim engagement, exactly where §4(O.32)/(O.33)/(O.35) placed it (GROUNDED, `vms-694`, pure-observation wire read of current-HEAD OVMX return captures vs real-VAX return captures on the tank volume, 2026-08-12)

**Frame.** §4(O.35) surfaced "deterministic rejoin is member-driven, not OVMX-forceable"; the operator then corrected the *premise* (2026-08-12: real VMS rejoin was "remarkably error-proof", so OVMX has a DEFECT a real VAX does not — not an inherent race). A prior §4(O.36) agent (died on a weekly API limit before grounding it) flagged a strong candidate defect: spec §4(a).0/§4(a).1 already document a directed-HELLO channel-verify handshake whose FAILURE signature is exactly §4(O.35)'s symptom — §4(a).0: "with the channel unverified that peer never opens SCS connections to the joiner at all, so … the joiner is stuck at `NEW`." Hypothesis: on the RETURN path OVMX omits or mis-sends the directed HELLO (the abs-30 `b2/b3/b4` word, or the abs-16 dest-logical mirror bug §4(a).0 flags), so a member never verifies OVMX's channel and never `ACCEPT_REQUEST`s. §4(O.36) grounds that hypothesis on the actual return wire — the directed-HELLO layer is BELOW every prior increment's CM/connection reads and had never been decoded THROUGH an OVMX return. Method is pure observation (no wire guess toward a live VAX — the RE'd crash class, §4(O.29)): decode the `0x6007` directed HELLOs frame-by-frame (abs-30 per-frame word, abs-16 dest-logical, abs-24 src-logical, abs-92 incarnation, HW MACs) on a real-VAX return baseline and on OVMX return captures, and diff.

**The real-VAX return baseline (GROUNDED).** A returning real node runs the §4(a).1 handshake to COMPLETION with BOTH members and holds the steady `b3↔b4` oscillation, addressing each member at its OWN cluster-logical address (abs-16), incarnation incrementing on a true return:

| capture (real VAX) | returner → member A | returner → member B | abs-16 (dest-logical) | incarnation |
|---|---|---|---|---|
| `vax3-2to3-established-join` (VAX3 joins established VAX1+VAX2) | to VAX1: `b3`-REQ→`b4`-CONFIRM, then `b3↔b4` | to VAX2: `b3`-REQ→`b4`-CONFIRM, then `b3↔b4` | each member's own (`aa:00:04:00:01:04` / `…02:04`), NOT its HW MAC | 1 |
| `vax3-class03-crash-REJOIN-SUCCESS` (VAX3 crashes, rejoins) | to VAX1: `b3↔b4`, member INITs one `b2` | to VAX2: `b3↔b4`, member INITs one `b2` | each member's own | member advertises **2** on the return |

The joiner never originates `b2` (§4(a).1: `b2` is the member's first-contact INIT only); it originates `b3` and saturates the peer's `b3` with `b4`. This is the reliable pattern all successful (re)joins share.

**OVMX return, on the wire — byte-consistent with the real VAX, on a LOSE (GROUNDED).** OVMX return captures `o32.pcap` and `942-return-vs-firstjoin.pcap` (daemon `md5=d782c694…` == `origin/main` `src/vmsscs` HEAD for the directed-HELLO path — `scs_hello.c` and the scsd.c directed-HELLO responder are BYTE-IDENTICAL `2d82d3f8`↔current HEAD; the only intervening `src/vmsscs` commits are §4(O.35)'s kill-switched negative control and #346's classifier honesty, neither touching directed-HELLO emission) each drive a first-join arm AND a same-identity return arm; **both return arms LOSE** (`o32`: `admitted=1 engaged=0 XITDONE=0`; `942`: `proposing` delta 0, no `XITDONE`). Time-split to the RETURN arm only:

| directed-HELLO observable (RETURN arm) | real-VAX return | OVMX return (`o32`/`942`, LOSE) |
|---|---|---|
| OVMX→member `b4`-CONFIRM reached, BOTH members | yes / yes | **yes / yes** (`o32` return arm: OVMX→VAX2 `b4`×13, OVMX→VAX1 `b4`×15) |
| member `b2`-INIT re-send count (§4(a).0 pathology = 51×) | 1 each | **1 each** (no re-send flood) |
| abs-16 to the non-DECnet-HW-MAC member | member's own logical | **member's own logical** (OVMX→VAX2 abs-16 `aa:00:04:00:02:04`, NOT VAX2's HW `08:00:2b:…`) |
| return incarnation read + echoed | 2 | **2** (daemon `SCSD-I-INCARN`×2, `incarnation=2` both peers) |
| §4(k) padded size-verify both directions | 1500 SCA | **1500 SCA** (`peer_padded_sca=1500` both peers) |
| member channel-verified + START complete | yes | **yes** (daemon: both peers `channel=UP`, `STARTDONE`, `start_acked=1`) |

**The §4(a).0 abs-16 caveat is MOOT for OVMX's send side.** §4(a).0 notes a 2-node DECnet-MAC lab cannot exhibit the mirror bug because HW MAC == logical there; but this lab's VAX2 carries a NON-DECnet HW MAC (`08:00:2b:…`), so OVMX's abs-16 correctness toward a non-DECnet-MAC member is DIRECTLY tested here without a third node — and OVMX passes it (abs-16 = the member's logical, not its HW MAC). No third node was required to falsify the abs-16 half of the lead.

**Verdict — MEMBER-SIDE at the CM/SYSAP layer, NOT a directed-HELLO defect.** On the return, OVMX's NISCA directed-HELLO channel verifies COMPLETELY and correctly with BOTH members (b2→b3→b4 to terminal `b4`, steady `b3↔b4`, correct abs-16, correct return-incarnation echo, padded size-verify), so §4(a).0's "channel-unverified → member never opens SCS connections" does NOT apply: the members DO verify OVMX's channel and DO open SCS connections (the `VMS$VAXcluster` VC reaches OPEN, §4(O.32)). The LOSE sits STRICTLY DOWNSTREAM of a verified channel — `cm_responses=0`, `remote_conid=0` (VAX1), `member=DISC ACK / joiner=CONNECT SENT` (VAX2) — the member-driven `SCS$DIRECTORY`+`ACCEPT_REQUEST` reclaim engagement of §4(O.35), which OVMX cannot force (§4(O.35)'s 0/10 negative control). The directed-HELLO lead is refuted; it does NOT relocate the frontier off §4(O.35). No OVMX-forceable defect is isolated in this layer, so **no `src/vmsscs` change ships** (STOP at isolation, operator discipline).

**Explicit non-claims — the operator's live question is PRESERVED, not re-closed.** (1) This does NOT reinstate §4(O.33)/(O.35)'s "inherent race" as the answer — the operator's premise correction stands: real VMS rejoin is reliable, so a real-VAX-vs-OVMX DEFECT is expected. §4(O.36) removes ONE defect candidate (the directed-HELLO) and localises the surviving defect to the member-side CM/SYSAP `SCS$DIRECTORY`+`ACCEPT_REQUEST` reclaim engagement; what a returning REAL VAX does at THAT layer to reliably elicit both members' `ACCEPT_REQUEST` (and what OVMX presents differently) is the OPEN next frontier (§4(O.37)), NOT "inherent nondeterminism." (2) It does NOT claim OVMX's directed-HELLO is byte-perfect in every field — only that the channel-verify handshake, abs-16 dest-logical, and return-incarnation echo are correct and the channel reaches verified/START-complete with both members on the return. (3) It adds no reliability claim about the return outcome — that remains §4(O.35)'s reachable-not-reliable state; §4(O.36) only strikes the directed-HELLO from the defect table.

**Evidence** (host, tank volume; pure observation — no lab mutation, no fresh capture required as the current-HEAD directed-HELLO path is byte-identical to the captured daemon). Real-VAX return baselines `/data/training/vax/cluster/captures/vax3-2to3-established-join-20260730.pcap` and `…/vax3-class03-crash-REJOIN-SUCCESS-20260801.pcap`; §4(a).1 formation reference `…/clean-cluster/captures/formation-clean-2node.pcap` (decoder validated byte-exact: member `b2`=1/`b3`=72/`b4`=70, joiner `b2`=0/`b3`=70/`b4`=72). OVMX return captures `/data/training/vax/cluster/work/o32.pcap` (`o32.status`: return `admitted=1 engaged=0 XITDONE=0`) and `…/942-return-vs-firstjoin.pcap` (`942-J.log`: both peers `channel=UP STARTDONE incarnation=2 peer_padded_sca=1500`, `cm_responses=0`, `remote_conid=0`); daemon-currency diff `git diff 2d82d3f8 origin/main -- src/vmsscs/scs_hello.c` = empty and the scsd.c directed-HELLO responder region unchanged. Clean-room: the directed-HELLO decode reads only the reference-lab wire (offsets from §4(a)); no VSI/HPE source or binary was read. *VAXcluster Principles* pp. 7-23/7-24/7-25/7-39 (CM/JOIN context, cited by page only).

#### 4(O.37) THE CM/SYSAP RECLAIM *CONNECTION* IS NOT THE OVMX DEFECT — decoded on the wire, the `SCS$DIRECTORY` reconnect **and** the `VMS$VAXcluster` `ACCEPT_REQUEST` reclaim COMPLETE IDENTICALLY on the WIN (first-join) and the LOSE (return): on the return BOTH members open their `SCS$DIR_LOOKUP`→OVMX`.SCS$DIRECTORY` (OVMX answers both), BOTH members' `VMS$VAXcluster` VCs reach OPEN via `RCV_ACCEPT_REQ`, the member's poller runs the SAME p.2-26 open→lookup→disconnect, and OVMX reaches `dir_connected=YES`/`start_acked=1`/`vc=OPEN` with BOTH members — the SOLE divergence between the same-daemon F1-vs-J single-factor bracket is `cm_responses` (202/34 WIN vs **0/0** LOSE), i.e. the member's CM membership-COMMIT transaction, which is member-driven and races the residual-CSB deallocate+rebuild (§4(O.33)/(O.34)/(O.35)); so the operator's hypothesized reclaim-layer defect DOES NOT EXIST at this layer, and the one OVMX-side anomaly — a `SCSD-W-STRAYACK` (the member acks OVMX's op 0x01/op 0x02 **7654 ms** late vs the reference's ~1 ms) driving a **22 s** op-0x02 re-send deferral — is a SYMPTOM of the member being busy with its CSB reclaim, not the reclaim-connection gate, and sits in the RE'd crash-prone CM-message region (§4(O.29)); STOP at isolation, no `src/vmsscs` change (GROUNDED, `vms-694`/`vms-922`, pure-observation decode of the real-VAX return reclaim baseline vs the current-HEAD OVMX F1-vs-J single-factor return bracket on the tank volume, 2026-08-12)

**Frame.** §4(O.36) struck the directed-HELLO from the defect table and left the frontier at the member-driven `SCS$DIRECTORY`+`ACCEPT_REQUEST` reclaim engagement: *what does a returning REAL VAX present at THAT layer to reliably elicit both members' `ACCEPT_REQUEST` that OVMX does not?* Every prior increment read the reclaim from OVMX's derived verdicts or from the `VMS$VAXcluster`/CSB layer; NONE had decoded the `SCS$DIRECTORY` connection handshake AND the `VMS$VAXcluster` connect/accept, frame-by-frame, THROUGH a real-VAX return and an OVMX return and diffed them. §4(O.37) runs that decode. Method is pure observation (no wire guess toward a live VAX — the RE'd crash class, §4(O.29)); the OVMX captures are the current-HEAD single-factor F1-vs-J brackets `942`/`o32` whose daemon (`md5=d782c694…`) is byte-identical to `origin/main` `src/vmsscs` HEAD for the entire connect/accept/CM-commit path (`git diff 2d82d3f8 origin/main -- src/vmsscs/scsd.c` is READMIT verdict-TEXT only — the #346 classifier-honesty edit — with NO functional change to `scsd_svc_emit_dir_accept`, `scsd_svc_emit_member_accept`, or the CM-config/commit path), so no fresh capture is required (the §4(O.36) precedent).

**THE AUTHENTIC RECLAIM BASELINE (GROUNDED, real-VAX return `vax3-class03-crash-REJOIN-SUCCESS`).** VAX3 (`08:00:2b:11:22:33`) returns to VAX1 (`aa:00:04:00:01:04`) + VAX2 (`08:00:2b:78:56:b9`). Decoded (`tools/cluster/dissect_sca.py`, SYSAP name pair at the §4(h)(2)/§4(N) offsets, Con.ID pair, message-type byte abs-30), the `SCS$DIRECTORY` reclaim is member-driven, bidirectional, and completes into the commit within ~1.5 s:

| t (s) | frame | meaning |
|---|---|---|
| 110.060 | **VAX1 → VAX3** `SCS$DIR_LOOKUP`→`SCS$DIRECTORY`, mt `0x5b`, Con.ID `0`→`0c00d32f` | member VAX1 OPENS its directory-lookup connection to the returner |
| 110.062 | VAX3 → VAX1 `SCS$DIRECTORY`→`SCS$DIR_LOOKUP`, echoes `0c00d32f`, assigns `0700b945` | returner ANSWERS (CONNECT-ECHO/RESPONSE) |
| 111.162 | **VAX2 → VAX3** `SCS$DIR_LOOKUP`→`SCS$DIRECTORY`, Con.ID `0`→`0c002f0d` | member VAX2 OPENS its directory-lookup connection to the returner |
| 111.167 | VAX3 → VAX2 answers, assigns `0700ba45` | returner ANSWERS the other member |
| 111.472 | **VAX3 → VAX1** `SCS$DIR_LOOKUP`→`SCS$DIRECTORY`, mt `0x4b`, Con.ID `0`→`0700bb45` | returner opens ITS OWN directory-lookup back to VAX1 (post-connect discovery) |
| 111.60–111.67 | VAX2 → VAX3 **190-byte `0x4b` burst** | op-0x05 lock/resource-DB rebuild — the COMMIT / membership burst |
| 112.468 | VAX3 → VAX2 `SCS$DIR_LOOKUP`→`SCS$DIRECTORY` | returner opens its directory back to VAX2 — **AFTER** the commit (Process Poller discovery, not a JOIN gate) |

Both members open `SCS$DIRECTORY` to the returner and the returner answers; the returner's OWN directory connections to the members straddle the commit (VAX1 before, VAX2 after), confirming they are poller discovery, not the admission gate.

**OVMX'S RETURN — the reclaim CONNECTION is byte-consistent, and IDENTICAL WIN-vs-LOSE (GROUNDED, current-HEAD `942`/`o32`, OVMX=`62:7a:6c:c2:f0:7d`).** Time-split the OVMX return (J) arm and compare against the OVMX first-join (F1) arm of the SAME single-factor bracket:

| reclaim observable | F1 first-join (WIN, `admitted=2`) | J return (LOSE, `admitted=0/1`) |
|---|---|---|
| members that open `SCS$DIR_LOOKUP`→OVMX`.SCS$DIRECTORY` (OVMX answers) | 2 (t≈26.1, 27.2) | **2** (t≈102.3, 103.3) |
| OVMX `SCS$DIRECTORY` server reaches OPEN then p.2-26 poller DISCONNECT | both (`82F30007`,`82F30017`) | **both** (`F32D0007`,`F32D0017`) — IDENTICAL dialogue |
| members' `VMS$VAXcluster` VC connect+ACCEPT (94-byte `0x4b`) with OVMX | both | **both** (t≈102.263, 103.269) |
| OVMX joiner `VMS$VAXcluster` VC → OPEN via `RCV_ACCEPT_REQ` | both | **both** (`SCSD-I-VAXCLMEMBER … reached OPEN`) |
| OVMX per-peer final: `vc=OPEN channel=UP dir_connected=YES start_acked=1` | both | **both** |
| **`cm_responses` (member's CM membership-COMMIT)** | **202 / 34** | **0 / 0** |

The entire SCS-connection reclaim — the `SCS$DIRECTORY` reconnect (both members open, OVMX answers, the p.2-26 poller open→lookup→disconnect) AND the `VMS$VAXcluster` `ACCEPT_REQUEST` (both members' VCs reach OPEN) — is BYTE-FOR-BYTE the same on the WIN and the LOSE. The `SCS$DIRECTORY` open-then-disconnect that first looked like a return anomaly is present IDENTICALLY on the winning first-join, so it is the normal poller dialogue, not a discriminator. **The SOLE divergence is `cm_responses`: on the WIN the member runs the CM membership-COMMIT (op 0x02 → op 0x12 RELAY → op 0x03 COMMIT → op 0x05 lock-rebuild → op 0x06 MEMB, `cm_responses=202/34`); on the LOSE it runs ZERO.**

**The isolated gap, and the OVMX-side anomaly that is NOT the gate.** The member's CM-COMMIT is member-driven (`ACCEPT_REQUEST`/op-0x03 are the member CM's decision, §4(O.35)) and races the residual-CSB deallocate+rebuild (§4(O.33), authentic per §4(O.34)); OVMX cannot force it (§4(O.35)'s `OVMX_JOIN_ALL_MEMBERS` negative control lost 0/10). On the LOSE the member is DEMONSTRABLY slow to engage OVMX's join: `SCSD-W-STRAYACK` records the member acking OVMX's `msg#2` (a cat 0x01 op 0x01) **7654 ms** after OVMX sent it — where the real-VAX reference acks within ~1 ms and never acks an op 0x01 — and OVMX, its message numbering confused by the late ack, re-drives its DEFERRED op 0x02 at **t+22 s** (`SCSD-I-CMCONFIG2`), long past the member's transition window. This is a real OVMX-side latency anomaly and the strongest surviving defect CANDIDATE, but it is (1) a SYMPTOM of the member being busy reclaiming its CSB (the member's `cm_responses=0` is the primary fact, and it holds even in the run where the abort precedes the deferred op 0x02, §4(O.32)/(O.33)), (2) NOT at the reclaim-CONNECTION layer the operator named (that layer completes identically), and (3) sited in the CM-message ordering region whose speculative mutation toward a live VAX is the RE'd crash class (§4(O.29) op-0x0d bugcheck).

**Verdict — the reclaim-CONNECTION defect does not exist; STOP at isolation, no `src/vmsscs` change.** The `SCS$DIRECTORY`+`ACCEPT_REQUEST` reclaim CONNECTION completes identically on the WIN and the LOSE and matches the real-VAX baseline shape (both members open `SCS$DIRECTORY`, the returner answers, the `VMS$VAXcluster` VCs reach OPEN). There is no OVMX-forceable defect at the layer §4(O.36) pointed §4(O.37) to. The surviving divergence — the member's CM-COMMIT decision, `cm_responses=0` — is member-driven and not cleanly OVMX-forceable (§4(O.35)), and the one OVMX latency anomaly (STRAYACK / 22 s op-0x02 deferral) is neither proven causal nor safely mutable here. Per the operator's discipline (ship only a cleanly-isolated, non-crash-prone, validatable fix; else STOP), **no fix ships.**

**Relocated frontier (§4(O.38)).** The operator's premise is PRESERVED, not re-closed: real VMS rejoin is reliable, so an OVMX defect is still expected — but §4(O.37) removes the reclaim CONNECTION (`SCS$DIRECTORY`+`ACCEPT_REQUEST`) from the defect table (identical win-and-lose) and re-points the surviving candidate at OVMX's **CM-message ordering/numbering and ack-timing on the return** — the STRAYACK: OVMX presents its op 0x01/op 0x02 in an order/numbering that the member acks 7.6 s late (reference ~1 ms), and OVMX's own message-number machinery then strands the join in a 22 s re-send. §4(O.38) grounds, by PURE OBSERVATION, the real-VAX return's CM-message SEQUENCE and the member's ack-latency at the commit layer (op 0x01/op 0x02/op 0x03/op 0x12 ordering; the ~1 ms ack cadence), and diffs OVMX's — to determine whether OVMX's ordering is the defect that keeps the member from committing, BEFORE any wire change in that crash-prone region.

**Explicit non-claims.** (1) This does NOT reinstate §4(O.33)/(O.35)'s "inherent race" as the final answer — it NARROWS where an OVMX defect can be: NOT the `SCS$DIRECTORY`/`ACCEPT_REQUEST` reclaim connection (identical win-and-lose), possibly OVMX's CM-message ordering (the STRAYACK, §4(O.38)). (2) It does NOT claim OVMX's whole CM path is byte-perfect — only that the reclaim CONNECTION (the `SCS$DIRECTORY` handshake and the `VMS$VAXcluster` connect/accept to OPEN) completes identically on the WIN and LOSE and matches the real-VAX baseline shape. (3) The STRAYACK / 22 s op-0x02 deferral is FLAGGED as the §4(O.38) target, not shipped as a proven cause — the member's `cm_responses=0` is the primary fact and the STRAYACK is a symptom of it. (4) No reliability claim about the return outcome is added or removed — it remains §4(O.35)'s reachable-not-reliable state; §4(O.37) only strikes the reclaim CONNECTION from the defect table and localises the surviving candidate. (5) Pure observation on current-HEAD captures (byte-identical connect/accept/CM path, §4(O.36) precedent) — no lab mutation.

**Evidence** (host, tank volume; pure observation — no lab mutation, byte-identical current-HEAD path). Real-VAX reclaim baseline: `/data/training/vax/cluster/captures/vax3-class03-crash-REJOIN-SUCCESS-20260801.pcap` (VAX3 reclaim t≈110.06–112.47: both members open `SCS$DIRECTORY`, returner answers, op-0x05 commit burst t≈111.6). OVMX current-HEAD single-factor F1-vs-J brackets (daemon `md5=d782c6943123f3a34d2d0270090c10d3` == `origin/main` `src/vmsscs` HEAD `2d82d3f8` for the connect/accept/CM path): `/data/training/vax/cluster/work/o32.pcap` (J arm t≈102–103: both members' `SCS$DIRECTORY` + `VMS$VAXcluster` reach connect+ACCEPT with OVMX; `o32.status` return `admitted=1 engaged=0`) and `/data/training/vax/cluster/work/942-return-vs-firstjoin.pcap` + `942-F1.log`/`942-J.log` (F1 `cm_responses=202/34 admitted=2 SCSD-I-XITDONE`; J `cm_responses=0/0`, `SCS$DIRECTORY` open→p.2-26-disconnect IDENTICAL to F1, `SCSD-W-STRAYACK 7654 ms`, `SCSD-I-CMCONFIG2` deferred op 0x02 at t+22 s, both peers `vc=OPEN channel=UP dir_connected=YES start_acked=1`). Member oracle (LOSE vs WIN): `o35dbg.timeline` (both members' CSB `BRK_NON`→`NEW`, `CLUSTER_NODES` stays 2) vs `o35rel.timeline` (`CLUSTER_NODES` 2→3). Daemon-currency diff: `git diff 2d82d3f8 origin/main -- src/vmsscs/scsd.c` = READMIT verdict-TEXT only (#346 classifier honesty; no connect/accept/CM-commit change). Decoder `tools/cluster/dissect_sca.py` (SYSAP name pair + Con.ID + abs-30 message-type). Clean-room: only reference-lab wire (offsets from §4(h)/§4(N)) and public docs read; no VSI/HPE source or binary. *VAXcluster Principles* pp. 2-25/2-26 (connect data / poller disconnect dialogue), 7-23/7-24/7-25 (CSB states; reclaim = deallocate old CSB + build new "as if joining for the first time"), 7-39 (Rule of Total Connectivity).

#### 4(O.38) THE "7.6 s-LATE CM ACK" IS NOT AN OVMX DEFECT — it is REFUTED on fresh member-oracle-grounded wire: on both a WIN and a genuine `CLUSTER_NODES`-confirmed LOSE the engaging member acks OVMX's `op 0x14`/`op 0x01`/`op 0x02` in **~0.3–0.5 ms** (reference ~1 ms), so the initial CM-config handshake is PROMPT, not 7.6 s late; the `7654 ms` figure §4(O.37) read as "the member acks OVMX's op 0x01 late" is OVMX's own `SCSD-W-STRAYACK` INSTRUMENTATION — OVMX EMITTING a periodic `cat 0x04` ack that names a frame **OVMX itself received** seconds earlier (measured this run: `cat 0x02 op 0x01` DLM-lock and `cat 0x01 op 0x0c` backlog, NEVER a config `op 0x01`) during a member lull, a direction-reversed + mislabelled read of the same class the progression has caught ≥3× (§4(O.25)/(O.31)/(O.32)/(O.33)); the only genuine multi-second latency is OVMX's OWN **deferred `op 0x02`** (`SCSD-I-CMCONFIG2`, ~20 s retry) which the member acks in 0.8 ms but which is INERT because that member already committed at t+3 ms — and the REAL win/lose gate, decoded on a fresh genuine LOSE, is §4(O.35)'s member-driven engagement (WIN = OVMX exchanges CM config with BOTH members; LOSE = only the coordinator engages, the second member never opens its CM connection to the returner and OVMX cannot force it, §4(O.35) lever 0/10); rejoin measures **4/5 member-oracle WINs** (`F$GETSYI("CLUSTER_NODES")` 2→3 on BOTH members), and OVMX's own `XITDONE`/`cm_responses` self-verdict is an UNRELIABLE predictor (a member-oracle WIN ran `XITDONE=0`), so §4(O.33)–(O.35)'s "reachable-not-reliable / reliably-loses" was substantially reading OVMX's untrustworthy verdict, not the authoritative oracle; STOP at isolation, no `src/vmsscs` change (GROUNDED, `vms-694`, FRESH single-factor first-join-vs-return bracket + 5-return oracle reliability run on VIRGIN `vaxlab-3`, VAX `F$GETSYI("CLUSTER_NODES")` oracle + tcpdump + `tools/cluster/dissect_sca.py` CM-message decode, current-HEAD Debug daemon `md5=17c25b26` == `origin/main` `src/vmsscs` HEAD `e3d908ce`, 2026-08-12).

**Frame.** §4(O.37) struck the reclaim CONNECTION from the defect table (identical win-and-lose) and re-pointed the surviving candidate at OVMX's CM-message ordering/numbering and ack-timing — specifically the `SCSD-W-STRAYACK` reading that "the member acks OVMX's `op 0x01`/`op 0x02` **7654 ms** late vs the reference's ~1 ms," driving a ~20 s `op 0x02` re-send. Every prior §4(O.3x) increment (bar the §4(O.35) oracle read) leaned on OVMX-derived captures/verdicts; §4(O.37) explicitly SKIPPED a fresh member-SDA LOSE. §4(O.38) does that fresh run: a virgin pod, a single-factor first-join-vs-return bracket, a five-return reliability sweep read ONLY through each member's `F$GETSYI("CLUSTER_NODES")` (the oracle OVMX cannot colour), and a frame-by-frame CM-message decode of a WIN and a genuine oracle-confirmed LOSE. Method is pure observation (no wire guess toward a live VAX — the RE'd crash class, §4(O.29)).

**Finding 1 — the 7.6 s-late ack is REFUTED; it was an OVMX-log misread.** On the fresh return, the members ack OVMX's CM config PROMPTLY: `OVMX op 0x14/op 0x01/op 0x02 snd=1/2/3 @t → member acks (amsg=3) @t+0.3–0.5 ms`, on the WIN AND on the genuine LOSE, matching the reference's ~1 ms reciprocation cadence (Davis-grounded, `src/vmsscs/scsd.c` `vms-e81` note). The `7654 ms` of §4(O.37) is not a member ack latency at all: `SCSD-W-STRAYACK` fires inside OVMX's OWN `cat 0x04` ack emitter (`scsd.c`, `vms-584` instrumentation) when OVMX's receive high-water has not advanced for ≥1 s — it logs OVMX acking a frame **OVMX received** seconds ago, and every stray this run named a `cat 0x02 op 0x01` (DLM lock) or `cat 0x01 op 0x0c` / `cat 0x04 op 0x0c` backlog frame, NEVER a config `op 0x01`. Reading "the member acks OVMX's op 0x01 late" reversed the direction and mislabelled the frame — the identical OVMX-verdict-misread class flagged as the ⚠⚠ RECURRING ROOT LESSON. The only genuine multi-second latency measured is OVMX's OWN **deferred `op 0x02`** (`SCSD-I-CMCONFIG2`, re-driven at ~t+20 s), which the member acks in 0.8 ms — but that member had already sent `op 0x03 COMMIT` at t+3 ms, so the deferred `op 0x02` and its "late" ack are redundant/inert, not a gate.

**Finding 2 — rejoin is member-oracle-reliable (4/5), and `XITDONE` is an unreliable predictor.** Five consecutive same-identity returns on the virgin pod, read by each member's `CLUSTER_NODES`: `WIN WIN WIN LOSE WIN` = **4/5** genuine admissions (`CLUSTER_NODES` 2→3 on BOTH members). OVMX's own `XITDONE` disagreed with the oracle: return 3 was a member-oracle WIN with `XITDONE=0`. So OVMX under-reports its own success, and §4(O.33)–(O.35)'s "reachable-not-reliable / Debug reliably-loses" framing was substantially an artifact of reading OVMX's self-verdict rather than the member oracle — the current-HEAD Debug daemon (`md5=17c25b26`, the very build §4(O.35) called "reliably loses") in fact WON 4/5. This does not claim 10/10, but it flatly contradicts "reliably loses" and moves OVMX much closer to the operator's "real VMS rejoin is reliable" premise than any prior increment reported.

**Finding 3 — the surviving LOSE is §4(O.35)'s member-driven second-member engagement, decoded fresh.** On the genuine oracle-confirmed LOSE, the coordinating member (`d5:24:fe`) engaged and committed PROMPTLY — `op 0x14/op 0x01/op 0x02` exchange, `op 0x03 COMMIT` at t+3 ms, `op 0x05 LOCKRB` — nothing OVMX sent it was late or refused. The OTHER member (`aa:00:04:00:01:04`) NEVER entered the CM config exchange at all: OVMX↔that-member carried only channel HELLO (`cat 0x00`, verified per §4(O.36)) plus pre-existing steady-state app traffic — ZERO `cat 0x01 op 0x14/op 0x01/op 0x02` in either direction, i.e. that member never opened its CM connection to the returner and OVMX therefore never drove config to it. On the WIN, by contrast, OVMX exchanges the `op 0x14/op 0x01/op 0x02` config with BOTH members. This is exactly §4(O.35)'s Rule-of-Total-Connectivity gate (Davis p.7-39): a WIN needs BOTH members to engage, a LOSE has only one, the engaging member always commits, and the missing member's non-engagement is its residual-CSB reclaim/CM state (§4(O.33)/(O.34)), member-driven — the one OVMX lever to force it (`OVMX_JOIN_ALL_MEMBERS`) was already measured to LOSE 0/10 (§4(O.35)). The real-VAX reference return has the SAME structure (only the coordinator runs `op 0x02`→`op 0x03`→`op 0x05`→`op 0x06`; the other member only exchanges `op 0x14`/`op 0x01`), so OVMX's per-member commit shape is authentic.

**Verdict — the 7.6 s-late-ack defect does not exist; STOP at isolation, no `src/vmsscs` change.** The §4(O.38) target dissolves under fresh measurement: the initial CM-config ack is prompt (~0.5 ms) on both WIN and LOSE, the `7654 ms` was OVMX's own stray-ack instrumentation (mislabelled, direction-reversed), and the ~20 s deferral is OVMX's own inert retry to an already-committed member. The surviving win/lose divergence is the member-driven second-member engagement of §4(O.35), not OVMX-forceable (lever 0/10, and any mutation sits in the RE'd crash-prone CM region, §4(O.29)). Per the operator's discipline (ship only a cleanly-isolated, non-crash-prone, validatable fix; else STOP), **no fix ships.**

**Is there ANY remaining OVMX-side lever? (honesty note — 6th consecutive narrowing increment.)** One clean OVMX-side defect IS isolated, but it is NOT a rejoin-wire fix: **OVMX's `XITDONE`/`cm_responses` self-verdict is wrong — it reports LOSE on a genuine member-oracle WIN** (return 3). Correcting OVMX's admission self-diagnosis is OVMX-forceable and non-crash-prone (local reporting logic, not a wire change) and would end the phantom-LOSE verdicts that have misled ≥5 increments — but it would NOT convert the true 1/5 LOSE (member never engages) into a WIN, so it does not meet the "prove reliable rejoin ≥10 returns" bar, and touching the verdict emitter risks the `vms-584` "a guard that hides a bug is worse than no guard" trap. It is SURFACED for an operator/conductor call (fix OVMX's rejoin self-diagnosis as a separate honest-instrument item), not shipped here. Beyond that, NO new OVMX-forceable rejoin-wire lever remains: the admission gate is the member's decision to open its CM connection to the returner, which OVMX cannot force and which real VMS itself does not always run instantly (the residual-CSB reclaim, authentic per §4(O.34)). The operator's premise is preserved and, importantly, now largely MET on the wire: measured by the authoritative oracle, OVMX rejoins 4/5, not "reliably loses."

**Explicit non-claims.** (1) This does NOT claim OVMX rejoin is 10/10 reliable — it measures 4/5 on a five-return sweep and one genuine LOSE; the point is that the rate read by the member oracle is FAR higher than OVMX's self-verdict suggested, refuting "reliably loses." (2) It does NOT claim the `XITDONE` self-verdict fix would improve the admission rate — it would only make OVMX's reporting honest and stop the phantom-LOSE misreads. (3) It does NOT re-open the reclaim CONNECTION (§4(O.37), identical win-and-lose) or the directed-HELLO (§4(O.36), byte-correct) — both stay struck from the defect table. (4) It removes the STRAYACK / 7.6 s-late-ack from the defect table as an OVMX-log artifact, and confirms the surviving gate is §4(O.35)'s member-driven engagement, not OVMX CM-message ordering. (5) Fresh lab run, member-oracle-grounded, single-factor bracket + reliability sweep; no wire mutation toward a live VAX.

**Evidence** (host, tank volume; FRESH virgin-pod run, member-oracle-grounded). Bracket + reliability harnesses `/data/training/vax/cluster/work/o38run.sh` (single-factor F1-vs-J) and `o38oracle.sh` (5-return `CLUSTER_NODES` sweep) on virgin `vaxlab-3` (scaled up from arrival count 3, cleaned back to 3 after, peers `vaxlab-0/1/2` preserved). Oracle result `o38o.status`: `RESULT: 4/5 wins; sequence: WIN WIN WIN LOSE WIN`; return 3 = member-oracle WIN with `XITDONE=0`. Captures (pod-local, epoch-aligned): `o38.pcap` (F1-vs-J), `o38o-r4.pcap` (genuine LOSE — engaging member commits at t+3 ms, second member `aa:00:04:00:01:04` has ZERO `cat 0x01` CM config, only `cat 0x00` HELLO), `o38o-r5.pcap` (WIN — OVMX exchanges `op 0x14/op 0x01/op 0x02` with BOTH members). CM-message decode `o38_analyze.py` (offsets from spec §4(g)/§4(N) + `src/vmsscs/scs_member.c`: `[30]` msgtype, body@72 `[72:74]` send_msg / `[74:76]` amsg / `[80]` category / `[81]` opcode). Real-VAX reference return `captures/vax3-class03-crash-REJOIN-SUCCESS-20260801.pcap` (t≈110–112: both members send `op 0x14`/`op 0x01` to the returner, coordinator VAX2 runs `op 0x02`→`op 0x03`→`op 0x05`→`op 0x06`). STRAYACK named-frame census from `scsd-o38-J.log` (all stale `cat 0x02`/`cat 0x01 op 0x0c`/`cat 0x04` backlog, none a config `op 0x01`). Daemon-currency: `md5=17c25b26` == `origin/main` `src/vmsscs` HEAD `e3d908ce` (the §4(O.35) build). Clean-room: only reference-lab wire and public docs read; no VSI/HPE source or binary. *VAXcluster Principles* pp. 2-25/2-26 (connect data / poller), 7-23/7-24/7-25 (CSB reclaim = deallocate + rebuild at rejoin), 7-39 (Rule of Total Connectivity).


#### 4(O.39) OVMX'S REJOIN ADMISSION SELF-REPORT IS FIXED — `XITDONE` now tracks the member oracle, ending the phantom-LOSE. The AUTHENTIC admission signal is the coordinator's RECEIVED `cat 0x01 op 0x06` MEMBERSHIP burst (its post-commit membership publication), which is 1:1 with `F$GETSYI("CLUSTER_NODES")` 2→3 across a fresh five-return sweep INCLUDING both genuine LOSEs; the pre-fix `XITDONE` derived from OVMX's OWN `op 0x0c` barrier completion, a DIFFERENT event that RACES the coordinator's fan-out and is wrong in BOTH directions — a PHANTOM-LOSE when the barrier stalls part-way on a genuine WIN (§4(O.38) r3), and a FALSE-WIN when the barrier completes on a genuine LOSE (this run's `v197ref` r5). §4(O.39) grounds the op 0x06 burst as the 1:1 admission signal on the wire, repoints `XITDONE` (latched once) at it, renames the barrier line to `SCSD-I-XITBARRIER`, and adds an authoritative `membership_committed=` field to the `READMITMAP-SUMMARY`. DIAGNOSTICS-ONLY: the op 0x06 frame is one OVMX already receives, acks and traces — the diff adds two per-run counters, a latch, the self-report emission and the rename, and touches NO wire emission, NO SCS/CM state machine and NO emitted frame (GROUNDED, `vms-197`/`vms-694`, before/after single-factor five-return oracle brackets on VIRGIN `vaxlab-3`, unpatched ref daemon `md5=17c25b26` == `origin/main` HEAD vs patched `md5=18ee0765`, VAX `F$GETSYI("CLUSTER_NODES")` oracle + tcpdump + `o38_analyze.py` op-0x06 census, 2026-08-12).

**Frame.** §4(O.38) measured rejoin at 4/5 member-oracle WINs but flagged that OVMX's own `XITDONE` self-verdict is an UNRELIABLE predictor: its return 3 was a member-oracle WIN (`CLUSTER_NODES` 2→3 on BOTH members) that self-reported `XITDONE=0`. That broken self-verdict — not any wire defect — is what dragged §4(O.33)–(O.37) in circles chasing a non-defect (each increment read OVMX's derived verdict as if it were the membership fact). §4(O.39) fixes the self-report itself: it grounds, on the wire, the RECEIVED signal that correlates 1:1 with the member oracle, and drives `XITDONE` from THAT. Method is pure observation (no wire guess toward a live VAX — the RE'd crash class, §4(O.29)).

**Finding — the op 0x06 MEMBERSHIP burst is the 1:1 admission signal; the barrier is not.** A per-return `cat 0x01 op 0x06` receive census over the unpatched-daemon five-return bracket (`v197ref`, `md5=17c25b26`), decoded by OVMX's true HW MAC this pod boot (`7e:66:37:58:1b:94`; coordinator `08:00:2b:d5:24:fe`, second member `aa:00:04:00:01:04`):

| return | member oracle | recv op 0x06 (from coordinator) | pre-fix `XITDONE` (barrier) |
|--------|---------------|--------------------------------:|:---------------------------:|
| r1 | **WIN** (3/3) | **254** | **0**  ← phantom-LOSE |
| r2 | **WIN** (3/3) | **254** | **0**  ← phantom-LOSE |
| r3 | LOSE (2/2)    | **0**   | 0 |
| r4 | **WIN** (3/3) | **254** | 1 |
| r5 | LOSE (2/2)    | **0**   | **1**  ← FALSE-WIN |

The op 0x06 burst is present (254 frames) on EVERY member-oracle WIN and ABSENT (0) on EVERY genuine LOSE — 1:1 across all five returns, including both LOSEs. The pre-fix `XITDONE` (OVMX's own `op 0x0c` barrier completion) disagreed with the oracle on 3/5 returns, and in BOTH directions. This is authentic: the coordinator publishes op 0x06 only once the transition COMMITS cluster-wide, which requires the Rule of Total Connectivity (Davis p.7-39) — the exact condition the oracle measures — so a LOSE (only one member engaged) never elicits it, while the racy barrier can stall on a WIN or complete on a LOSE. On the WIN the real-VAX reference return has the coordinator run `op 0x02`→`op 0x03`→`op 0x05`→**`op 0x06`** (§4(O.38)); op 0x06 is that sequence's terminal publication.

**The fix (`src/vmsscs/scsd.c`, diagnostics-only).** (1) New per-peer `membership_bursts` counter and a cluster-wide `ovmx_cluster.membership_committed` latch, set from a PURE READ of the received `cat 0x01 op 0x06` frame (the same frame the ack-cadence block already consumes — no new wire behaviour). (2) `SCSD-I-XITDONE` is now emitted ONCE, from the first op 0x06 receipt, as the authentic admission self-report; the former barrier-completion `XITDONE` line is renamed `SCSD-I-XITBARRIER` and explicitly annotated as NOT the admission verdict. (3) `READMITMAP-SUMMARY` gains `membership_committed=YES/no (op 0x06 burst)`, the one self-report field grounded 1:1 against the member oracle, and the per-peer `READMITMAP` line gains `membership_bursts=`. The #346 honest-classifier work is preserved (the per-peer OVMX-side buckets are unchanged; only the authoritative-aligned cluster fact is added). The kill-switch discipline holds: `git diff` touches only the self-report/verdict path — the CM state machine, `send_seq`/`recv_ack`, `cm_send_*`, and every emitted frame are byte-identical.

**Proof — `XITDONE` now agrees with the member oracle (patched daemon `md5=18ee0765`).** Same virgin pod, same `o38oracle.sh` five-return sweep:

| return | member oracle | recv op 0x06 | patched `XITDONE` (op 0x06 latch) | `membership_committed` | agrees? |
|--------|---------------|-------------:|:---------------------------------:|:----------------------:|:-------:|
| r1 | WIN (3/3) | 254 | 1 | YES | ✓ |
| r2 | WIN (3/3) | 254 | 1 | YES | ✓ |
| r3 | WIN (3/3) | 254 | 1 | YES | ✓ |
| r4 | WIN (3/3) | 254 | 1 | YES | ✓ |
| r5 | WIN (3/3) | 254 | 1 | YES | ✓ |

Every return: op 0x06 received ⟹ `XITDONE` emitted exactly once (latched) ⟹ `membership_committed=YES`, and the barrier line (`SCSD-I-XITBARRIER`) fires independently of it. `XITDONE` is now a pure function of the received op 0x06 burst, which the unpatched bracket proved 1:1 with the oracle in both directions — so a genuine LOSE (op 0x06 absent) yields `XITDONE=0` by construction, and the phantom-LOSE and false-WIN are BOTH gone. This does NOT change the admission RATE (a member-side item, §4(O.35)); it makes the SELF-REPORT honest and stops the phantom-LOSE misreads that misled ≥5 prior increments.

**Verdict — the self-report defect is FIXED; diagnostics-only, no wire change.** `XITDONE` tracks the authoritative member oracle. The one OVMX lever that would change the admission rate remains member-side and not OVMX-forceable (§4(O.35), 0/10); §4(O.39) does not touch it. What §4(O.39) removes is the untrustworthy self-verdict that made a 4/5-reliable rejoin read as a repeated failure.

**Explicit non-claims.** (1) No admission-RATE claim: the fix corrects OVMX's REPORT, not the member-side second-member engagement gate (§4(O.35)). (2) No wire/state-machine change: the op 0x06 frame is received/acked/traced exactly as before; only OVMX's conclusion about its own admission changed. (3) op 0x06 as the 1:1 signal is grounded on THIS pod's fresh bracket (both LOSEs op06=0, all WINs op06=254) and matches §4(O.38)'s captures; it is not asserted from prose. (4) The barrier is a real event, not a bug — it is simply not the admission verdict, so it is retained under its own marker. (5) A direct LOSE→`XITDONE=0` capture with the PATCHED daemon is a nice-to-have not gathered this run (a genuine LOSE is only ~1/5 and the after-brackets drew all WINs); it is unnecessary because the fix sets `XITDONE` from the RECEIVED CM admission signal (the op 0x06 membership burst), so a genuine LOSE — on which that burst is absent, proven 1:1 on the unpatched bracket's two LOSEs — reports `XITDONE=0` by construction.

**Evidence** (host, tank volume; FRESH virgin-pod before/after brackets). Sweeps `POD=vaxlab-3 o38oracle.sh` (scaled up from arrival count 3 to a virgin `vaxlab-3`, cleaned back to 3 after, peers `vaxlab-0/1/2` preserved). BEFORE `v197ref.status` (unpatched `md5=17c25b26` == `origin/main src/vmsscs` HEAD): `RESULT: 3/5 wins; sequence: WIN WIN LOSE WIN LOSE`, pre-fix `XITDONE` = `0 0 0 1 1` (disagrees on r1/r2 phantom-LOSE + r5 false-WIN). AFTER `v197fix.status` (patched `md5=18ee0765`): `RESULT: 5/5 wins`, `XITDONE=1` on every return, all agreeing with the oracle. op 0x06 census `o38_analyze.py` over `v197ref-r1..r5.pcap` = `254 254 0 254 0` (1:1 with the oracle `WIN WIN LOSE WIN LOSE`); per-return daemon logs `scsd-v197fix-r*.log` confirm `SCSD-I-XITDONE` (op 0x06 latch, once), `READMITMAP peer ... membership_bursts=254` on the coordinator / `=0` on the second member, and `READMITMAP-SUMMARY ... membership_committed=YES`. Unit test `tests/vmsscs/test_scsd_wire.c` green (default wire path unchanged). Clean-room: only reference-lab wire and public docs; no VSI/HPE source or binary. *VAXcluster Principles* pp. 7-23/7-24/7-25 (CSB reclaim), **7-39 (Rule of Total Connectivity)**; the op 0x02→op 0x03→op 0x05→op 0x06 coordinator commit sequence per §4(O.38)/Davis pp. 7-37/7-38.
