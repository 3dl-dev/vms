				   ...
				   Elf64_Phdr *ephdr, int ephnum)
{
	...
	/* Locate this executable's own .init_array range (if any) while `fd` is
	 * still open -- ovmx_find_section reads the file's raw section headers,
	 * same mechanism as OVMX_IMP_SECTION/OVMX_TLS_SECTION above. Zeroed when
	 * absent, which is the correct empty range (vms-ee2). */
	unsigned long initarr_addr = 0, initarr_size = 0;
	ovmx_find_section(fd, ELF_INIT_ARRAY_SECTION, &initarr_addr, &initarr_size);

	/* Locate this executable's .vms$ehf DWARF frame-registration descriptor
	 * (if any) while fd is still open (vms-70d). Zeroed when absent -- the
	 * correct "nothing to register" state for a pure-C image. */
	unsigned long ehf_addr = 0, ehf_size = 0;
	ovmx_find_section(fd, OVMX_EHF_SECTION, &ehf_addr, &ehf_size);
