...
A="${ART_ROOT:-$R/art}"
case "${1:-}" in
  cap) setsid nohup tcpdump -i br4f0 -s0 -w "$R/cn3.pcap" 'ether proto 0x6007' > "$R/tcpdump.out" 2>&1 < /dev/null & ;;
  C)   cd $R && setsid nohup python3 $R/nodedrv.py $R/nodeC $R/VAXC.console.log --boot "B DUA0" > $R/nodeC.drv.out 2>&1 < /dev/null & ;;
  A)   setsid nohup env ART=$A/A OUT=$R/OVMXA.console.log TAP=tapA4f0 \
  ...
  B)   setsid nohup env ART=$A/B OUT=$R/OVMXB.console.log TAP=tapB4f0 \
