...
### E69 — ✅ LANDED (fc-e69 @ f9db0e48, integrated tip f9db0e48, 60/60 + unit 109/109 + gate OK 73 + guest-payload PASS + image-parity OK + kif census PASS+negctl + vms.ko clean + elf32-vax ALL PROOFS PASSED). CNXMAN join FSM diag ring: `vms_cnxman_diag.{h,c}` 256-slot coalescing ring (DISPATCH/ARRIVAL/EMIT records; EMIT `detail` = the gate enum SENT/no-csb/no-open-vc/no-send-op/scs-refused/codec-refused = WHERE the MODEL/PARAMS emit is blocked; SCS seam records cdt-open vs cm-accept half; coalesces the 1Hz watchdog so the drive's first record survives to the ~600s dump). ioctl `VMS_IOCTL_CLUSTER_DIAG_JOIN` (0xC420566D, ahead of reg gate, NetBSD mirrored). Dumper `SYS$SYSTEM:CNXTRACE.EXE` both arches, staged. labjoin_pod_boot.sh prints it between `OVMX-CNXTRACE-BEGIN`/`-END` markers after JOIN_POLL — integrator reads via `sed -n '/OVMX-CNXTRACE-BEGIN/,/OVMX-CNXTRACE-END/p' $HOSTL/ovmx-node-$TAG.log`. **INV-6: zero wire-behavior change (memcmp-proven).** Escalation: the ring will now SHOW the FAILED-is-terminal fragility (DISPATCH into FAILED w/ detail=EMPTY-CELL). NEXT: DIAGNOSTIC re-fire captures the trace → E70 reads WHERE the promotion drive is gated.
...
### E69 DIAGNOSTIC RE-FIRE (vaxlab-2, TAG e69refire-1788501429) — ✅ CNXTRACE CAPTURED, gate PINPOINTED. CNXTRACE.EXE ran clean (25 records). join state=FAILED failure="path lost" ignored_events=5. **THE PROMOTION LOGIC FIRES** — seq 13-15: EMIT MODEL(cat01/op0x14) + PARAMS(op0x01) + CONFIG(op0x02) in states ADVERTISE/ADMIT — **but ALL THREE `detail=scs-refused rc=-1`** on connection handle `0x0a7c000b` (the cm-accept connection from seq5, member-opened+OVMX-accepted). SCS REFUSES THE SEND → zero CM frames on the wire (cross-checked). Timeline: seq5 cm-accept ...000b (t=16024); seq7 **CDT_OPEN ...000b hit EMPTY-CELL in DIR_ROUND (FSM did NOT react to the connection opening)**; seq11 DIR_RESULT→MSCP_CONNECT; seq13-15 advertise on ...000b → scs-refused; seq18 ...000b cdt-closed rc=5 ("path lost") → seq19 ADMIT→FAILED; seq21 a 2nd cm-accept ...000e (the other VAX) arrives at t=17923 but FSM already FAILED → EMPTY-CELL dropped (=ignored_events). The emit-gate says `scs-refused` NOT no-csb/no-open-vc/codec-refused → the VC was present + frame built, but the SCS SEND op returned -1. → E70.

### E70 — ✅ LANDED (fc-e70 @ abbb7af7, integrated tip abbb7af7, 60/60 + unit 109/109 + gate OK + NetBSD ELF32 ALL PROOFS PASSED + Linux clean). The `rc=-1` was the REASON BEING DESTROYED: `cnxman_fsm_rc()` was `SS$_NORMAL?0:-1` (flattened every status, against its own contract), and `scs_glue_status()` is many-to-one. Credit hypothesis DISPROVEN (creditless CM send → bound Credit-Wait pool → SENT not refused). Narrowed to silent DISC_RCVD or PORT-level refusal (invisible before). FIX: (a) return the real refusal not -1; (b) CDT keeps last refusal + the PORT's verbatim code; (c) **re-offer the refused burst on the join's beat** (join_reoffer_burst, grounded p.2-51 "the poller REPEATS"; only never-sent messages, in §4(o) order, fresh send-msg#); (d) **full port-refusal observability** in the ring/CNXTRACE — named causes port-nocircuit/nocredit/ringfull/badframe/txfail/cdt-not-sendable(+cdtstate,credit_send), all live reads from pe_vc/scs_cdt (INV-6). So the next re-fire either SUCCEEDS (re-offer unblocks a transient refusal) or the trace names the EXACT port cause with zero ambiguity. Escalation (carried, Fable): is "advertise on connection-open" ever correct before the disk walk? (join_h_cm_accepted deliberately adopts-without-advertising to preserve lookup→walk→advertise order; the seq7 CDT_OPEN empty-cell is BY DESIGN). SCS discards on transient port refusal (design §3.2.5 = SCS never retries) → fixed at SYSAP layer (re-offer). NEXT: build+re-fire, read the CNXTRACE.

### E70. ⚠ NEXT (dispatched, o5) — WHY does SCS refuse the advertise send (scs-refused rc=-1) on the adopted CM connection? The E69 trace proves the promotion FSM drives correctly (builds+attempts MODEL/PARAMS/CONFIG) but the SCS send op returns -1. Emit-gate ruled OUT no-csb/no-open-vc/codec-refused, so: a VC exists + the frame is built, but scs send fails. STRONG hypotheses: (1) the cm-accept connection isn't OPEN/sendable when OVMX advertises — note seq7 CDT_OPEN was DROPPED (EMPTY-CELL in DIR_ROUND), so the FSM advertises (driven by its own DIR_RESULT timer) on a connection whose open it never processed → sends before the accept handshake completes / before the CDT is in a sendable state; (2) no SEND CREDITS on the CM connection (E62: CNXMAN_JOIN_DIR_CREDITS/MSCP_CREDITS/CM_CREDITS are bare constants w/ no ledger; the member may grant 0, or OVMX's per-CDT credit accounting refuses). DIAGNOSE: trace the SCS send path (`scs_send`/the join `ops.send`→SCS) — what makes it return -1 on handle ...000b (connection state? credit? wrong CDT?). Consider whether the ADVERTISE should be TRIGGERED BY the connection-open event (add a CDT_OPEN/CM-open handler that transitions to ADVERTISE) instead of the join's own timer — i.e. advertise only once the CM connection is confirmed OPEN & sendable. May need to instrument the SCS send-refuse reason (extend the ring's EMIT aux or add an SCS diag) if not determinable from code. Fix faithfully (INV-6). Commit fc-e70; integrator orchestrates. NOTE: also 2 members each open a CM connection (...000b, ...000e) — ties to FC-P3.3 per-peer, but the FIRST connection must become sendable regardless.

### E70 RE-FIRE (vaxlab-2, TAG e70refire-1788506505) — CNXTRACE shows a DIFFERENT path than E69 (run-to-run variance), and exposes a regression/surfacing + the terminal-FAILED fragility as the STRUCTURAL blocker. join state=FAILED failure="**connect refused locally**" ignored_events=4. Trace: IDLE→DIR_ROUND→(DIR_RESULT)→MSCP_CONNECT; **seq4 ARRIVAL MSCP_CONNECT cdt-closed rc=3 aux=0xa51c0006 → seq5 DISPATCH MSCP_CONNECT→FAILED** (died at the MSCP step, BEFORE any ADVERTISE/EMIT — so nothing for E70's re-offer/named-cause to act on this run). seq6-13 FAILED; a genuine later cm-accept (aux=0xa51c000a, t=16399) arrives + is EMPTY-CELL-dropped (=ignored_events). Zero OVMX cat=0x01 on wire. CN=2, CSB State-09-wait votes 0/0. **KEY: in E69 the SAME rc=3 MSCP cdt-close went MSCP_CONNECT→ADMIT (E68 non-fatal working); here it goes →FAILED "connect refused locally" — E70's rc-unflattening (cnxman_fsm_rc now returns real status) CHANGED the MSCP-close classification** (regressed E68's non-fatal path OR surfaced a real local-refusal that the 0/-1 flattening had hidden — meaning E69 may have proceeded past MSCP on a FALSE success). → E71. **RECURRING STRUCTURAL PATTERN (3rd time): the join FAILS TERMINALLY on an early/transient connect event, then the REAL membership cm-accept arrives too late and is dropped. `FAILED` is terminal, nothing restarts (cnxman_join_drive refuses non-IDLE). E68+E70 both flagged this for the architect.**

### E71 — ✅ FIXED (fc-e71 @ 62968b3c, integrated tip 62968b3c, 60/60 + unit 109/109 + full 236/236 + gate OK + NetBSD ELF32 ALL PROOFS PASSED + Linux clean). **Q1: SURFACED, not a regression — the E69↔E70 divergence is a RACE.** cnxman_fsm_rc's 0/nonzero classification was UNCHANGED by E70 (only the nonzero VALUE changed). E69: cm-accept 0x0a7c000b ADOPTED in DIR_ROUND (t+16.024) BEFORE the MSCP close (t+16.458) → join_open_cm saw cm_open set → ADMIT, no outbound connect ever attempted. E70: no adoption, join_open_cm issued scs_connect (outbound VMS$VAXcluster) → refused locally → FAILED. So E70 exercised a real outbound-connect-refused path E69 never reached; flattening never hid it. Fix: all 3 MSCP$DISK outcomes (peer-reject/loss/local-refuse) now NON-FATAL; VMS$VAXcluster connect refusal also non-fatal. **Q2: JOIN RESILIENCE grounded in the book's OWN CSB reconnect states** (p.7-23/24 the 10 CSB states WAIT/REACCEPT already in vms_cnxman_csb.c; p.7-30 once-a-second; §7.10 timer). **REACCEPT is exactly what both live members offered and what [FAILED] couldn't take.** Fix: lost/un-transmittable VMS$VAXcluster conn → `[VC CONNECT]` retry state (reason named PATHLOST/CONNECT) NOT [FAILED]; **Con.ID DROPPED on loss** (else re-offers rejected as cm_already_held); the [VC CONNECT] beat ADOPTS csb->cdt_conid before opening a 2nd, ends when the ladder gave up, else re-issues; honest end = `FAIL_TIMEOUT` "reconnect interval expired" → released to IDLE so the sweep starts a fresh join; [FAILED] now ONLY a verdict (peer REJECT/no-CM/codec). no-target start deferred in IDLE; glue drives the join EVERY beat. **INV-6 proven: 30 refused beats → no CSID, no member flag, zero frames, handed_off()==0.** Adjacent: join_target_csb re-verifies the slot still holds the selected SYSTEM (p.7-25 rebuilds CSBs); re-offer gate moved lifetime-*_sent → per-connection mask (post-reconnect the counters wrongly said "already advertised"). New obs: CNXMAN_DIAG_R_CONNECT_REFUSED (SS$_ status + dest). R2 sim leg: full reference drive reaches barrier hand-off THROUGH a transient. → build+re-fire (strongest structural candidate — join now survives transients + adopts the members' re-offered CM connection).

### E72. FINDING (E71, not fixed — self-healing, own item): the member's FIRST membership offer is REFUSED for want of a CSB — both runs open `ARRIVAL IDLE detail=refused-no-csb aux=0x401` (~2s before the discovery sweep allocates the block); `cnxman_join_connect_req()` refuses an inbound VMS$VAXcluster connect from a system the CLUB has no block for. Self-healing (member retries; E71 resilience now survives to take the retry) but costs the first offer. p.7-23 NEW state = "a newly discovered remote Connection Manager" — an inbound CONNECT *is* a discovery, so arguably should allocate/NEW the CSB. Worth an item. Also carried: the local-connect-refuse REASON is still unnamed (E71's record gives the SS$_ status; a full `scs_connect_refusal()` readback mirroring E70's send-refusal would name the layer — next re-fire prints `connect-refused rc=<SS$_>`).

### E71. ⚠ NEXT (dispatched, o5) — reconcile E69-vs-E70 + JOIN RESILIENCE to transient connect failures. TWO questions: (1) **why does the MSCP `cdt-closed rc=3` now classify as "connect refused locally"→FAILED** (E70's cnxman_fsm_rc real-status change) when E68 made it non-fatal (→ADMIT in E69)? Is "connect refused locally" a REAL local bug (OVMX's own SCS declining OVMX's MSCP$DISK connect — MSCP SYSAP not registered? resource/state?) that flattening hid, or an E70 regression of E68's join_disk_client_gone path? Reconcile + fix so an MSCP connect outcome (peer-reject OR local-refuse OR lost) is NON-FATAL to the join per E68's grounded intent (golden joiner itself rejects MSCP mid-join). (2) **JOIN RESILIENCE (the recurring structural blocker):** the join must NOT give up terminally on a transient connect event and drop the genuine membership cm-accept that arrives ~1-2s later. Ground the FAITHFUL VMS behavior: how does real CNXMAN keep a join alive through transient SCS connect failures until membership commits or a real timeout — the recnx FSM (vms_cnxman_recnx_fsm.c exists), retry, the §7.10 reconnect timer? Implement a faithful join-resilience/restart so a transient doesn't wedge it, WITHOUT fabricating a "looks-joined" state (INV-6) — a join that retries until it genuinely completes or honestly times out, matching VMS. **If a true architecture fork appears that you can't ground from VAXcluster Internals + the golden joiner + the recnx FSM, ESCALATE to Fable** (this is the join-supervisor design E68/E70 flagged). Use both CNXTRACEs (E69 reached ADVERTISE→scs-refused; E70 died at MSCP). Commit fc-e71; integrator orchestrates.

### E71 RE-FIRE (vaxlab-2, TAG e71refire-1788511419) — ✅ RESILIENCE WORKS + CSB ADVANCED; new small wall = no VC_CONNECT→ADVERTISE transition. join state=VC_CONNECT (NOT FAILED — first run that doesn't wedge), failure="path lost", ignored_events=2 (was 4-5). Trace: DIR_ROUND→MSCP_CONNECT; seq3 MSCP cdt-closed rc=3 → **seq4 MSCP_CONNECT→VC_CONNECT (the E71 retry path — survives!)**; seq5 refused-no-csb (E72 first-offer); seq7/10 more cdt-closed rc=5 handled in VC_CONNECT (stays alive); **seq12-15 the members' re-offer: detail=accepted, cm-accept aux=0x5a9c000a, CM_ACCEPTED fired, CDT_OPEN fired — GENUINELY PROCESSED (not EMPTY-CELL)**; then seq17 TIMER_JOIN rep=1471 for the whole rest of the run, ZERO further transitions. **So the join adopts the re-offered CM connection AND processes its CDT_OPEN, but has NO transition VC_CONNECT→ADVERTISE — it never emits the promotion burst** (zero EMIT records; wire cross-check zero cat=0x01). **VAX1 OVMXJ1 CSB now `State: 05 connect`** (was 09 wait every prior run — REAL movement), votes still 0/0. CN=2. → E72.

### E72 — ✅ FIXED (fc-e72 @ 2b8cb0bb, integrated tip 2b8cb0bb, 60/60 + unit 109/109 + full 236/236 + gate OK + NetBSD ELF32 ALL PROOFS PASSED + Linux clean). Root cause: the join REFUSED the member's genuinely-OPEN connection because it still held its OWN connect that never opened (`cm_already_held++`) → the subsequent CDT_OPEN named an unheld Con.ID → ignored (=the 2 ignored_events). FIX: (1) `join_h_cm_accepted` yields ONLY on a TRUE simultaneous open (cm_conid!=0 && cm_open); otherwise ADOPTS (counts cm_superseded) — grounded in the executive's OWN ladder `csb_table[WAIT/RECONNECT][CONNECT_RCVD]→h_reaccept` ("a join refusing what its own CSB ladder accepts is contradictory"); the CDT_OPEN then drives the transition on a confirmed-open (sendable) connection. (2) `join_cm_sync_with_csb()` reads BOTH CSB facts every beat (which Con.ID p.7-23 + whether OPEN p.7-24) and advertises on the OPEN even when the Con.ID didn't move (fixes the 1471 silent beats). Ordering (E70 esc-1 SETTLED): VC_CONNECT reachable only via join_open_cm←join_lookups_complete, so lookup+walk done by construction. E72-first-offer FOLDED IN (csb_ensure before acceptance policy; p.7-23 NEW=inbound-CONNECT-is-discovery). INV-6 traced: VOTES←cl->params.votes (SYSGEN), MODEL←cfg, envelope←real CSB dialogue counters, LOCKDIRWT still not-advertised+counted; "reaching ADMIT still isn't membership — only a real op-0x06 coordinator CSID promotes" (honest). Host test drives VC_CONNECT→adopt-supersede→CDT_OPEN→ADVERTISE (send_failures==0)→op-0x06→MEMBER w/ wire-learned generation. → build+re-fire (should finally put the burst on the wire + let the VAX fill the CSB).
**⚠ CARRIED:** (1) `cm_other_member` — if the accept comes from a member the join is NOT driving through, neither fix fires (retargeting mid-join = p.7-37/38 selection, protocol/ECO unpinned — NOT invented). Mitigation shipped: emits `%CNXMAN, a cluster member opened a VMS$VAXcluster connection ... that is not the member this join is driving through` → IF that line appears in the re-fire console, route to FABLE (join-target-selection). (2) `csb->cdt_conid` never cleared on close (h_conn_lost/csb_give_up leave it) → cnxman_csb_send (vms_cnxman.c:366) can hand a dead Con.ID to SCS → own item.

### E72. ⚠ NEXT (dispatched, o5) — wire the VC_CONNECT adoption to ADVERTISE + drive the promotion burst on the now-OPEN connection. The E71 trace proves the join now survives + adopts the members' re-offered VMS$VAXcluster connection and processes CDT_OPEN (seq15 fired) — but sits in VC_CONNECT forever with no rule to advance to ADVERTISE. FIX: in VC_CONNECT, on the adopted connection reaching OPEN (CDT_OPEN / CM_ACCEPTED once open), transition to ADVERTISE and emit MODEL/PARAMS/CONFIG. **This also resolves E70's escalation-1 tension** ("is advertise-on-connection-open correct before the disk walk?") — in VC_CONNECT the FSM has ALREADY passed DIR_ROUND+MSCP_CONNECT (the lookup→walk is DONE), so advertise-on-open is correctly ordered here (unlike DIR_ROUND). **And it likely fixes the E69 scs-refused**: E69 advertised driven by the join's own timer on a possibly-not-yet-open connection; advertising triggered BY CDT_OPEN means the connection is confirmed OPEN & sendable. Watch for: does the EMIT now show detail=SENT (burst reaches the wire)? does the CSB fill with votes → MEMBER? Ground the advertise-on-open ordering (VAXcluster Internals connection-manager join). Also fold in E72-first-offer if cheap (allocate/NEW a CSB on an inbound VMS$VAXcluster CONNECT from an unknown system — p.7-23 NEW = "a newly discovered remote Connection Manager"; self-healing today but costs the first offer). Commit fc-e72; integrator orchestrates.

### E72 RE-FIRE (vaxlab-2, TAG e72refire-1788515862) — ✅✅ PROMOTION BURST ON THE WIRE (first ever). CNXTRACE: seq16-18 **EMIT detail=SENT rc=0** MODEL(0x14)/PARAMS(0x01)/CONFIG(0x02) — NO scs-refused; state VC_CONNECT→ADMIT (seq19); **join state=ADMIT failure=none ignored_events=1** (first failure=none, lowest ignored). Wire: all 3 cat=0x01 frames OVMX→VAX2 at t=18.684. cm_other_member message NOT present (clean). BUT CN=2, two precise reasons: **(A) the burst went to VAX2, but CN is polled from VAX1 — VAX1's connection arrived in IDLE (seq1-3, BEFORE START) and its CDT_OPEN hit EMPTY-CELL (IDLE has no handler) → VAX1 never got the burst → VAX1 CSB unchanged 09 wait, votes 0/0.** (B) **After ADMIT the FSM PARKS** (TIMER_JOIN rep=1472 whole rest of run) — sends the burst but never drives op-06 MEMBERSHIP → barrier → commit, so even VAX2's exchange doesn't complete to MEMBER. So promotion now STARTS on the wire but doesn't COMPLETE. → E73. pcap/node-log on tank (join-e72refire-1788515862).

### E73 — ✅ LANDED (fc-e73 @ bd74c8c4, integrated tip bd74c8c4, 60/60 + full 236/236 + unit 109/109 + gate OK + NetBSD ELF32 ALL PROOFS PASSED + Linux clean; test_cnxman_join 388 checks +48). **Post-ADMIT park ROOT = the BEDS-vs-RUNTIME gap AGAIN:** VAX2 DID answer OVMX's burst with op-03 COMMIT 0.7ms later, but OVMX couldn't PARSE it — FC-P3.15 converted the CM codec SEND half + left the RECEIVE half calling `vms_frame_classify()` (reads Ethertype@abs12, which a SYSAP body `frame+72`/132B design§3.2.4 doesn't have) → E_NOTSCA on EVERY real inbound CM msg (VAX1 op-01, VAX2 op-01, VAX2 op-03 all lost). Host tests missed it — fed composed 204B frames the executive never uses. GOLDEN completion decoded (vax3-2to3): joiner sends identity op-14/01 to BOTH members; CONFIG op-02 to the COORDINATOR ONLY (VAX2); coord relays op-12 to VAX1; VAX2→J op-03 COMMIT→J echoes 0x81/03; op-05×4 rebuilds→0x81/05; op-06×260 MEMBERSHIP→J cat-04 ack; op-09 bmap-M=3→0x81/09; op-0a GO; 12× star barrier 0b/0x81-0b/0c; **op-0c #12 = COMMIT POINT**. FIX: (1) body-level RECEIVE (vms_cluster_codec_cm parse→cm_check_body; join/barrier/coord _rx_frame→_rx_body); (2) CSB-ADDRESSED origination (barrier couldn't send op-0b — no CSID learned; added ops.send_csb, coordinator_csb as address per p.7-23); (3) PER-PEER identity exchange (cnxman_join_advertise_peers on 1Hz beat over every OPEN CSB, idempotent, covers VAX1's IDLE-arriving conn; PARAMS files in the SENDER's CSB); fixed a duplicate send-msg# at join→barrier handover (all via cnxman_envelope_originate). INV-6 traced: VOTES←SYSGEN, envelope←dest CSB dialogue counters, dest←csb->cdt_conid, barrier epoch/class/bitmap←coordinator's own frame, CSID←generation off a REAL op-06 | own SCSSYSTEMID. Tests: MEMBER only on a real op-06 CSID (no-CSID → barrier completes, local_csid_valid==0, state≠MEMBER); per-peer beat asserts no membership movement; send-msg# strictly monotonic. → build+re-fire (the CN=3 milestone candidate).
**⚠ CARRIED ESCALATIONS:** (1) **FABLE — COORDINATOR IDENTIFICATION (the flagged fork).** §4(o): a non-coordinator SILENTLY DISCARDS op-0x02; "how a joiner identifies the coordinator is NOT grounded." `join_select_target()` uses the book's residual rule (CSB nearest CLUB queue tail) = discovery-order dependent → it picked VAX2 (the real coordinator) on E72 by LUCK. If a re-fire picks WRONG, the CONFIG→commit drive stalls (per-peer identity still reaches all, but op-02 to a non-coordinator is discarded). Proposed answer (measured-safe, NOT implemented — policy): treat silence on op-0x02 as "not coordinator" + re-issue to next member (d94-e15 byte-verified all 3 members getting identical op-0x02 within 400ms, exactly ONE transition, no abort → fan-out measured safe; who-answers-op03/09 is wire-observable). → if the re-fire stalls on wrong-coordinator, ROUTE TO FABLE with this exact question. (2) **MSCP receive twin** (vms_mscp_cl_fsm.c has the identical classify-instead-of-body defect; can't block a join per E68 but hangs the MSCP walk if MSCP$DISK connects) → rd item.

### E73. ⚠ THE LAST STRETCH (dispatched, o5 w/ Fable-escalation authority) — the MEMBERSHIP-COMMIT choreography: per-peer coverage + post-ADMIT drive to MEMBER. TWO parts: **(A) PER-PEER (FC-P3.3, the repeatedly-flagged architecture piece):** the join drives whichever member wins the race (VAX2 this run) and DROPS member connections that arrive in a state with no handler (VAX1's arrived in IDLE→CDT_OPEN EMPTY-CELL). CN is read from VAX1, and VAX1 is likely the coordinator — so OVMX MUST drive the promotion with VAX1/the coordinator (and faithfully with all existing members). Handle inbound member CM connections arriving in ANY join state. **(B) POST-ADMIT COMPLETION:** after the MODEL/PARAMS/CONFIG burst + ADMIT, OVMX parks — it must process the member's responses (golden: op-03/op-05 answered 0x81) and drive **op-06 MEMBERSHIP burst → cat-04 BARRIER → the count-commit** that transitions NEW→MEMBER and makes VAX1 fill the CSB (votes) + F$GETSYI CLUSTER_NODES=3. GROUND in the golden **vax3-2to3** full completion (decode what the joiner sends/receives AFTER its own MODEL/PARAMS/CONFIG through to MEMBER) + VAXcluster Internals (connection-manager membership/config/vote + the CSB commit) + the PRESERVED strawman wire-spec [[cluster-barrier-5of12-empty-payload]] (barrier op-0x0b) + [[cluster-promotion-gap]] (count-commit cat-0x01 op-0x03). **INV-6 ABSOLUTE — this is the final temptation to fabricate: no synthetic CN=3, no faked barrier/vote/CSB; a member is counted ONLY when the real VAX commits it. Only a real op-0x06 coordinator CSID promotes (E72 agent's own words).** If the join-target-selection (which member drives / coordinator identification) or the barrier-commit state model is a genuine design fork you can't pin from golden+book+preserved-spec, ESCALATE to Fable with the specific question (this is the join-supervisor/per-peer design flagged since E67). CNXTRACE + the diag ring are your runtime eyes; extend them if the post-ADMIT drive needs new visibility. Commit fc-e73; integrator orchestrates.

### E73 RE-FIRE (vaxlab-2, TAG e73refire-1788521630) — ✅ receive-fix WORKS + VAX RECORDS OVMX'S VOTES (first ever); parks at ADMIT, no commit → CN=2. CNXTRACE: RX_CONFIG now FIRES (seq9,22 — the E73 body-level receive parse works); OVMX sent MODEL+PARAMS to VAX2 (aux ...0007, conn closed before CONFIG) then FULL MODEL+PARAMS+CONFIG to VAX1 (aux ...000a). Wire: OVMX→VAX1 all 3 cat=01 (t=18.074); OVMX→VAX2 only 2 (t=17.798, closed before op-02). **NO op-03/05/06/09/0a/0b/0c frame anywhere — the commit sequence never comes back from EITHER VAX.** state=ADMIT failure=none. **⭐ NEW at the VAX: VAX1 OVMXJ1 CSB now `Flags: status_rcvd`, `Votes=1`, `Last seq num rcvd=0003`** (matches the 3-frame burst) — a real VAX GENUINELY RECEIVED+PARSED OVMX's votes/config for the first time. But `State: 09 wait`, `CSID 00000000` — no membership block assigned. → E74. **The blocker = COORDINATOR IDENTIFICATION (the ungrounded fork, §4(o)):** the commit (op-03 COMMIT etc.) is driven by the COORDINATOR; the full CONFIG went to VAX1 while VAX2 (E72's identified coordinator) got only a partial burst → no commit. join_select_target's residual rule is discovery-order-dependent (picked VAX2 on E72, VAX1 got the full CONFIG on E73). Which node is the coordinator, and how a faithful joiner identifies it, is NOT grounded.

### E74 — ✅ FABLE RULING (coordinator-ID + E73 non-commit). **THERE IS NO COORDINATOR TO IDENTIFY — the RECIPIENT of op-02 BECOMES VAX_A/coordinator** (VAXcluster Principles p.7-2 "effectively random"; p.7-37/38 JOIN CLUSTER: the JOINER picks the member by highest protocol-level→highest ECO→CSB-nearest-CLUB-queue-tail; that member runs quorum + requests coordinator permission via p.7-32 coordinator lock). **§4(o) premise REFUTED.** Wire-proven: golden J→VAX2 op-02 → VAX2→VAX1 op-12 coordinator-lock relay → op-03; E72 OVMX→VAX2 op-02 → VAX2 console "proposed addition of node OVMXJ1"; E73 OVMX→VAX1 op-02 → **VAX1 console "received VAXcluster membership request from node OVMXJ1" but NEVER proposed**. **⭐⭐ THE E73 NON-COMMIT IS LAB RESIDUE, NOT CODE:** vaxlab-2 pod running since 2026-09-02 (startTime), VAX1 incarnation identical E72/E73, ~15 OVMXJ1 residue incarnations across ALL E55-E73 re-fires (the standing lab-reset protocol was NEVER run). VAX1 logged the spec's documented coordinator-side WEDGE (§4(O.24/25/27): "received request but NEVER proposed... the wedge outlives the hammer"); VAX2 never saw it + proposed (E72). VAX2's connection did NOT close (E73 note misread — seq13 cdt-closed was OVMX's OWN unanswered outbound CONNECT_REQ to VAX1). **STEP 0 (executing): reset pod + re-fire the UNCHANGED e73-build (dc3a5948 code == a11975f7 == e73-build; docs-only atop) on a FRESH CN_2, grade on BOTH consoles. Fable prediction: VAX1 proposes, op-03 (now parseable) drives to CN=3.** IF still declines on a virgin pair → QD-watcher path (VAX1 is the quorum-disk watcher) is the residual suspect. FAITHFUL SPEC (implement AFTER the reset experiment, regardless of outcome — OVMX identity records are hollow): (1) accept passive CONNECT_RESPONSE with cfg.conndata (E31 quad) not NULL — currently 16 spaces → VAX records "Eco/Version 32/32"; (2) join_select_target rank by (conndata byte1,byte0) then queue-tail; (3) count-gate op-02 until #CSBs{OPEN,status_rcvd}==member-count (op-01 body[18:20]); (4) consume member's op-01 BEFORE the burst (op-02 ack#); (5) silence→re-issue to next ranked member after ~6 beats, never fan-out (fan-out UNFAITHFUL — real joiners send op-02 to exactly ONE member); (6) honest op-14 MODEL + op-01 LOCKDIRWT[20:22]/EXPECTED_VOTES+QDSKVOTES[24:28]/incarnation[64:72]/protocol-consts[72:80]/SWVers[88:96] fills; (7) MEMBER only on real op-06 CSID. Flags: Eco-vs-Version byte order + EXPECTED_VOTES/QDSKVOTES offsets unpinned.

### E74. ⚠ ROUTED TO FABLE — COORDINATOR IDENTIFICATION + the commit-drive (the design fork flagged since E67, now the CN=3 blocker). Everything below the commit works: connection, resilience, per-peer identity, the full CONFIG burst SENT, and VAX1 now records Votes=1/status_rcvd/seq-3. What's missing: the coordinator must receive the full CONFIG and drive op-03 COMMIT → op-05 → op-06 → barrier → the count-commit. FABLE QUESTIONS: (1) **how does a faithful VMS joiner identify the coordinator** to send op-02 CONFIG to? (golden vax3-2to3 sent CONFIG to the coordinator VAX2 ONLY, which relayed op-12 to VAX1 — so it KNEW the coordinator; §4(o): a non-coordinator silently discards op-02 + "how the joiner identifies the coordinator is NOT grounded"). Is it a CLUB/CSB coordinator field, a wire role byte, who-relays-op-12, who-answers-op-03? (2) is re-issue-on-silence (op-02→member, no op-03 within T → next member) faithful, or fan-out to all members (d94-e15: all 3 members got identical op-02 within 400ms, exactly ONE transition, no abort → MEASURED SAFE — but golden sent coordinator-only, so fan-out may be unfaithful)? (3) why does the coordinator not drive op-03 here — wrong member got the full CONFIG, or the partial burst to VAX2 (conn closed before op-02), or does the coordinator need something more post-CONFIG? Give Fable both traces (E72 burst→VAX2 parked; E73 full→VAX1 records Votes=1 parked), the golden, §4(d)/(j)/(o), the CSB states, [[cluster-promotion-gap]]+[[cluster-barrier-5of12-empty-payload]]. Then implement per ruling. INV-6: no synthetic membership.

### E74 RESET RE-FIRE (vaxlab-2 FRESH pod, TAG e74reset-1788527226) — residue hypothesis REFUTED, but BIGGEST LEAP + the real wall PINNED. Reset clean (verified CN_2 both VAX MEMBER, ZERO OVMXJ1 residue before firing). Unchanged e73-build. Still CN=2 — so NOT lab residue, it's a code gap. BUT on the fresh pair **VAX2 was the coordinator** (OVMX drove the full CONFIG to VAX2 — the member that answered): **wire t=18.4072 OVMX→VAX2 op-02 CONFIG → t=18.4078 VAX2→OVMX op-03 COMMIT — FIRST COMMIT EVER ON THE WIRE.** CNXTRACE: seq25 **RX_COMMIT fired (first ever — OVMX parses the real op-03)**; VAX1 CSB reached `State: open Votes:1` for many polls (better than prior 09-wait) before degrading. Coordinator-ID CONFIRMED WORKING (recipient-becomes-coordinator; OVMX drove VAX2 which answered, not a silent member). **THE REAL WALL (E70 observability named it exactly): seq22-24 `send-refused rc=-10` → `port-nocredit rc=-2 refused_credit=2` → `scs-refused rc=44 cat=0x81 op=0x03` — OVMX CANNOT SEND the COMMIT-ECHO (0x81/03) back for lack of SEND CREDIT.** It spent its send credit on the MODEL/PARAMS/CONFIG burst and never replenished → receives the COMMIT but can't ack it → parks. Distinct send-credit/flow-control gap (E62 SYSAP-credits-no-ledger territory), NOT the START-body advertised credit (E60). → E75.

### E75 — ✅ FIXED (fc-e75 @ ca1e315b, integrated tip ca1e315b, 60/60 + unit 109/109 + gate OK + NetBSD amd64 PASS + elf32-vax ALL PROOFS PASSED). Root cause: TWO send-credit accounts; the PORT one drained. SCS-CDT `credit_send` HEALTHY (grant = VAX2's real ACCEPT_REQ credit = 10, executive-backed; CNXMAN_JOIN_*_CREDITS are DEAD `(void)credits`). PE-VC `vc->send_credit` (max = VAX2's abs-95 grant = 10) replenished ONLY by standalone 0x48 credit-returns — but **every PIGGYBACKED ack called vc_release_acked() which freed the ring entry AND destroyed the credit** → window drained monotonically, never refilled. Wire-measured: **94-99.8% of real acks are piggybacked** (only 0.2-5.6% via 0x48), so a 0x48-only port stalls (tool: scs_credit_window_measure.py; grant is a hard ceiling, outstanding never exceeds 10). FIX (vms_pe_fsm.c): `vc_release_acked()` returns exactly the count of entries the peer's real cumulative ack released — credit rides back on WHATEVER class carries the ack; `h_vc_rx_credit` drops its own +1 (0x48's worth IS the released entry); `vc_credit_grant`=grant−unacked (re-STACK can't refund held). Invariant `send_credit + unacked == send_credit_max`, every value from real state (peer abs-95; count released by a real ack). INV-6 clean. **E70's re-offer now SUCCEEDS: VAX2's op-03 COMMIT carries the recv_ack covering the burst → h_vc_rx_seqmsg releases+credits BEFORE delivery → the re-tried COMMIT-echo has credit.** Cites pp.2-43/44/45, §4(h)(3)/(4)/(4a)/(g). New test verified RED on a 0x48-only regression ("the COMMIT-echo E75 could not send now goes"). **LIKELY THE LAST WALL — build+reset+re-fire the CN=3 milestone test.** Flags: (1) tests/netbsd/crosscompile.sh omits vms_pe_fsm.c (pre-existing test-gap; elf32-vax gate covers it) → own item; (2) 0x7b/0xb3 SCS block frames may fail the {0x4b,0x5b} filter → vc_rx_parse_failed (not on CM join path); (3) CNXMAN_MSCP_CREDITS=4 hardcoded vs real VMS SYSGEN MSCP_CREDITS=8 — a grant we extend (not INV-6 break) but should come from sysgen → E62-adjacent item.

### E75. ⚠ NEXT (dispatched, o5) — SEND-CREDIT flow-control on the CM connection: OVMX exhausts its send credit on the burst and can't send the op-03 COMMIT-echo. The E70 ring named it: `port-nocredit refused_credit=2` on the cat=0x81/op=0x03 send after OVMX received VAX2's op-03 COMMIT. OVMX's SEND credit = what the PEER (VAX2) advertised/grants to OVMX, replenished as OVMX's sequenced frames are acked. OVMX sent 3 sequenced frames (MODEL/PARAMS/CONFIG) consuming credit; the COMMIT-echo needs 2 more it doesn't have. DIAGNOSE: OVMX's send-credit accounting on the CM CDT — the initial grant (VAX2's advertised credit at connect/accept — is it read correctly? the E62 CM_CREDITS bare constant?), and whether OVMX REPLENISHES its send credit when VAX2 acks/credit-returns (VAX2's op-03 COMMIT itself carries an ack + likely a credit-return OVMX isn't crediting). Note E70 added a re-offer for refused sends — verify it re-tries the echo AND that the credit ever replenishes (if not, re-offer loops forever). FIX faithfully (executive-backed per E60's ledger pattern — the send-credit tracked from the peer's real grant + replenished on real acks, never a stuffed value). INV-6. This is likely the LAST wall before CN=3 — OVMX receives the COMMIT, just can't ack it. Ground vs VAXcluster Internals p.2-43/2-45 (send credits / credit-return) + the golden joiner's credit flow. Commit fc-e75; integrator orchestrates.

### E75 RE-FIRE (fresh pod, TAG e75refire-1788532514) — ⚠ WORSE than E74: no COMMIT, connection CHURN, regressed to IDLE. Reset clean (verified CN_2). e75-build. join state=**IDLE** failure="no member to join through" ignored_events=6. CNXTRACE (102 records): FSM cycles through **8+ connection attempts** (aux ...05/08/0a/0b/20/21/23/38), each hitting `scs-refused rc=2692` OR **`cdt-not-sendable rc=2 cdtstate=2 credit_send=0`** (cdtstate=2 = connection NOT open); a few bursts DO get SENT (full 3-op) but **each connection cdt-closes almost immediately after**; regresses VC_CONNECT→IDLE (seq47), retries, →IDLE again (seq91), then parks IDLE firing START (rep=1329, rest of run), NO further cm-accept. **NO RX_COMMIT anywhere** (E74 got one). Wire: OVMX sent full burst to BOTH VAXes; both answered op-01 PARAMS only, **NEITHER sent op-03** this run; zero cat=0x81/04/0b. Both consoles: no request/propose for OVMXJ1 (VAX2 only teardown msgs at end = a VC formed+lost). CN=2, OVMXJ1-status=absent throughout (worse than E74's NEW/open). **The ONLY code delta since the clean E74 (reached COMMIT) is E75 → E75 likely REGRESSED the VC ack/credit path (connection instability, not the clean credit-drain it targeted)** — OR real-cluster run-variance (the churn = OVMX thrashing connections). → E76. pcap/node-log on tank (join-e75refire-1788532514, 9383 frames; full CNXTRACE in the node log).

### E76 — ✅ DIAGNOSED (fc-e76 @ d8173219 tool integrated, 60/60). **E75 did NOT regress — it is EXONERATED** (its credit accounting proven correct on the suspect run's OWN wire: outstanding-at-transmit ceiling=3 vs grant 10, no uint8 wrap, no unacked underflow, 0 seq/ack anomalies over 10 circuit generations, 0 port-nocredit; every refusal was `SCS_ERR_NOTOPEN cdtstate=2`=CONNECT-SENT, the CDT connect-verb grant not the port window). **⚠⚠ THE REAL CAUSE: OVMX BUGCHECKS THE REAL VAXes (CNXMGRERR).** 14:35:40.237 OVMX→VAX1 op-14/01/02 → **1.2ms later VAX1 emits §4(O.30) departure marker 0xb1** → `**** Fatal BUG CHECK ... CNXMGRERR, Error detected by VAXcluster Connection Manager` → dump/HLT/reboot; 14:36:04.829 →VAX2 same 0.2ms later. Both crash → no cluster → no coordinator → no op-03. (E74 pcap: ZERO 0xb1, live VAX1↔VAX2 throughout.) **TRIGGER:** the two fatal bursts opened a BRAND-NEW dialogue (new Con.ID pair) at send_msg 8 and 13, **acking a peer send_msg on a connection the peer NEVER sent on.** `join_emit_cm()` (vms_cnxman_join_fsm.c:407) increments the per-CSB dialogue counter BEFORE the send + BURNS it on refusal (exactly 5 refusals on VAX1's handle 2→8, 10 on VAX2's 2→13) and **NEVER RESTARTS it when the connection is torn down.** Golden: every fresh CDT opens at send_msg=1. Tool `tools/cluster/cm_dialogue_audit.py` (E74=0 findings, E75=8 at the departure timestamps). ⚠ This LIKELY explains much of the cross-run "connection closed/path lost/churn" — OVMX was CRASHING the reference VAXes. → E77 (top priority).

### E77 — ✅ FIXED (fc-e77 @ 611fc131, integrated tip 611fc131, 60/60 + unit 109/109 + gate OK + NetBSD ELF32 ALL PROOFS PASSED + Linux clean; neg-control reproduces E76 signature "got 10 want 1 / acks got 17 want 0"). Per-connection envelope: single-writer `cnxman_csb_bind_connection(csb,conid)` (only writer of csb->cdt_conid) resets the dialogue (cm_send_msg/cm_ack_msg→0, new field cm_dialogue_conid) whenever the Con.ID differs → next origination pre-increments to send_msg=1 and this node ACKS NOTHING until the peer sends on THAT connection. Tied to all 4 glue lifecycle sites (outbound connect, accepted, + BOTH reconnect paths cnxman_vc_closed/act_on_recnx_rec — where a burned number used to cross a teardown). ack# advanced only by cnxman_csb_dialogue_heard() resolving the CSB by the ARRIVING Con.ID (a frame on a dead conid → no CSB → honestly not acked). INV-6 skew-guard `join_emit_gate()`/CNXMAN_DIAG_G_SKEW refuses to stamp a mismatched dialogue. **Stops CNXMGRERR: fresh Con.ID envelope = send_msg=1 ack=0, what the VAX CM expects (vs 8/13 acking never-sent msgs).** Golden: vax3-2to3/formation-ci1 stations open every fresh dialogue at 1/0. ⚠ a bed that SKIPPED binding the Con.ID was part of why this was invisible (beds-vs-runtime again — beds now mirror the glue). Cites §4(j)/(O.30), p.7-23. Escalations: (1) skew window in ADMIT — if the executive rebinds a Con.ID while the join is in ADVERTISE/ADMIT it emits nothing until the old CDT close arrives (strictly better than the crash; if a lab run stalls with `dialogue-skew` in the ring → resync the join on the ADMIT beat); (2) cdt_conid not cleared on close (pre-existing; safe — Con.IDs never repeat within a boot). **→ build+reset+re-fire: E75(credit)+E77(stop-crash) together should let the join stay stable, reach the coordinator, COMMIT, ACK it, and drive op-05/06/barrier → MEMBER → CN=3.**

### E77. ⚠⚠ CRITICAL (dispatched, o5) — OVMX crashes real VAXes via a per-CSB (not per-connection) envelope send_msg counter → CNXMGRERR bugcheck. FIX: the SCS dialogue send-msg# + ack# must be PER-CONNECTION (per Con.ID/CDT), restarting at send_msg=1 on each NEW connection (golden-grounded: every fresh CDT opens at 1), and OVMX must NEVER ack a peer send_msg the peer has not sent ON THAT connection (the fatal §4(j) envelope violation the VAX's CM bugchecks on). `cnxman_envelope_originate/stamp` reads/advances the CSB's dialogue counter (cnxman_csb_dialogue_sent) — retie it to the connection lifecycle: reset the send/recv dialogue counters when a new CM Con.ID is adopted (CDT_OPEN / cm-accept for a new handle), keep join_emit_cm's burn-on-refusal WITHIN a connection but not across a teardown. Verify against the golden §4(j) envelope (`cm_dialogue_audit.py` must show 0 findings on a fixed OVMX trace) + VAXcluster Internals p.7-23 (CSB holds the connection's dialogue state) + spec §4(j)/(O.30). INV-6/Rule-8. Host test: adopting a new Con.ID after a refusal-burn resets send_msg to 1 and ack# to the peer's actual seq on the new connection (a regression to per-CSB continuation reds). This is likely a big unlock — stop crashing the VAXes and much of the churn resolves. Commit fc-e77; integrator orchestrates the reset+re-fire.

### E76. ⚠ NEXT (dispatched, o5, DIAGNOSE-first) — did E75 REGRESS the VC ack/credit path, or is this run-variance? The E75 piggyback-credit INSIGHT is sound (94-99.8% acks piggybacked — do NOT revert the grounding); the suspect is a SECONDARY bug in E75's `vc_release_acked()` released-entry count / the `h_vc_rx_credit` +1-drop / `vc_credit_grant`=grant−unacked / the h_vc_rx_seqmsg release-before-deliver — any of which, if the released-entry count or unacked is miscomputed, corrupts recv_ack/send_seq or underflows send_credit → OVMX mis-acks or over-sends → the VAX RESETS the VC → the cdt-close churn + IDLE regression seen here. DIAGNOSE: pull the FULL 102-record CNXTRACE (node log ovmx-node-e75refire-1788532514.log on the tank) + the pcap; per connection, does OVMX's recv_ack/send_seq stay consistent, or does a mis-count show (e.g. send_credit wrapping, unacked negative, an ack releasing the wrong entries)? Compare to E74's clean single-connection trace (reached ADMIT+RX_COMMIT). Confirm E75-regression vs variance from the wire (does the VAX send a reset/disconnect after OVMX's burst, and why). If E75 regressed → FIX the accounting (keep the piggyback-replenish, correct the entry count) with a host test that reproduces the RUNTIME churn (not the bed). If it's variance → say so with evidence + I re-fire. INV-6. Commit fc-e76; integrator orchestrates. ⚠ standing lab-reset before each milestone re-fire.

### E77 RE-FIRE (fresh pod, combined E75+E77, TAG e77refire-1788538657) — ✅✅ DEEPEST EVER: no crash, echoes sent, CSID ASSIGNED + SELECTED. **CRASH-CHECK CLEAN — E77 works** (zero CNXMGRERR/BUG CHECK/0xb1, no reboot, both VAXes survived the whole run). **E75 CONFIRMED** — CNXTRACE seq23 EMIT SENT cat=0x81 op=0x03 (COMMIT-echo), seq25/27 SENT cat=0x81 op=0x05 (LOCKRB-echoes) — NO port-nocredit/scs-refused, NO dialogue-skew. Wire byte-exact: VAX2→OVMX op-03 COMMIT (t=18.524) → OVMX→VAX2 op-03 ECHO (first ever) → VAX2→OVMX op-05 LOCKRB×2 → OVMX→VAX2 op-05 ECHO×2 (first ever). **⭐⭐ VAX1 OVMXJ1 CSB: `CSID 00010003` (GENUINE NONZERO, first ever — always 0 before) + `Flags: selected` (new) + status=open SUSTAINED whole run.** The coordinator SELECTED OVMXJ1 for membership + assigned a CSID. BUT `State: 09 wait long_break`, not MEMBER; SHOW CLUSTER BRK_NEW; CN=2. **STALLS right after the op-05 LOCKRB echoes — no cat-04/cat-0b barrier, no op-06 MEMBERSHIP ever** (golden had op-05×4→op-06×260→barrier; here op-05×2 then VAX2 stopped, VAX1→long_break=timed out waiting). → E78. pcap/node-log on tank (join-e77refire-1788538657).

### E78 — ✅ FIXED (fc-e78 @ a73c6f8b, integrated tip a73c6f8b, 61/61 (x2) + unit 110/110 + full 237/237 + gate OK + Linux clean + elf32-vax ALL PROOFS PASSED+negctl). **THE MIRROR OF E75:** E75 = OVMX's SEND credit (couldn't send); E78 = OVMX not RETURNING receive credit to the peer (so the PEER couldn't send). Root cause: only vms_scs_dir.c ever called scs_return_credit() — the VMS$VAXcluster + MSCP SYSAPs NEVER released their receive buffers → credit_held grew, credit_pending stayed 0 → **every frame OVMX sent carried credit=0 at abs-62.** OVMX's ACCEPT granted VAX2 credit 4; VAX2 spent exactly 4 sequenced msgs (PARAMS, COMMIT, op-05×2) and — never repaid (pp.2-43/44: receiver copies Pending Receive Credit into abs-62, sender ADDS to Send Credit) — **WENT MUTE**, couldn't send op-06 → transition timed out with the real CSID stranded in VAX1's CSB (long_break). So "only 2 op-05" = the credit ceiling truncating VAX2's stream, NOT a smaller rebuild; op-06 is coordinator-driven 0.1ms after the last echo PROVIDED it has send credit. Golden decode: op-03→81/03(credit2)→op-05×4(2 rebuild streams from VAX2's lock DB)→81/05→op-06×254→04/49,00,02(credit3)→op-09→GO→12×barrier→MEMBER. FIX (vms_cnxman.c): `cnxman_release_receive_buffer()` — 1 buffer/msg, FIRST in cnxman_vc_message()/cnxman_mscp_message() (before dispatch, matching golden f292 credit-2); VC grant `cnxman_vc_grant()`←SYSGEN CLUSTER_CREDITS (refuses vs guesses); `scs_fsm_return_credit()` refuses > credit_held (can't manufacture credit); mscp_srv.c same. New CNXMAN_DIAG_R_PEER_NOCREDIT ring record. **The membership/barrier/CSID ladder was ALREADY correct + UNCHANGED** (E73's op-06→wire-learned-CSID→81/09→GO→12×0b/0c→MEMBER, gated on real coordinator CSID by test_member_only_on_a_real_op06_csid). 10-grant labeled honestly (GROUNDED: reference VC opens at credit 10 both ways; INFERRED: 10==CLUSTER_CREDITS). elf32-vax gate CAUGHT an arch-asymmetric include miss (vms_cluster_sysgen.h reached vms_cnxman.c transitively Linux-only → explicit include). Caveat: 2 intermediate full-suite runs on a loaded shared host showed 1-failed (unpinned, not in touched layer, subsequent runs+my 2x cluster re-runs green — shared-host flake). **→ build+reset+re-fire: this should let VAX2 keep its send credit → drive op-06→barrier→commit → OVMXJ1 MEMBER → CN=3.** No synthetic membership/CSID/barrier. No Fable escalation (fork resolved to a grounded ledger defect).

### E78. ⚠ THE FINAL PHASE (dispatched, o5, Fable-escalation authorized) — the DLM-rebuild → op-06 → BARRIER → commit completion. The join is now SELECTED with a real CSID (00010003) — the coordinator wants it in. Gap: after VAX2 sends op-03 COMMIT + op-05 LOCKRB×2 and OVMX echoes them, the transition STALLS — VAX2 never sends op-06 MEMBERSHIP, VAX1's CSB goes long_break (timeout waiting for OVMX). DIAGNOSE from the golden vax3-2to3 (op-03→op-05×4→op-06×260 MEMBERSHIP→OVMX cat-04 ack→op-09 bmap→op-0a GO→12× barrier 0b/0c→op-0c COMMIT→MEMBER): (1) why only 2 op-05 here vs 4 golden — is the rebuild set/count OVMX-driven? (2) after the op-05 echoes does the joiner send something to progress to op-06 (a rebuild-complete signal / its own NL-mode rebuild contribution), or does the coordinator send op-06 unprompted + OVMX isn't triggering it? (3) WHY does VAX1 go long_break — what is it waiting for from OVMX post-op-05? Decode OVMX's park in ADMIT + the golden's post-op-05 frames. Ground vs the PRESERVED barrier spec [[cluster-barrier-5of12-empty-payload]] (barrier op-0x0b empty payload) + [[cluster-promotion-gap]] (count-commit) + docs/cluster-protocol-spec.md §4 + VAXcluster Internals (DLM rebuild + the Phase1/Phase2 barrier + the transition commit). Fix OVMX to complete the rebuild + drive/answer op-06 + the barrier → MEMBER. ⭐⭐ INV-6 ABSOLUTE (the final fabrication temptation): no synthetic MEMBER/barrier/op-06; the CSID must stay wire-learned (00010003 is VAX2's real assignment); MEMBER only when the real VAX commits. Escalate to Fable if the barrier/rebuild-completion state model is a genuine fork. Commit fc-e78; integrator orchestrates the reset+re-fire.

### E78 RE-FIRE (fresh pod, TAG e78refire2-1788549071) — ✅ credit-return WORKED (coordinator drove op-06, FSM reached MEMBER internally FIRST EVER) but ❌ OVMX FLOOD-CRASHED VAX2 (INVEXCEPTN, NEW crash ≠ E77 CNXMGRERR). (First attempt e78refire-1788547301 = infra flake: harness cap-guard aborted pre-qemu on a CAP_NET_RAW effective-set timing flake, no data, VAXes untouched — retried on the same clean pair.) E78 credit fix CONFIRMED: VAX2 kept send credit + sent the **op-06 MEMBERSHIP burst** (never reached before). BUT **OVMX answers the op-06 burst with ONE cat-04 op-00 per frame → 254 cat-04 frames to VAX2 in 31.6ms (~8000/sec) → VAX2 `**** Fatal BUG CHECK INVEXCEPTN, Exception while above ASTDEL or on interrupt stack, Current process=OPCOM, PC=8702687F`. VAX2 DOWN (no auto-reboot); VAX1 up.** Golden joiner answers the SAME op-06 ×254 burst with only **THREE** frames (04/49, 04/00, 04/02). CNXTRACE: state=MEMBER failure="path lost" ignored_events=140, ring OVERFLOWED (held256/recorded696/dropped440); ~240× CSID_LEARNED(EMPTY-CELL)→EMIT cat-04 op-00 SENT→RX_MEMBERSHIP loop; then RX_TR_OPEN/RX_TR_GO→cdt-closed rc=5→no-open-vc→RX_CLOSE; **NO barrier op-0b/0c ever.** Premature-MEMBER: the FSM hit MEMBER on the op-06/CSID stream, BEFORE the barrier/op-0c commit (wrong — golden reaches MEMBER only after the 12× barrier). Not a real CN=3 (no VAX-side admission; VAX2 crashed). E78 is CORRECT (unblocked op-06); the flood is a PRE-EXISTING op-06-response bug never exercised until now. → E79. ⚠ VAX2 DOWN — pod MUST be reset before any next re-fire; NO re-fires until the flood is fixed. pcap/node-log on tank (join-e78refire2-1788549071).

### E79 — ✅ FIXED (fc-e79 @ d3d5882a; + crash-safety gate merged; integrated tip 86f0d831, 63/63 + unit 112/112 + full 239/239 + gate OK + wire-safety-self-test PASS + elf32-vax ALL PROOFS PASSED). **The op-06 ack IS the credit return, coalesced at quantum 3** (independently converged with the crash-audit + operator correction): golden 254 op-06 → 84 cat-04, `am` advances by EXACTLY 3 each (never 1/2/0), credit@abs62 {3:85,1:1}. `SCS_CREDIT_COALESCE=3` (census-attached constant). FLOOD FIX: `join_h_membership()` consumes-only; carrier `cnxman_credit_carrier()` gated on `scs_fsm_credit_return_due()`=`credit_pending>=3`; cat-04 not op-8 (golden 0 op-8 on the 204B VC class; op-8 = backstop, spends no send credit, breaks mutual-starvation). `am` traceability: carrier envelope-stamped from CSB dialogue_heard, moved per inbound record BEFORE any FSM sees it (advance = records consumed, not computed). Found+fixed an ORDERING bug: `scs_fsm_return_credit` fired the p.2-44 flush INSIDE the SYSAP callback, beating the answer about to carry it → `delivery_depth` defers+re-tests at delivery end (teardown op-8 must NOT defer — caught by test_scs_directory). **MEMBER gate: ONLY on `cnxman_barrier.commits` in barrier_finish() (real op-0c #12)** via CNXMAN_EV_TRANSITION_DONE; blocked w/ no learned CSID (E73 assertion kept) or if the nodemap NAMED us + said no; nodemap SILENCE promotes + counts commits_unmapped (§4(p): bitmap width undetermined → silence-as-refusal would strand a real join; Phase-2 three-valued). Barrier drive (vms_cnxman_barrier_fsm.c) was ALREADY correct — never fired live because the flood killed VAX2 before op-09. The 04/49/04/00/04/02 "triple" = recycled-buffer artifacts (body[9] uninit), NOT a phase set. Tests: 255 op-06 → exactly 85 carriers, none below quantum, returned-credit==quantum×carriers (explains §4(h)(1c)'s "last credit never returned" = sub-quantum remainder); MEMBER absent after releases 1-11, present only on #12. Residual: ack-ordering vs coordinator pacing confirmable only live — read `commits_unmapped`/`commits_not_ours` next re-fire.
### CRASH-SAFETY GATE (fc-crash-safety-audit @ 652afed4, merged into 86f0d831) — operator invariant [[ovmx-never-crashes-a-peer]]. `tools/cluster/cm_wire_safety_audit.py` + `tools/ci/cluster_wire_safety_gate.sh` + `docs/cluster-crash-safety.md`: 9 crash-vector classes / 11 checks, thresholds MEASURED from 234,555 real CM frames, ZERO false positives over 19 reference captures; retro-audit flagged all 3 known crashes (CNXMGRERR/INVEXCEPTN/LOCKMGRERR) + found NEW vectors → rd vms-93f (N2 over-send 11-15 vs grant 10), vms-de1 (N3 CM config to Con.ID 0), vms-7c6 (N5 per-conn-vs-per-node counter scope: E77 premise corpus-contradicted — real VMS opens fresh Con.ID at send-msg 9; moot for OVMX-as-joiner-starts-fresh; S1→WARN; Fable). RUN THE GATE on every re-fire pcap + pre-fire. → build+reset+re-fire (E78+E79: coordinator drives op-06 → OVMX coalesce-acks (no flood, no crash) → op-09/GO/12-barrier → op-0c → MEMBER → CN=3).

### E79. ⚠⚠ CRITICAL (dispatched, o5) — OVMX FLOODS the op-06 MEMBERSHIP response → crashes VAX2 (INVEXCEPTN). OVMX emits ONE cat-04 op-00 per received op-06 (254 in 31.6ms) instead of the golden joiner's SMALL batched ack set (04/49, 04/00, 04/02 = 3 frames answering the whole ~254-frame op-06 burst). Also: the FSM reaches MEMBER PREMATURELY on the op-06/CSID stream, before the barrier — MEMBER must be gated on the real barrier op-0c COMMIT, not op-06. FIX: (1) decode the golden op-06 response byte-exact — WHEN/how many cat-04 the joiner sends (04/49, 04/00, 04/02: what do they mean — a Phase count/bitmap ack, sent once at burst-end or on specific op-06s?), and the op-09/GO/12×barrier(0b/0c)/op-0c commit that follows → MEMBER; (2) fix OVMX's op-06 handler to NOT emit per-frame (consume the burst, emit the correct batched cat-04 set) — a flood-guard/rate-cap alone is not enough; do it the golden way; (3) move the MEMBER transition to the real barrier op-0c commit; (4) drive the barrier (op-0b/0c) which never fired. Ground vs golden vax3-2to3 + [[cluster-barrier-5of12-empty-payload]] (op-0b empty payload) + docs/cluster-protocol-spec.md §4 + VAXcluster Internals Phase-1/2 barrier. ⭐⭐ INV-6: no synthetic MEMBER; MEMBER only on the real op-0c; every cat-04/barrier value from real CSB/wire state. This is critical — OVMX is CRASHING the reference VAXes. Commit fc-e79; integrator resets pod (VAX2 down) + orchestrates re-fire ONLY after the fix.

### E79 RE-FIRE (fresh pod, TAG e79refire-1788556956) — ✅✅ CRASH + FLOOD CONFIRMED FIXED (safety gate CLEAN); stalled EARLY on the coordinator race (never reached COMMIT). **CRASH-CHECK CLEAN** (zero CNXMGRERR/INVEXCEPTN, both VAXes up, 0 reboots — E77+E79 held). **⭐ SAFETY GATE CLEAN on OVMX's live output: `0 FATAL, 0 WARN` (cm_wire_safety_audit.py on the pcap)** — the flood is gone, no over-send/flood pattern. But CN=2: this run OVMX drove the FULL op-02 CONFIG to **VAX1** (VAX2 got only MODEL+PARAMS, no CONFIG); VAX1 answered PARAMS only, **NO op-03 COMMIT**, no RX_COMMIT ever; parked at ADMIT right after RX_CONFIG (earlier than E74/E77/E78 which drove VAX2 and reached COMMIT). No credit/flood/skew — a plain stall. CSB State-09-wait, CSID 0. **ROOT: the coordinator-selection RACE + the missing E74 re-issue-on-silence.** E74/E78 reached COMMIT because the race drove VAX2 (which becomes coordinator + commits); this run drove VAX1 (the quorum-disk watcher — plausibly can't/won't propose for a new member), and OVMX did NOT re-issue to VAX2. The E79 barrier fix is thus STILL UNTESTED live (never reached op-06 this run). → E80. pcap/node-log on tank (join-e79refire-1788556956).

### E80 — ✅ FIXED (fc-e80 @ 0999560e, integrated tip 0999560e, 63/63 + unit 112/112 + full 239/239 + test_cnxman_join 494 checks + gate OK + wire-safety PASS + elf32-vax ALL PROOFS PASSED). RE-ISSUE-ON-SILENCE (deterministic coordinator, stop depending on the TCG race): on [ADMIT][TIMER_JOIN], if the selected member doesn't PROPOSE (RX_COMMIT/MEMBERSHIP/TR_OPEN/TR_GO/BARRIER/REBUILD from ITS OWN CSB) within `CNXMAN_JOIN_ADMIT_SILENCE_BEATS=6`×1000ms (grounded: E74 real VAX took a 2nd request 6.2s later vs a willing coordinator proposing in ~ms) → log `%CNXMAN, membership request to <node> not answered`, mark declined per-attempt, `join_next_askable()` reselects CLUB-queue-tail-first (skips declined, requires OPEN conn + nonzero Con.ID), `join_reissue_to()`→`join_cm_take(new conid)` (resets burst mask, E77) + re-offer MODEL/PARAMS-if-due + CONFIG. ONE member at a time, NEVER fan-out; all-declined → FAIL_UNANSWERED, back off one RECNXINTERVAL (from club.recnxinterval), →IDLE (E71). **Sharp call: the cat-04 ACK alone is NOT "answered"** (E73's VAX1 acked all run + never proposed) — decline gated on the PROPOSAL, ack tracked as `declines_after_ack`. ⭐ CRASH-SAFETY GUARD: the beat is HELD while `cnxman_barrier_transition()` says a real transition runs (p.7-30 forbids a 2nd) — closes the only way a re-issue could worsen things. Safety gate reasoned class-by-class + wire-safety self-test PASS. IDENTITY FILLS folded in (grounded): passive CONNECT_RESPONSE conndata=E31 quad (§4(N), was 16 spaces→VAX Eco/Version 32/32), op-01 SWVers from cluster_sysgen_sw_version(). DEFERRED (no offset guessed): LOCKDIRWT/EXPECTED_VOTES/QDSKVOTES/incarnation (§4(j) grounds only VOTES@body[22:24]+version; rest RE-gap, counted-absent); **op-14 MODEL — NO executive source exists** → omitted+counted → rd follow-up (carry model via SYSGEN_LOAD). E74 conndata protocol/ECO ranking NOT implemented (all 148 VAX values identical → not a rank). Escalation (judgement, non-blocking): decline gated on proposal not ack-advance (both-absent would never fire the fixed case); one-liner if Fable wants ack-advance as a stopping fact. **→ build+reset+re-fire: deterministic coordinator → op-06 → the E79 barrier (finally tested end-to-end) → op-0c → MEMBER → CN=3.**

### E80. ⚠ NEXT (dispatched, o5) — implement the E74 coordinator spec, primarily RE-ISSUE-ON-SILENCE (deterministic coordinator, stop depending on the TCG race). E74 Fable ruling: recipient-of-op-02 BECOMES coordinator, joiner sends op-02 to ONE member (never fan-out), silence = the willing coordinator doesn't ack (cat-04 within ~0.3ms) / doesn't propose (op-03 within ~ms). FIX (E74 spec item 5): if the target CSB's dialogue shows NO op-03 / no SYSAP-ack advance within a grounded interval (~6 beats/6s per E74, from a real VAX accepting a 2nd request 6.2s later), log `%CNXMAN, membership request to <node> not answered`, mark it declined FOR THIS ATTEMPT, re-select excluding declined (rank by conndata (byte1,byte0) then CLUB-queue-tail), re-emit MODEL/PARAMS-if-due + CONFIG on the new target's OPEN connection; after all decline, back off one RECNXINTERVAL + restart. NEVER fan-out. ALSO fold in the E74 hollow-identity fills IF cheap (they weren't the E78/E79 blocker but are needed): passive CONNECT_RESPONSE conndata=E31-quad not 16-spaces (VAX records Eco/Version 32/32); op-14 real MODEL; op-01 LOCKDIRWT[20:22]/EXPECTED_VOTES+QDSKVOTES[24:28]/incarnation[64:72]/SWVers[88:96] — flag the unpinned offsets, don't guess. ⭐⭐ CRASH-SAFETY: run `tools/cluster/cm_wire_safety_audit.py` reasoning on the re-issue output — the re-issue must NOT introduce a new crash-vector (a fresh op-02 to a new Con.ID = normal; ensure envelope send_msg=per-connection + no over-send). INV-6: no synthetic membership; the re-issue is a faithful VMS behavior (any member is a legal VAX_A; coordinator "effectively random"). Grounds: E74 ledger + Fable ruling + golden. Commit fc-e80; integrator resets+re-fires (this also finally tests the E79 barrier fix end-to-end when a committing coordinator is reached).

### E80 RE-FIRE (fresh pod, TAG e80refire-1788563452) — ❌❌ CRASH-LOOP: OVMX crash-looped the reference cluster; CNXMGRERR is BACK (E77's fix re-triggered by E80's re-issue); the "MEMBER" reading was a FALSE POSITIVE (post-reboot window). Agent self-corrected a false-negative crash-check (its `SYSBOOT>` grep missed reboots; the real boot banner "Major version id" fired). ACTUAL: **vax1.log 7 distinct CNXMGRERR blocks / 8 reboots; vax2.log 12 CNXMGRERR / 13 reboots** — all PC=87BC0B11 (same bug), register dumps differ (genuinely distinct events). Live post-run: VAX1 F$GETSYI CLUSTER_NODES=**1** (lost VAX2). The harness "(b) PASS OVMXJ1 MEMBER, 40 polls" was almost certainly captured in a narrow post-reboot window during the crash-reboot cycling — NOT a stable/real MEMBER (walked back). Possible node-identity confusion after repeated reboots (flagged, unresolved). **TWO STRUCTURAL LESSONS:** (1) **OVMX RESILIENCE (E71/E80) turns ANY single crash-vector into a CRASH-LOOP** — it retries/re-issues → crashes the peer → peer reboots → retries → crashes again (7-13×). Catastrophic vs prod. (2) **The crash-safety gate is POST-HOC (audits a pcap AFTER) — it did NOT PREVENT the crash.** The operator invariant [[ovmx-never-crashes-a-peer]] must be enforced at EMIT TIME (runtime guard), not audited after. ⚠ LAB DEGRADED — full reset to recover; **HOLD ALL RE-FIRES until E81+E82 land.** → E81, E82. pcap/node-log on tank (join-e80refire-1788563452).

### E81 — ✅ FIXED (fc-e81 @ 6cc87573, integrated tip 6cc87573, 64/64 + unit 113/113 + wire_safety+emit_guard 2/2 + full 240/240 + gate OK + elf32-vax ALL PROOFS PASSED; teeth: revert → 3 further CONNECT_REQ = the loop). **REFUTED the S2/unbacked-ack premise** — measured ALL 467 OVMX CM frames in the E80 crash pcap: SYSAP-envelope ack#>peer-high = 0, SCS-VC recv_ack>peer-send_seq = 0, auditor 0 FATAL/0 WARN. OVMX's acks were ALWAYS backed (deep dialogue opens send=1/ack=0). **REAL CRASH VECTOR (deterministic 15/15): a SECOND VMS$VAXcluster CONNECT_REQ to a system that ALREADY answered.** Sequence: OVMX's ACCEPTED working CM CDT (81290012) spuriously closes PATHLOST 17.4s after OVMX advertised (VAX still HOLDS it) → the p.7-30 ladder reconnects (CONNECT_REQ) → VAX answers REJECT_REQ (0.4ms, it holds the conn) → the reject reached nobody → OVMX ASKS AGAIN (1.02s) → ~470ms later VAX bugchecks CNXMGRERR + 0xb1. 15/19 departures preceded by that connect pair; the 1 isolated connect = NO crash. ROOT IN CODE: `cnxman_vc_closed()` routed SCS_CLOSE_REJECTED to `cnxman_join_rejected()` + returned — the CSB LADDER was never told, so the ladder (CNXMAN_CSB_ACT_RECONNECT) kept doing exactly the retry the join forbids (p.2-25/D12: a REJECT is the peer's judgement). FIX (FSM only, guard untouched): new `CNXMAN_CSB_EV_CONNECT_REJECTED`; `h_connect_rejected` (WAIT/RECONNECT→WAIT, next_attempt_ms=deadline_ms = ZERO further attempts this window, membership held, [WAIT]CONNECT_RCVD→h_reaccept kept = stop ASKING not ANSWERING); `h_initial_connect_rejected` ([CONNECT]→NEW); `cnxman_vc_rejected()` tells ladder+join + RELEASES the refused Con.ID via `cnxman_csb_bind_connection(csb,0)`; `connect_rejects` counter. Rate-limit = the executive's own deadline_ms (max RECNXINTERVAL, remote-supplied), no new constant. Ack side VERIFIED not-the-bug: bind_connection already restarts cm_send_msg AND cm_ack_msg per Con.ID (test: ladder's fresh Con.ID → send-msg#1/ack0). Guard proof: corrected re-issue → guard_refused==0 (SAFE not tolerated); an ack above the circuit high still refused. Honest limit: the guard's high-water is per-CIRCUIT not per-Con.ID-pair (E82 calibration — corpus shows real VMS continuing counters across Con.ID pairs) → the per-Con.ID ack rule is the CSB's (test_cnxman_csb.c), not the guard's. → E83.

### E81 RE-FIRE (fresh CN_2, TAG e81refire-1788573880) — ✅ crash-LOOP GONE (E80: 7-13 crashes each → now exactly ONE per VAX, no self-perpetuation, ring didn't overflow) but ❌ NOT zero + the guard didn't catch it. vax1: 1× CNXMGRERR (1 crash-reboot); vax2: 1× CNXMGRERR (1 crash-reboot). Safety gate CLEAN (0 FATAL/0 WARN) + **E82 GUARD NEVER FIRED** → **guard COVERAGE GAP: the crashing frame is a connect-family CONNECT_REQ, and the guard judges ONLY the sequenced 190-content VMS$VAXcluster class** → E84. CNXTRACE: state=IDLE failure="no member to join through", VC_CONNECT↔ADMIT cycling w/ cdt-closed rc=5, 2 IDLE regressions (seq35/67), then parks IDLE (no ring overflow — E80 had 696/440 dropped). Confirms the E83 spurious-PATHLOST shape. Post-run lab DEGRADED (both VAX CLUSTER_NODES=1, not re-converged). ⚠ recurring anomaly: vax2.log.in shows `node VAX1` banner (identity-confusion, flagged twice, likely a lab-console FIFO artifact after reboots — operator note). **BOTH the residual crash AND the stall root to the spurious PATHLOST (E83): accepted working conn dropped → ladder reconnects → a reconnect to a conn the VAX still holds crashes it once. Fix E83 → no reconnect → no crash + no stall.** → E83 (keystone), E84 (guard connect-family gap).

### E84. ⚠ GUARD COVERAGE GAP (operator invariant [[ovmx-never-crashes-a-peer]]) — E82's emit guard at pe_vc_send_frame judges only the sequenced 190-content VMS$VAXcluster class; the E81/E80 crash is a CONNECT-FAMILY frame (CONNECT_REQ to a system the peer still holds) which the guard does NOT judge → it crashed the VAX with the guard clean. Extend the guard (or add a peer-connect guard at the connect-family send point) to refuse a CONNECT_REQ when the executive/CSB shows the peer already holds an OPEN connection for that SYSAP+node (the "2nd connect to already-connected" vector). ⚠ hard part: the spurious-PATHLOST means OVMX THINKS the conn is dead — so the guard needs the CSB/ladder state, not just OVMX's local VC state; if E83 removes the spurious drop this path closes, but the guard gap is a real defense-in-depth hole for OTHER connect-family vectors. Lower priority than E83 (which removes the trigger); do after E83.

### E83 — ✅ FIXED (fc-e83 @ 010027f1, integrated tip 010027f1, 64/64 + full 240/240 + unit 113/113 + wire_safety+emit_guard 2/2 + gate OK + elf32-vax ALL PROOFS PASSED + test_pe_vc 379 checks; all 4 guards teeth-proven). **REFUTED the VC-idle-timeout premise** — pcap: OVMX's advertise (seq13/14) was ACKed (VAX 0x48 ack=14), unacked==0 → TIMVCFAIL DISARMED; the circuit never went down, only its CONNECTIONS did. **REAL ROOT: ONE VAX ON MULTIPLE CHANNELS.** VAX1 presented on 3 Ethernet sources (85:61, 7a:fa:e2, 01:04) all carrying LAVC aa:00:04:00:01:04 = SCSSYSTEMID 1025 node VAX1. `vms_pe_fsm.c` keyed a circuit on the CHANNEL → one VAX got 3 circuits; SCS keys its System Block on the SYSTEM → `scs_fsm_vc_down()` on ONE dead channel closed EVERY connection to VAX1. The 17.4s = channel 85:61's PE_LISTEN_TIMEOUT_DEFAULT_MS (20s, §4M) from its last frame — while VAX1 was ALIVE on 01:04 (HELLOs throughout). FIX = the FAITHFUL model (`vms_pe.h` §4 "a VC with the remote SYSTEM"; design-cluster-node.md:36 "one or more paths/channels"): `vc_for_path()` one circuit PER SYSTEM (a 2nd verified channel = a PATH not a circuit); `vc_failover()` on CHANNEL_DOWN MOVES the circuit to another live channel (no vc_down/path-lost/reconnect/CONNECT_REQ); `pe_vc_rx_frame()` delivers alt-path frames to the one circuit; `vc_readdress()` rebuilds a retransmit's 4 link-addr fields from the current channel (INV-6, ring bytes kept). Nothing assumed-alive (path counts only if b4 + inside §4M deadline RE-TESTED + link up; link-down told to circuits before channels). **CRASH-SAFE: no new frame class/value; failover emits nothing; NET strictly FEWER connect-family frames → removes the crash trigger faithfully; E82 S2 holds; one-VC-per-system tightens the guard's per-circuit ledger to per-system.** **EXPLAINS the identity-confusion anomaly: SIMH churned VAX1's MAC across the crash-reboots (85:61 advertised sysid=1026 VAX2 early, 1025 VAX1 from t=111.7) — a real lab fact the fix handles** (⚠ reset the lab CLEAN before re-fire or the topology stays weird). Escalations: branch-name misnomer (kept); multi-rail is a faithful MINIMUM (one channel at a time + failover; full load-share deferred); E84 connect-family guard gap mooted-not-closed; new counters not in the frozen /dev/vms view (4-place ABI, not taken). **→ build+CLEAN-reset+re-fire: no spurious PATHLOST → the CM connection stays up through the coordinator wait → op-06 → the E79 barrier → MEMBER → CN=3, AND crash-free (the reconnect trigger is gone).**

### E83. ⚠ NEXT (the join STALLS even post-E81 — no crash now, but no progress): the SPURIOUS PATHLOST on the ACCEPTED CM connection. E81 stops the crash but the ROOT trigger remains: SCS reported OVMX's accepted, working VMS$VAXcluster CM connection (81290012) as PATHLOST `cdt-closed rc=5` **17.4s after OVMX advertised on it, while the VAX still HELD it** (hence the VAX's later REJECT of the reconnect). That local-vs-peer VC-timeout SKEW drops the working connection → the join loses its progress + stalls. DIAGNOSE: why does OVMX's VC declare the CM connection PATHLOST at ~17.4s while the peer holds it — a VC listen/idle timeout (TIMVCFAIL/§4k) firing during the post-advertise wait for the coordinator's op-03? missing keepalive on an idle-but-live VC? Fix so a live CM connection (peer still holding it) is NOT declared PATHLOST during the coordinator-response wait (keepalive, or the correct VC-timeout grounded from the golden's inter-frame gaps + p.7-30). ⚠ RUN E82's guard + crash-check on any re-fire. Grounds: golden VC timing + VAXcluster Principles VC/PATHLOST/§7.10.

### E81. ⚠⚠ CRITICAL (dispatched, o5) — the E80 re-issue RE-TRIGGERS CNXMGRERR (PC=87BC0B11) → crash-loop. E77 fixed the per-CSB-not-per-connection envelope for the normal path, but E80's re-issue-on-silence (opens a NEW Con.ID + re-emits, rapidly, on every silent member) re-triggers the SAME CNXMGRERR the VAX CM bugchecks on. E80's crash-safety REASONING said the re-issue resets the envelope (send_msg=1/ack=0) but RUNTIME proves otherwise (beds-vs-runtime AGAIN). DIAGNOSE from the pcap `join-e80refire-1788563452.pcap`: decode the exact envelope (send_msg#/ack#/Con.ID) on OVMX's re-issued CONFIG frames that the VAX bugchecked on (~1.2ms after) — does the re-issue's new-Con.ID binder (E77) actually fire, or does the rapid re-issue race it / carry a stale ack / reuse a counter? Fix so the re-issue's envelope is provably per-connection-clean AND rate-limited (a crash-loop means the re-issue fired far too fast/often). Ground vs the golden. Commit fc-e81. ⚠ do NOT re-fire until E82 (guard) also lands.

### E82 — ✅ LANDED (fc-e82 @ d4a5779b, integrated tip d4a5779b, 64/64 + unit 113/113 + wire_safety+emit_guard PASS + gate OK 75 + NetBSD ELF32 ALL PROOFS PASSED). RUNTIME EMIT-TIME SAFETY GUARD [[ovmx-never-crashes-a-peer]] at `pe_vc_send_frame()` (lowest point every sequenced frame passes, before the seq is taken — a refused frame consumes nothing; NOT on retransmit). Pure TU `vms_cluster_emit_guard.{c,h}`. DROP: S2 ack≤peer_high+1, S3 ack-coalesce≥3, S10 conid-nonzero, S11 frame-size-204, S12 credit send_seq−peer_recv_ack≤grant, S8 no-response-to-0a/0c, S9 no-txn-0-response. WARN: S1 envelope-jump, S4 ack-rate, envelope-stall. **CALIBRATION: WARN-vs-DROP = only what the auditor's ZERO-FALSE-POSITIVE corpus covers → never blocks correct output** (proof: pre-existing 63/63 stayed green with ZERO edits; golden dialogue guard_refused=0; E77-rebind→no finding). INV-6: peer_high from frames really DELIVERED (vc_deliver), own_high from frames the substrate really TOOK (E76's counter-advanced-by-never-sent root cause has a named test); unsafe DROPPED never clamped. Reporting: PE_VC_SEND_UNSAFE=-7, per-circuit+port counters, CNXMAN_DIAG_R_UNSAFE_EMIT=22, throttled `%CNXMAN refused to emit an unsafe cluster frame (<vector>)`. **⚠ CAUSAL CLAIM REFUTED by E81 (E80 pcap = 0 S2 at both layers, auditor clean — OVMX's acks were ALWAYS backed): the CNXMGRERR is NOT an unbacked ack. E82's guard is still valuable defense-in-depth, but the real crash cause is E81's (retry-a-REJECTED-connect). Original (wrong) hypothesis was: unbacked ack (S2) vs the send-msg# restart E77 chased** (real VMS opens a new Con.ID CONTINUING its count → send-msg≠1 is WARN). So E80's re-issue crashes by carrying an ack the peer never backed on the new Con.ID — the guard's S2 now PREVENTS it. Escalations (send-point can't cover): echo-fidelity S5/S6/S7 (LOCKMGRERR/INCONSTATE — needs per-txn request body held in the responder FSM, not port state); Con.ID-open half already gated in scs_fsm_send_msg (cdt OPEN + vc_up); guard counters not yet in /dev/vms `vms_pe_view` (4-place ioctl follow-on); judges only the 190-content VMS$VAXcluster class (MSCP bodies never ride it, withheld not asserted). **The guard is the release-gating backstop; the FSM still gets fixed (E81).**

### E82. ⚠⚠ THE OPERATOR'S STRUCTURAL ASK (dispatched, o5) — RUNTIME PRE-SEND SAFETY GUARD so OVMX is STRUCTURALLY INCAPABLE of emitting a peer-crashing frame [[ovmx-never-crashes-a-peer]]. The post-hoc gate (cm_wire_safety_audit.py) didn't prevent E80. Build an EMIT-TIME guard on OVMX's cluster send path (the SCS/PE send, `vms_scs.c`/`vms_pe_fsm.c`, before the frame hits the wire) that enforces the 9-class measured invariants (envelope send_msg per-connection + ack ≤ what the peer sent on THIS connection; credit ≤ granted / no over-send; no flood/coalesce quantum; valid Con.ID nonzero; frame size; op-legal-in-state) and REFUSES to emit (fail-safe, counted+logged `%CNXMAN` + a diag ring record) any frame that would violate — so no FSM bug (present or future) can crash a peer. This is defense-in-depth: the FSM should be correct, the guard GUARANTEES safety. Reuse the audit's grounded thresholds. INV-6: the guard checks against real per-connection/CDT/credit state, never fabricates. Host tests: a synthesized unsafe frame is REFUSED (not sent); a valid frame passes; the refusal is counted. This is release-gating for any cluster ship. Commit fc-e82.

### E83 KEYSTONE RE-FIRE (fresh single-identity CN_2, TAG e83refire-1788612567) — ✅✅ DEEPEST EVER (reaches BARRIER + genuine barrier-ack) but ❌ still 1 crash/VAX + connection dies post-barrier-ack + E83 failover didn't fire. Rigorous pre-check passed (VAX1 self-sysid 1025, VAX2 1026, no residue, CN_2). CNXTRACE state=**BARRIER** (deepest): seq346 ADMIT→BARRIER **RX_TR_GO** (clean op-0a GO), seq349 BARRIER **RX_BARRIER_ACK** (genuine, FIRST EVER), no flood, ring 382/126 (E80 was 696/440). **BUT seq350 cdt-closed rc=5 (connection dies right after the barrier-ack)** → reconnect cycles ~4min (each RX_CONFIG→MODEL+PARAMS→cdt-closed) → parks BARRIER failure="path lost", never op-0c/MEMBER. CRASH-CHECK: vax1 1× CNXMGRERR (1 reboot), vax2 1× (1 reboot) — same as E81, NOT zero. Post-run VAX1 CN=1 (degraded). Safety gate CLEAN (0 FATAL/0 WARN, 414 CM frames); **multi-MAC CONFIRMED present (3 MACs: 85:61, 01:04, 7a:fa:e2) so E83's target scenario WAS exercised — but E83 failover ("channel lost, VC continues on another channel") did NOT appear + E82 guard didn't fire.** So E83's one-VC-per-system failover isn't engaging at runtime for THIS close, OR the post-barrier-ack close isn't the channel-listen-timeout E83 handles. Leg-(b) "OVMXJ1 MEMBER SDA" PASSED 40 polls BUT is again a crash-window false positive (CN=2, connection died). → E85. pcap/node-log on tank (join-e83refire-1788612567).

### E85. ⚠ NEXT (dispatched, o5) — KEEP THE CONNECTION ALIVE THROUGH THE BARRIER (fixes BOTH the residual crash AND the stall). The connection dies (cdt-closed rc=5, "path lost") RIGHT AFTER the RX_BARRIER_ACK (seq349→seq350), before op-0c → the barrier never completes + the reconnect that follows crashes the VAX once (the E81 vector, only the RE-ask is stopped). E83's one-VC-per-system failover did NOT fire despite 3 MACs present. DIAGNOSE from the E83 pcap/CNXTRACE: (1) the EXACT close reason at seq350 — is it the ~17.4s channel-listen-timeout (§4M, which E83's vc_failover SHOULD catch but didn't → E83 runtime path not engaging, beds-vs-runtime), or a NEW close (peer DISCONNECT, a post-barrier timeout, the barrier's own timing)? (2) why E83 failover didn't fire (does vc_failover only handle CHANNEL_DOWN, but this is a CDT-close on the active path? is the alternate channel not being recognized as a path?). (3) The barrier is a 12-step star (0b/0c×12); this run got ONE barrier-ack then died — does OVMX need to DRIVE the next barrier steps (send its 0b) and it's idle-waiting → the connection times out? FIX so the connection survives the full barrier (12 steps) → op-0c → MEMBER, and NO reconnect (→ no crash). ⭐⭐ operator never-crash [[ovmx-never-crashes-a-peer]]: if the connection can't always be kept alive, the reconnect must NOT crash the peer — coordinate with E84 (guard connect-family). INV-6. Also: E84 (extend E82 guard to connect-family CONNECT_REQ — the still-open defense-in-depth hole; the residual crash is a connect-family frame the guard doesn't judge) remains the backstop. Commit fc-e85; integrator resets+re-fires.
