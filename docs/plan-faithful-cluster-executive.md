...
| FC-P2.2 | SCS FSM: SB per remote system, CDT connection ladder (OPEN→DISC SENT/RCVD→MATCH→CLOSED), Con.ID allocator (§4(t)), credit ledger per CDT (Send/Receive/Pending, special credit message 8 → 9), MTYPE dispatch to the CDT input routine; the CDL is live for data. | R1: every ladder transition; credit conservation property test; the 8-before-DISCONNECT invariant. | `vms_scs_fsm.{c,h}`, `vms_scs.h` | P0.1, P1.3, P2.1 | **O5** (high) — the daemon's CDL/credit path was dead code because this is where the design judgment lives | — |
...
| FC-P3.12 | Coordinator FSM, ADD class (`vms_cnxman_coord_fsm.c`): op-02 receipt → op-12 relay to members → op-03/05 → op-09 open (epoch, bitmap) → op-0a GO → `12×(M−1)` barrier driver holding step N until all report → op-0d rebuild push hook (DLM callback; echo-only until P5). Selection: drive only when this node received the op-02 or detected the event and no other CM has opened a transition; defer on collision. | R1 per step; R2 an all-OVMX 3-node cluster forms and adds a 4th. | `vms_cnxman_coord_fsm.{c,h}` | P0.1, P3.1, P3.5 | **O5** (high) — coordinator obligations are the `12×(M−1)` law; the selection predicate is INFERRED (§5.5, CORRECTED by book-grounding D7: the joiner picks + the coordinator lock) | — **LANDED** with three DEFERRALS, each counted in the FSM, none faked: (a) **op-0x06 MEMBERSHIP burst not originated** — the `{SCSSYSTEMID, incarnation, CSID}` record has no isolated offset (spec §4(j) RE gaps), so a coordinated ADD completes the barrier but never TELLS the joiner the CSID it assigned; closing it needs the op-0x06 record layout from a capture (**blocks FC-P3.10's R2 ‘forms and adds a 4th’ and FC-P3.3's CSID-learning**). (b) **op-0x05 lock-rebuild burst and the ORIGINATING cat-0x02 op-0x0d not built** — the op-0x0d L1 region body[16:34] is only ever observed inbound; FC-P3.12 implements the other half of that obligation (HOLD the barrier while records are outstanding, spec §4(p)) through `cnxman_coord_rebuild_ops`, and FC-P5.5 owns the push. (c) **Phase 1 proposal cells** (proposed quorum / CEVOTES / qdisk votes / foundation time / founder / rebuild type, book p. 7-40) go out ZERO — no offset is isolated for any of them. |
...
| FC-P7.2 | Cluster-wide file access: an RMS open on a served volume takes the `F11B$`/`RMS$` locks through P4/P5 and blocks against a VAX holder. | R4 with two OVMX nodes; R5 `RMS-E-FLK` both directions + SDA shows both CSIDs on the file lock. | `vmsfs_acp.c` lock paths (already real), test scripts | P7.1, P5.9 | S (medium) | LAB |
...
P0 16 · P1 7 · P2 6 · P3 15 · P4 11 · P5 9 · P6 4 · P7 2 · P8 5 · P9 (4 candidates) — **75 items + 4 candidates**. Opus-5-tier: 25; Sonnet-tier: 50 (of which 16 are lab-lane).

Standing seam rulings that bind the glue items (P0.9, P1.3, P2.2, P2.4, P3.8): design §3.2.3 (receive-level lock — `exec_rxlock_t`, one object) and §3.2.4 (buffer granularity — body-level at every SYSAP seam; port owns 0–55, SCS 56–71, SYSAP 72+; `pe_send_msg` takes a service kind; `cnxman_envelope_stamp` is the only writer of body[0:8]).
