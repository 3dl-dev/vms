/*
 * ovmx_rms_io.c — OVMX addition (vms-0b6b), NOT part of stock GNU binutils.
 * Mirrors third-party/tcc/ovmx/ovmx_rms_io.c (bead vms-4ba.5) — same RMS
 * calls, applied to GNU as (gas) instead of tcc. See ovmx_rms_io.h for
 * scope/rationale. All sys$ calls trace to stderr ("OVMX-RMS: ...") so a
 * harness can grep proof that the RMS path — not raw POSIX open/read/write
...
int ovmx_rms_open_read(const char *filename)
{
    ...
    sl->fab.fab$b_rfm = FAB$C_FIX; /* raw fixed-size chunks: byte-exact, no
                                     * text delimiter munging of the .s source */
...
/* scratch file gas/BFD already finished writing (raw ELF bytes untouched)*/
