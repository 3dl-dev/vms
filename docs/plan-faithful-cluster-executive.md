...
| FC-P2.7 | **Receive-classify widen for SCS application messages** (design §3.2.7 ruling, E48): new class `VMS_FCLS_SCS_APPLMSG` matching `content[44:46]==0x0004 ∧ content[46:48]==10 ∧ content[42:44]==content_len−44` at any length, granting `VMS_FCAP_CONID` at content [50:58], body at content 58; ordered after the connection-control classes; `SCS_APPLMSG94` kept as an alias; block-transfer frames unaffected (fail the format-word test). `vc_deliver` routes every MSCP END (86/90/102/110) to the class driver's CDT. | R1: the vms291 END specimens at 86/90/102/110 classify `SCS_APPLMSG` with the Con.ID pair = the provoking command's pair reversed; frozen-table vectors unchanged (no-regression net); R2: FC-P6.5's scenario shows `vc_rx_undelivered == 0` for END messages; R4: a booted class driver completes SCC/GUS/ONLINE/READ against an OVMX server. | `vms_cluster_codec.{c,h}` (classify table), `vms_cluster_codec_scs.h`, `vms_pe_fsm.c` (`vc_deliver` unchanged in logic), `tests/cluster/host/` | P2.1, P6.2 | S (low) — the rule is exact and the specimens exist | — |
...
| ID | Outcome | Done-condition | Files / interfaces | Blocked-by | Caliber | Gates |
|---|---|---|---|---|---|---|
| FC-P4.1 | **DONE 2026-09-03** — `docs/research-dlm-directory-algorithm.md` (Davis ch. 6/7, page cites): vector + index rule fully published (rung A); hash function not bit-level, but the 16-bit value rides every lookup on the wire (p. 6-50) → rung A′ hash-from-the-wire; residual = novel root names OVMX touches first; recommendation A + A′ now, C(i) to the operator with the p. 6-50 framing. | Landed. | — | — | — | — |
| FC-P4.2 | **Directory checks (revised per FC-P4.1 §5).** Offline (no lab time): (1) confirm body[10:12] of cat-02 op-01 is the hash — constant per resource name across senders/occurrences, varying across names — over `/tmp/ref.pcap` + the db20-b/2f2025a7 pcaps; (2) with P3.2's LOCKDIRWT values and CSV-index order, check every lookup's destination == `ldwv[hash mod n]` (zero residuals confirms the order; a permutation identifies it). On the clone (one session): the mis-addressed lookup with the **correct** hash → reply class + `SDA> SHOW RESOURCE` on the wrong node (entry created ⇒ probing unsafe). | Note `docs/research-dlm-directory-observed.md` + CSV; pcap retained. | existing captures; lab (second half) | P3.2 (second half only) | S (medium; lab for the second half) | LAB (second half) |
| FC-P4.3 | **DONE 2026-09-03** — `dir_resolve` per the resolved ladder. `struct vms_ldwv` in the CLUB (`vms_cluster.h`) with its behaviour in the new pure TU `vms_dlm_ldwv.{c,h}`: entries per member = LOCKDIRWT, one each when every weight is 0 (and when none has been advertised), contiguous runs, own entries 0, CSV-index order over the SELECTED CSBs; invalidated at Phase 1 by both transition FSMs and refilled at Phase 2 (task 5 of `cnxman_phase2_commit`). `rsb->hash16`/`hash_known` LEARNED by `vms_lock_dlm_learn_dir_hash()` from `vms_dlm_dir_hash_parse()` (body[10:12], INFERRED pending FC-P4.2); `dir_resolve` = `ldwv[hash16 mod n]` (0 ⇒ self) requiring `hash_known`, reached from the engine through `dir_resolve`/`dir_generation` on the FC-P4.4 requester ops — a seam that takes the HASH, never a name. A cached `dir_csid` carries the vector's GENERATION, so a vector change invalidates every one of them by construction. Novel root ⇒ `SS$_UNSUPPORTED` with nothing sent. Self-check `cnxman_dir_lookup_received()` → `dir_lookup_misaddressed`. `exec_jhash` DELETED from all four seam backends; the static `dlm_member_csids` insmod vector deleted with it. | R1 `tests/cluster/host/test_dlm_ldwv.c` (74 checks) + `test_lock_dir.c` (46) + the codec accessor in `test_codec_dlm.c`; R2 `tests/cluster/sim/scenarios/dlm_directory.c` (25) — four systems' independently built copies agree, every lookup lands, a deliberately mis-ordered vector is CAUGHT. | `vms_dlm_ldwv.{c,h}`, `vms_cluster.h`, `vms_lock.c`, `vms_dlm_proxy.h`, `codec_dlm`, `vms_cnxman_phase2.c`, both transition FSMs | P4.1, P3.5 | **O5** (high) | OP (rung C only, for the novel-name residual) |
...
| FC-P4.6 | **DONE 2026-09-03** — `vms_dlm_scs_fsm.{c,h}`: a pure `[state][event]` requester arm (IDLE/LOOKUP/ENQ/GRANTED × 10 events; an empty cell is COUNTED). Lookup → enqueue → grant → completion+commit (op-0x04/0x03, marked PROVISIONAL); outcome-2 REDIRECT and outcome-3 ASSUME; a DECLINE re-resolves through the CURRENT vector and STOPS when it names the same node (the grant storm's cure); inbound BLKAST → the holder's REAL local AST through the engine; convert; retransmit ladder; `dlm_req_fsm_observe()` is `vms_lock_dlm_learn_dir_hash`'s production caller (**E49 CLOSED**). INV-6 is STRUCTURAL: `struct dlm_req` holds no name, mode, master handle, value block or hash — every outbound field comes from `ops->refill_post` (`dlm_proxy_fill_post`), called freshly per frame and TWICE for the completion pair, and even a REDIRECT is recorded in the lock database (`vms_lock_dlm_record_master`, new) and RE-READ rather than remembered. Three new kernel-core-typed engine doors: `_record_master`, `_proxy_grant_recv` (with `valblk_present`, so a grant with no grounded LVB field cannot zero a real value block), `_proxy_blkast_recv`. HONEST STOPS, each counted: the cross-node RELEASE has no grounded cat-0x02 opcode → nothing sent, `releases_no_wire_op` (**E6's open half; needs a lab-grounded opcode**); the LVB write crossing likewise; a lookup with no wire-learned hash is refused with NOTHING SENT. | R1 `tests/cluster/host/test_dlm_requester.c` (166 checks) — the fake LKB is MUTATED BETWEEN FRAMES and the wire must follow; two negative controls (cache the grant's handle / send an unlearned hash) turned it RED. R2 `tests/cluster/sim/scenarios/dlm_requester.c` (41) — the REAL `vms_lock.c` + REAL LDWV + 4 simulated systems: a cross-node $ENQ resolves through the directory to the master and grants, every lookup LANDS by the receiver's own vector, a novel root name puts ZERO frames on the LAN; negctl mis-route → RED. Both substrates build (vms.ko; elf32-vax ILP32-clean). | `vms_dlm_scs_fsm.{c,h}`, `vms_dlm_proxy.h`, `vms_lock.c` | P4.3, P4.4, P4.5 | **O5** (high) | — |
...
P0 16 · P1 8 · P2 7 · P3 15 · P4 11 · P5 9 · P6 7 · P7 3 · P8 5 · P9 (4 candidates) — **81 items + 4 candidates**. Opus-5-tier: 27; Sonnet-tier: 54 (of which 17 are lab-lane).
