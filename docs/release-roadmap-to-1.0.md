...
Four roadmap docs live under `docs/`. This one is authoritative; the others are either
...
The two *Historical* companions are kept as pre-pivot design context, not as live
...
(`roadmap-waves.md`, the old swarm wave-dispatch DAG plan, was retired in the
`vms-813bf` doc-consolidation pass — superseded by the rd dependency graph +
`docs/internal/orchestration-conductor.md`, and its invariants had drifted stale.)
...
> Reconciled from rd (source of truth) **as of 2026-09-17** by `tools/roadmap/reconcile.py`. Milestones are the `rel-*` labels; workstreams are the 1.0-gate epics rolled up over their child items. Re-derive any line from `rd show <id>` before acting on it.
...
| **0.7** | Cluster wire fidelity — a booted OVMX node joins a real VAX cluster and is admitted to sustained CN=3 on a single circuit, crash-safe. | SHIPPED | 8 | 4 | 0 | 67% |
...
- `vms-298` [done] OVMX replays a stale send_seq + reuses a Con.ID when a peer opens a SECOND MSCP$DISK connect
- `vms-3e6` [done] Cluster-config guide: re-derive votes/quorum + raw-Ethernet transport + CLUSTER_AUTHORIZE sections against the executive-resident subsystem (they still cite the deleted src/vmsscs scsd)
...
- `vms-4ce` [inbox] INV-0: correct the V0.6-11 public release note (reconcile.py EDITORIAL_CONFIG) -- its real-VAX CN=3 admission rode a crash-bug-induced NISCA reformation (not a clean single-circuit join) and its 'crash-safe / never emit a frame that can crash a peer' claim was premature (the wire carried a latent unaddressed op-5 vector that survived by luck, went fatal in later builds). Accurate once vms-f3ec root-cause fully lands; edit from repo root at the next cut
...
- `vms-80c` [done] compat: ssh$server-daemon row cites the RETIRING libssh C-reimpl (vmssshd.c), note falsely claims 'wraps OpenSSH' — should point at the wrapped-OpenSSH server once vms-16b retires the second stack
