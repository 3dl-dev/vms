...
# RMS applies authentic VMS file-versioning on sys$create: `as -o test.o` lands as
# test.o;<n> (highest version) plus an optional .rms_meta sidecar — exactly as real
# VMS creates test.o;1 and the next tool reads test.o resolving to the highest
# version. Resolve to that object rather than the bare literal path (vms-0b6b: the
# RMS ;version seam vs a POSIX-shaped assertion). (vms-da0 F1)
ASOBJ="$WORK/test.o"
if [ ! -f "$ASOBJ" ]; then
	ASOBJ=$(ls -1 "$WORK/test.o;"* 2>/dev/null | grep -v '\.rms_meta$' \
		| sort -t';' -k2,2n | tail -1)
fi
[ -n "$ASOBJ" ] && [ -f "$ASOBJ" ] || { echo "FAIL: activated AS.EXE did not produce test.o (nor a test.o;<version>)"; exit 1; }
echo "-- RMS output object (VMS ;version): $ASOBJ --"
readelf -h "$ASOBJ" | tee "$WORK/test.readelf.h" >/dev/null
