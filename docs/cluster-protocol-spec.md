...
**But rejoin is NOT closed — NECESSARY-NOT-SUFFICIENT.** With the freeze gone and op 0x02 delivered in-order to the coordinator, the member STILL returns `cm_responses=0`, never opens the reciprocal connection (`connect_sent=0`, `remote_conid=0`), and never sends its op 0x04 — `XITDONE=0`, `CLUSTER_NODES` stays 2. The daemon's own exit note names it: *"the member still declines to reciprocate on the joiner VC"* (§4(O.10), `vms-e15`). So the §4(O.19) `recv_seq` freeze was one blocker, now removed; the residual is the member's non-reciprocation, **downstream** of the freeze — the same class §4(O.16)/§4(O.17) circled before the credit framing was refuted.
