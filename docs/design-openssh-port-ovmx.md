...
> upstream OpenSSH — is the live plan, tracked under `rd vms-9ef`. The authoritative
> build shape is the substrate ladder in `docs/design-openssh-devener-map.md`: raise
> DECC\$SOCKET/\$QIO + C-RTL until STOCK upstream OpenSSH links unmodified. §§0-6 below
> are the original port plan and prerequisite analysis; for current status re-derive
> from the compat register (`docs/compat/facilities/ssh.yaml`) and `rd vms-9ef`.
...
Current status lives in the compat register (`docs/compat/facilities/ssh.yaml`, the
SSOT) and `rd vms-9ef`. The substrate build ladder is tracked in
`docs/design-openssh-devener-map.md`. §§0-6 above are the original port plan and
