#!/bin/bash
# run-mbx.sh - orchestrate the mailbox ($CREMBX/$ASSIGN/$QIO, MBAn:) runtime
# proof (rd vms-fe8, parent vms-945e -> vms-476, epic vms-8e8): mailboxes
# proven cross-process against a REAL /dev/vms on NetBSD/vax under SIMH
# (INV-6 honest).
#
# Sibling of run-eflag.sh (event flags) and run-proctab.sh (the process
# table), for the VAX architecture under SIMH. Reuses P4-B/P4-E/proctab's
# compile-into-kernel fallback unchanged (vax GENERIC omits `options
# MODULAR'; see tests/lab-vax/README.md "P4-B"): build a custom
# GENERIC+MODULAR kernel, install it, and modload the UNCHANGED cross-built
# OVMX module (which already carries src/kernel-core/vms_mbx.c, the same
# facility source the per-PR B1 gate width-checks).
#
...
#      static vmsmbx guest tool (tools/cross-vax/build-mbx-vax.sh).
#   2. BUILD-KERNEL (ovmx-cross-vax): the GENERIC+MODULAR vax kernel
#      (netbsd-OVMX) via NetBSD build.sh. Cached: skipped if the kernel
#      artifact already exists (own mbx-artifacts cache, mirrors how the
#      proctab proof's proctab-artifacts is separate from P4-E's
#      eflag-artifacts).
...
#      mknod /dev/vms, run the cross-process mailbox proof; plus the INV-6
#      module-absent control and the MBX_SKIP_LOAD/MBX_SKIP_CREATE teeth
#      checks.
#
...
#   tests/lab-vax/run-mbx.sh negctl-load    # MBX_SKIP_LOAD teeth check
#   tests/lab-vax/run-mbx.sh negctl-create  # MBX_SKIP_CREATE teeth check
set -euo pipefail
...
# to run-eflag.sh/run-proctab.sh/run-devvms.sh -- same NetBSD/vax release, same
# shared disk) ------------------------------------------------------------------
...
ARTIFACTS_DIR="${CACHE_DIR}/mbx-artifacts"           # module+tool+kernel (small)
KBUILD_DIR="${KBUILD_DIR:-${CACHE_DIR}/mbx-kbuild}"
NBSRC_DIR="${KBUILD_DIR}/nbsrc"
KERNEL_MARKER="${CACHE_DIR}/.ovmx-mbx-modular-kernel-installed"
...
log() { echo "[run-mbx] $*"; }
die() { echo "[run-mbx] FATAL: $*" >&2; exit 1; }
...
# 1. cross-build the loadable module + mailbox tool
cross_build() {
  ensure_src
  mkdir -p "${ARTIFACTS_DIR}"
  log "cross-building vms.kmod (-fno-pic, loadable) + vmsmbx for elf32-vax"
  docker run --rm -v "${REPO}:/src" -w /src \
    -v "${NBSRC_DIR}:/nbsrc:ro" -v "${ARTIFACTS_DIR}:/out" \
    "${CROSS_IMAGE}" sh tools/cross-vax/build-mbx-vax.sh
  [ -f "${ARTIFACTS_DIR}/vms.kmod" ] && [ -f "${ARTIFACTS_DIR}/vmsmbx" ] || die "cross-build missing artifacts"
}
...
build_kernel() {
  ...
  local cid="ovmx-vax-mbx-kbuild-$$"; local rc=0
  set +e
...
ensure_disk() {
  ...
  local cid="ovmx-vax-mbx-install-$$"; local rc=0
  set +e
...
# nightly run and hit again, verbatim, on vms-2e0's proctab proof: a driver's
# clean negctl-load proof-failure, exit 16, reddened the JOB even though
# vaxharness_negctl_gate() should have inverted it). `errexit` is a GLOBAL
# shell option, not function-scoped: the `set -e` this function runs right
# before `return "${rc}"` stays in effect in the CALLER after the call
...
# merged run-access.sh / run-proctab.sh fixes, rd vms-4b7 / vms-2e0).
run_session() {
  local ovmx_mode="$1" skip_load="${2:-}" skip_create="${3:-}"; local cid="ovmx-vax-mbx-${ovmx_mode}-$$"; local rc=0
  set +e
...
case "${MODE}" in
  prove)
    ...
    if vaxharness_negctl_gate "${rc}" 0; then
      log "======================================================================"
      log "  MBX-VAX PASSED: a message written by process B into the mailbox"
      log "  process A created was read back byte-for-byte by process C, on"
      log "  real NetBSD/vax under SIMH, real /dev/vms (INV-6 honest)"
      log "======================================================================"
      exit 0
    fi
    die "MBX-VAX FAILED (driver exit=${rc}; see console output above)"
    ;;
  negctl-load)
    ...
    if vaxharness_negctl_gate "${rc}" 1; then
      log "PASS: MBX_SKIP_LOAD negative control failed as required (driver exit=${rc})"
      exit 0
    fi
    die "NEGATIVE CONTROL DID NOT FAIL: harness passed (exit=${rc}) with the module unloaded -- no teeth"
    ;;
  negctl-create)
    # Teeth: with process A never launched, no MBAn: name is ever captured,
    # which makes the SAME positive-assertion script the prove mode runs
    # fail for real (drive_mbx_vax.py does NOT special-case this into an
    # early "negctl ok" exit -- see its skip_create comment, and rd vms-cf5's
    # whole point). So the gate here is IDENTICAL in shape to negctl-load:
    # negctl=1, satisfied iff the driver exited nonzero.
    rc=0
    run_session prove "" skip || rc=$?
    if vaxharness_negctl_gate "${rc}" 1; then
      log "PASS: MBX_SKIP_CREATE negative control failed as required (driver exit=${rc})"
      exit 0
    fi
    die "NEGATIVE CONTROL DID NOT FAIL: harness passed (exit=${rc}) with process A never creating the mailbox -- no teeth"
    ;;
  *) die "unknown mode '${MODE}' (want: prove | negctl-load | negctl-create)" ;;
esac
