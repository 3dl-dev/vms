/*
 * proctab_negctl.c - width-safety NEGATIVE CONTROL for the process-table
 * guest tool (vmsproctab.c), re-expressed as a ctest build-negative
 * (vms-74b, epic vms-509 "unified cross-platform build" Rung D). Formerly
 * the CROSSCOMPILE_NEGCTL=1 heredoc in
 * tools/cross-vax/build-proctab-tool-vax.sh (rd vms-2e0) -- moved to a
 ...
 * this probe depends on (vms_proctab_nb.h's uint32_t vms_pid/uic fields) has
 * no teeth.
 */
#include <stdint.h>
