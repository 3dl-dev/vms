 ...
 * route/re-style/format/destination logic fails this test (CLAUDE.md
 * Rule 10: a test against a hand-maintained mock is worse than no test).
 *
 ...
 * end-to-end proof that the real vms.ko/vmsfs.ko records reach the real
 * console, and SYSKRNL lines reach real OPERATOR.LOG WITHOUT touching the
 * console, is tests/qemu/test_job_control_console.sh (extended by this
 ...
 *
 *   1. genuine vms:/vmsfs: executive events ROUTE TO THE CONSOLE under the
 *      OVMX facility, reformatted as bare "%OVMX-<S>-<IDENT>, text" lines
 *      (docs/design-opcom-executive-logging.md sec3/sec4).
 *   2. SYSKRNL (Linux-kernel-layer) lines RE-STYLE and ROUTE TO
 *      OPERATOR.LOG -- NEVER THE CONSOLE -- when their severity is NOTICE
 *      or more severe. This is the console-vs-log split (operator
 *      correction round 2, PR #365): the first cut routed these to the
 *      console too, which reopened the PR #358/vms-2213 leak (routine
 *      kernel chatter flooding OPA0:) on a CI runner whose real hardware
 *      emits far more NOTICE/WARNING lines than this repo's dev QEMU
 *      guest. Named after the exact two examples the ruling cited
 *      (module-taint, hrtimer), with their real MEASURED kernel severity
 *      levels (4 and 5 respectively -- see the design doc sec3).
 ...
 *      text, gets the OVMX/KMOD treatment like any other unrecognized
 *      vms: line, and therefore (like every other OVMX record) goes to
 *      the CONSOLE -- disclosed in the design doc sec4, not a defect.
 */
...
static const char *dest_name(int dest)
{
    ...
    case OPCOM_KMSG_CONSOLE:      return "CONSOLE";
...
static void check_console(const char *label, int pri, const char *text,
                           const char *want_line)
{
    check_dest(label, pri, text, OPCOM_KMSG_CONSOLE, want_line);
}
...
int main(void)
{
    printf("=== test_opcom_kmsg: /dev/kmsg -> operator surface reformatter ===\n");

    /* --- 1. genuine executive events -> CONSOLE, under OVMX ----------- */
    check_console("vms: kernel-module init -> console, KMOD, info",
                  6, "vms: initializing VMS kernel module",
                  "%OVMX-I-KMOD, initializing VMS kernel module\n");
    check_console("vms: /dev/vms registered -> console, KMOD, info",
                  6, "vms: /dev/vms registered successfully",
                  "%OVMX-I-KMOD, /dev/vms registered successfully\n");
    check_console("vms: disk unit mapping -> console, DEVTAB, info",
                  6, "vms: disk unit vda -> DKA0: (0:0)",
                  "%OVMX-I-DEVTAB, disk unit vda -> DKA0: (0:0)\n");
    check_console("vms: console terminal created -> console, DEVTAB, info",
    ...
    check_console("vms: system identity constant -> console, SYSID, info",
    ...
    check_console("vms: pr_err -> console, E severity",
                  3, "vms: failed to register /dev/vms: -16",
                  "%OVMX-E-KMOD, failed to register /dev/vms: -16\n");
    check_console("vms: pr_warn -> console, W severity",
                  4, "vms: out of memory creating disk unit vdb (DKA100:)",
                  "%OVMX-W-DEVTAB, out of memory creating disk unit vdb (DKA100:)\n");
    check_console("vmsfs: pr_err -> console, E severity",
                  3, "vmsfs: unable to read home block",
                  "%OVMX-E-VMSFS, unable to read home block\n");

    /* --- 2. SYSKRNL lines RE-STYLE and ROUTE TO OPERATOR.LOG, NEVER THE
     * CONSOLE -- the console-vs-log split (operator correction round 2).
     * Operator-ruling correction, 2026-08-12: these two are the exact
     * examples named as carrying real operator-relevant information; PR
     * #365 measured that routing them to the CONSOLE reopened PR #358's
     * kernel-chatter leak, so the destination is OPERATOR.LOG, not
     * suppression and not the console. Severities are the REAL kernel
     * levels, measured against an actual boot (design doc sec3): hrtimer's
     * scheduling-latency warning is KERN_WARNING (level 4). */
    check_operator_log("hrtimer scheduling-latency warning -> OPERATOR.LOG, re-styled, NOT the console",
    ...
    check_operator_log("a NOTICE-level SYSKRNL line (the cutoff's inclusive edge) still routes to OPERATOR.LOG",
                        5, "some-driver: entering compatibility mode",
                        "%SYSKRNL-I-KERNEL, some-driver: entering compatibility mode\n");
    check_operator_log("X.509 cert load (the exact CI-measured flood example) routes to OPERATOR.LOG, not console",
                        5, "Loaded X.509 cert 'Canonical Ltd.: Secure Boot'",
                        "%SYSKRNL-I-KERNEL, Loaded X.509 cert 'Canonical Ltd.: Secure Boot'\n");
    check_operator_log("a bare substring match on 'vms' with no real prefix still gets SYSKRNL/OPERATOR.LOG, not OVMX/console",
     ...
     * cased -- it is ordinary vms:-prefixed text, routed to the CONSOLE
     * under OVMX/KMOD like any other unrecognized vms: line (design doc
     * sec4), not diverted to OPERATOR.LOG despite originating in the
     * kernel rather than vms.ko. --------------------------------------- */
    check_console("kernel's OWN taint line, with the 'vms: ' prefix collision, routes to console as OVMX/KMOD",
                  4, "vms: loading out-of-tree module taints kernel.",
                  "%OVMX-W-KMOD, loading out-of-tree module taints kernel.\n");
    check_console("kernel's OWN signature-verification line, 'vms: ' collision, routes to console as OVMX/KMOD",
