...
| `boot.sh`, `distro/boot/run-qemu.sh`, `docs/install-guide.md` | New flow: build → distribution image; install = boot distrib + blank target; run = boot target with bootstrap initramfs. `run-qemu.sh` already grew `cache=writethrough` (other session, vms-9b7) — keep. The install flow is `docs/install-guide.md`'s CI-checked `PRODUCT INSTALL` procedure, over the one initramfs `distro/Dockerfile.bootable` builds. |
