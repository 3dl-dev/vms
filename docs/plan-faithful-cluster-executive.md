...
| | **LANDED** except the R4/R5 lab runs. The port grew the second half of the block service: a header-only block frame is a REQUEST DATA, and `pe_blk_rx_try` answers it ITSELF from the buffer its SOURCE name resolves to in this port's own table — READ's chunking, `+0/+4/+6` and both names echoed verbatim off the request (so the first answer frame is BYTE-IDENTICAL to it, the recorded vms291 pair), `+8` counting down, and NO SYSAP upcall. A request naming no buffer of ours is dropped and counted (`blk_req_unknown_buffer`); one naming a non-source buffer or a span outside it is refused WHOLE (`blk_req_refused`), never clamped. `pe_blk_send_ack` (FC-P6.1's misread of the pair) is replaced by `pe_blk_send_request` — the SERVER's half, roles swapped, our buffer the DESTINATION. The server FSM gained `request_write_data`: a WRITE now registers its staging slot AND asks for the bytes, and a request that could not be issued is an ANSWERED command (real controller error, name withdrawn) rather than an HRB waiting forever. The disk commit still goes to FC-P6.6's worker — gate RULE 5 stays green. The class driver is unchanged and now correct: it registers the caller's buffer `PE_BLK_ACC_SRC` before the command and completes on the server's end message. Proved: R1 `cluster_host_test_pe_block` (byte-exact answer vs. the request, chunked down-count, unknown-buffer drop, whole refusals, the send side's INV-6 refusals) + `cluster_host_test_mscp_cl` (a real WRITE moves real bytes to the server's block device at the caller's LBN, the port answering out of the caller's real buffer) + `cluster_host_test_mscp_srv` (the REQUEST DATA's fields all read off the host's Table A-6 descriptor; a refused request is answered); R2 `cluster_sim_test_sim_mscp_write` (two simulated nodes over the virtual LAN: command → REQUEST DATA → the client port's answer → the worker's commit → WRITE END → completion, with the volume asserted untouched while the fork context ran). **Open:** `tests/lab/tools/run_mscp_write_gate.sh` (exit 77) for R4/R5 — and R4 is additionally blocked on **E48**, raised by the R2 scenario: the port's RECEIVE path delivers a sequenced message to a SYSAP only for length classes the codec grounds a Con.ID for, and of the five measured MSCP end lengths only the 94-content WRITE END is one, so a booted OVMX class driver cannot yet receive an SCC/READ/ONLINE/GUS end off the wire. | | | | | |
| FC-P6.6 | **MSCP server I/O off the fork thread** (design §3.2.6 E42 corollary): served-unit `exec_blockdev_*` calls move from the fork work handler (`vms_mscp_srv.c:219/239`) to a served-I/O worker kthread (§15) that posts completions back to the fork queue; the fork thread builds end messages / SEND DATA from completions; CI grep gate: no `exec_blockdev_` symbol in any fork-context path. | R1: server FSM tests with a fake worker; R4: HELLO cadence jitter under a served READ loop stays < 10 ms on both substrates. | `vms_mscp_srv.c`, `vms_mscp_srv_io.{c,h}` (the worker-context TU), `vms_cluster_fork.{c,h}` + `_bind.c` (worker post + the second `exec_kthread`), `vms_mscp_srv_fsm.{c,h}` (`io_submit` / `mscp_srv_fsm_io_done`, the HRB's two waits), `tools/ci/cluster_core_includes_gate.sh` (RULE 5) | P6.3 | S (medium) | — |
| | **LANDED** except the R4 wire measurement. The server's `read_blocks`/`write_blocks` ops are gone: there is one `io_submit` that hands the transfer over and returns, and `mscp_srv_fsm_io_done()` builds the end message / drives the SEND DATA from the completion, on the fork thread. The HRB gained `WAIT_DATA`/`WAIT_IO` plus an `io_tag`, so nothing frees a staging slot the worker still owns (the reaper defers, a connection close abandons, a stale completion is dropped). `exec_blockdev_*` now appears in exactly one cluster TU, `vms_mscp_srv_io.c`, and gate RULE 5 enforces that file-scoped with its own negative control. Proved: R1 `cluster_host_test_cluster_fork` (worker queue, reserved completion, stop) + `cluster_host_test_cluster_fork_threads` (a real thread whose callback sleeps; the fork thread kept dispatching throughout) + `cluster_host_test_mscp_srv` (no block read on the command dispatch; slot ownership; stale completion); R3 `tests/qemu/test_kmod_cluster_fork_hammer.c` on a real kernel (real second kthread, real 10 ms `exec_timer` cadence, `CADENCE_TICKS_DURING_IO > 0`). **Open:** the R4 wire figure (`tests/lab/tools/run_mscp_srv_io_jitter_gate.sh`, exit 77) needs a peer running a sustained served READ loop — FC-P6.4's real-VAX MOUNT or FC-P7.2's ACP client — so it is deferred to those rows rather than measured against an idle server. | | | | | |
