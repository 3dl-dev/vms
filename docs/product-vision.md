...
Make OpenVMS free. OVMX is a **clean-room, open-source OpenVMS-compatible environment on Linux** that runs real VMS applications and **joins real VMSclusters** — so organizations can migrate off VMS Software Inc (VSI) node-by-node, at zero downtime and zero license cost.
...
OVMX must be a drop-in cluster member that runs the customer's software. That needs two parallel pillars — neither defers:
...
2. **VMS compatibility / image activation** (`vms-913` + compat work) — *what lets OVMX run the migrated software.* Full system-service, RMS, DCL, and image-activation compatibility, with **no Unix/Linux leaks** visible to users or applications. Without this, a cluster node is just a protocol emulator.

They converge at the **rolling evacuation**: moving a live workload off a VMS node onto OVMX *is* running VMS images on OVMX inside the cluster (`vms-ci.6`, gated by both the DLM rung and image activation).

### The one metric that matters

> Can a real, unmodified VMS application build/run on OVMX — and can an OVMX node join a real VMScluster — with **zero Unix leaks** visible to its users?
...
Reference lab (SIMH VAX 7.3 cluster) → capture + dissect SCS/NISCA/MSCP → OVMX appears in real `SHOW CLUSTER` → MSCP-served disk → distributed lock manager → rolling evacuation. Clean-room throughout.
