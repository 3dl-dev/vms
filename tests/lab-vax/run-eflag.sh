#!/bin/bash
# run-eflag.sh - orchestrate the P4-E runtime proof (rd vms-4e7, parent
# vms-476, epic vms-8e8; blocked-on vms-f78bb): common event flags proven
# cross-process against a REAL /dev/vms on NetBSD/vax under SIMH (INV-6
# honest).
#
# Sibling of run-devvms.sh (the P4-B PING proof) and of tests/netbsd/run_p2c.sh
# (the amd64 event-flag proof), for the VAX architecture under SIMH. Reuses
# vms-f78bb's compile-into-kernel fallback unchanged (vax GENERIC omits
# `options MODULAR'; see tests/lab-vax/README.md "P4-B"): build a custom
# GENERIC+MODULAR kernel, install it, and modload the UNCHANGED cross-built
# OVMX module (which already carries src/kernel-core/vms_eflag.c, the same
...
#      static vmseflag guest tool (tools/cross-vax/build-eflag-vax.sh).
#   2. BUILD-KERNEL (ovmx-cross-vax): the GENERIC+MODULAR vax kernel
#      (netbsd-OVMX) via NetBSD build.sh. Cached: skipped if the kernel
#      artifact already exists (own eflag-artifacts cache, mirrors how
#      netbsd-vax-sysboot's boot-artifacts is separate from devvms-artifacts).
...
#      mknod /dev/vms, run the cross-process event-flag proof; plus the INV-6
#      module-absent control and the EFLAG_SKIP_LOAD/EFLAG_SKIP_SET teeth
#      checks.
#
...
#   tests/lab-vax/run-eflag.sh negctl-load  # EFLAG_SKIP_LOAD teeth check
#   tests/lab-vax/run-eflag.sh negctl-set   # EFLAG_SKIP_SET teeth check
#
...
# drive_eflag_vax.py itself is NEVER negctl-mode-aware.
set -euo pipefail
...
# to run-devvms.sh -- same NetBSD/vax release, same shared disk) --------------
...
ARTIFACTS_DIR="${CACHE_DIR}/eflag-artifacts"        # module+tool+kernel (small)
KBUILD_DIR="${KBUILD_DIR:-${CACHE_DIR}/eflag-kbuild}"
NBSRC_DIR="${KBUILD_DIR}/nbsrc"
KERNEL_MARKER="${CACHE_DIR}/.ovmx-eflag-modular-kernel-installed"
...
log() { echo "[run-eflag] $*"; }
die() { echo "[run-eflag] FATAL: $*" >&2; exit 1; }
...
# 1. cross-build the loadable module + event-flag tool
cross_build() {
  ensure_src
  mkdir -p "${ARTIFACTS_DIR}"
  log "cross-building vms.kmod (-fno-pic, loadable) + vmseflag for elf32-vax"
  docker run --rm -v "${REPO}:/src" -w /src \
    -v "${NBSRC_DIR}:/nbsrc:ro" -v "${ARTIFACTS_DIR}:/out" \
    "${CROSS_IMAGE}" sh tools/cross-vax/build-eflag-vax.sh
  [ -f "${ARTIFACTS_DIR}/vms.kmod" ] && [ -f "${ARTIFACTS_DIR}/vmseflag" ] || die "cross-build missing artifacts"
}
...
build_kernel() {
  ...
  local cid="ovmx-vax-eflag-kbuild-$$"; local rc=0
  set +e
...
ensure_disk() {
  ...
  local cid="ovmx-vax-eflag-install-$$"; local rc=0
  set +e
...
run_session() {
  local ovmx_mode="$1" skip_load="${2:-}" skip_set="${3:-}"; local cid="ovmx-vax-eflag-${ovmx_mode}-$$"; local rc=0
  set +e
...
case "${MODE}" in
  prove)
    ...
    if vaxharness_negctl_gate "${rc}" 0; then
      log "======================================================================"
      log "  EFLAG-VAX PASSED: common event flags shared cross-process on"
      log "  real NetBSD/vax under SIMH, real /dev/vms (INV-6 honest)"
      log "======================================================================"
      exit 0
    fi
    die "EFLAG-VAX FAILED (driver exit=${rc}; see console output above)"
    ;;
  negctl-load)
    # Teeth: with modload skipped, the whole proof MUST fail.
    rc=0
    run_session prove skip || rc=$?
    if vaxharness_negctl_gate "${rc}" 1; then
      log "PASS: EFLAG_SKIP_LOAD negative control failed as required (driver exit=${rc})"
      exit 0
    fi
    die "NEGATIVE CONTROL DID NOT FAIL: harness passed (exit=${rc}) with the module unloaded -- no teeth"
    ;;
  negctl-set)
    # Teeth: with process A's SETEF skipped, the cross-process SET assertion
    # MUST fail (proves the positive B-sees-A's-set assertion is not vacuous).
    rc=0
    run_session prove "" skip || rc=$?
    if vaxharness_negctl_gate "${rc}" 1; then
      log "PASS: EFLAG_SKIP_SET negative control failed as required (driver exit=${rc})"
      exit 0
    fi
    die "NEGATIVE CONTROL DID NOT FAIL: harness passed (exit=${rc}) with SETEF skipped -- no teeth"
    ;;
  *) die "unknown mode '${MODE}' (want: prove | negctl-load | negctl-set)" ;;
esac
