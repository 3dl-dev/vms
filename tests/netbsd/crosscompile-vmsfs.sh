...
# userspace #else). The -isystem clang resource include dir (added above so the
# header's <stdint.h>-shaped includes resolve under -nostdinc) MASKED that
# per-PR: clang's own stdint.h satisfied the bad include, so only the nightly
...
check_ondisk_stdint_portability() {   # <header-file> -> 0 portable / 1 leak
    local hdr="$1" d tree stdint leak
    d="$(mktemp -d)"
    cp "$hdr" "$d/vmsfs_ondisk.h"
    printf '#include "vmsfs_ondisk.h"\nuint32_t _ovmx_probe;\n' > "$d/probe.c"
    tree="$d/tree.txt"
    # -I"$d" FIRST so this header copy wins; -H dumps the include tree to stderr.
    if ! "$CC" "${CFLAGS[@]}" -I"$d" -H -fsyntax-only "$d/probe.c" 2>"$tree"; then
        echo "  header-portability: probe FAILED to compile:"; sed 's/^/    /' "$tree"; rm -rf "$d"; return 1
    fi
    stdint="$(awk '/^[.]+ / { if ($NF ~ /stdint(-gcc)?\.h$/) print $NF }' "$tree" | sort -u)"
    rm -rf "$d"
    echo "$stdint" | grep -q '/sys/stdint\.h$' \
        || { echo "  header-portability: <sys/stdint.h> was NOT reached (fixed-width types from the wrong header)"; return 1; }
    leak="$(echo "$stdint" | grep -v '/sys/stdint\.h$' || true)"
    if [ -n "$leak" ]; then
...
if [ "${CROSSCOMPILE_NEGCTL_HDR:-}" = "1" ]; then
    reg="$OBJ/vmsfs_ondisk_regressed.h"
    mkdir -p "$OBJ"
    sed 's@#[[:space:]]*include[[:space:]]*<sys/stdint.h>@#include <stdint.h>@' \
        "$ONDISK/vmsfs_ondisk.h" > "$reg"
    if check_ondisk_stdint_portability "$reg"; then
        echo "FAIL (hdr-negctl): a regressed vmsfs_ondisk.h (_KERNEL -> bare <stdint.h>) PASSED the check -- NO TEETH"
        exit 1
    fi
    echo "PASS (hdr-negctl): a regressed vmsfs_ondisk.h is correctly REJECTED by the stdint-portability check"
    exit 0
fi
...
check_ondisk_stdint_portability "$ONDISK/vmsfs_ondisk.h" \
