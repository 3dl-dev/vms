...
| `boot.sh`, `distro/boot/run-qemu.sh`, `docs/install-guide.md` | New flow: build → distribution image; install = boot distrib + blank target; run = boot target with bootstrap initramfs. `run-qemu.sh` already grew `cache=writethrough` (other session, vms-9b7) — keep. As-built: `docs/install-0.1.md` (this row's original target) was retired and replaced by `docs/install-guide.md`, the CI-checked `PRODUCT INSTALL` flow — one initramfs (`distro/Dockerfile.bootable`), no `--slim` variant. |
