...
echo "--- formatting the blank upgrade target (VDA100:, label WORK) with $INIT_EXE ---"
...
echo "--- mastering the kit-carrier disk (VDA200:, label KITS) with $MASTER_EXE ---"
...
# "VDA200: would not mount as ODS-2"), exactly as the VDA100 target above notes.
# The kit carrier is a real mounted disk the guest reads OVMX-OS-*.KIT off of,
# so it must be a genuine ODS-2 volume just like VDA100:, not the master tool's
