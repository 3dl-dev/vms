#!/bin/bash
# run_job_control_negctl_e2e.sh - ctest entry point for job_control_negctl_e2e
# (vms-2a9).
#
# Same opt-in/SKIP convention as run_job_control_console_e2e.sh: the gate
# itself (tests/qemu/test_job_control_negctl.sh) drives a REAL docker + QEMU
# boot of the negative-control mastered image
# (/boot/ovmx-distrib-negctl.img, built by distro/Dockerfile.bootable). It
# is registered with ctest but does NOT run under a bare `ctest` invocation
# -- see run_parts_demo_e2e.sh's header for the full reasoning.
#
# Env knobs:
...
if [ "${OVMX_QEMU_FULL_E2E:-0}" != "1" ]; then
    echo "SKIP: job_control_negctl_e2e requires OVMX_QEMU_FULL_E2E=1 (real docker+QEMU boot)."
    echo "      Run directly: OVMX_QEMU_FULL_E2E=1 OVMX_BOOT_IMAGE=$IMAGE tests/qemu/run_job_control_negctl_e2e.sh"
    exit "$SKIP"
fi
...
exec docker run --rm \
    ...
    -v "$REPO_ROOT/tests/qemu/test_job_control_negctl.sh:/test.sh:ro" \
    --entrypoint bash \
    "$IMAGE" \
    /test.sh
