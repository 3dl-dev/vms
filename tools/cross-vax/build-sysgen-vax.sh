#!/bin/sh
# build-sysgen-vax.sh - cross-compile + link-prove SYSGEN.EXE
# (tools/vms_sysgen.c) for netbsd-vax (rd vms-d13, rung D of
...
# touches the host.
#
# SYSGEN.EXE (tools/CMakeLists.txt: target_link_libraries(vms_sysgen PRIVATE
# vmsfs vmslnm)) is needed for the install menu's SCSNODE/SCSSYSTEMID step.
# It inherits the pre-existing vms-597 gap (SYSGEN.EXE not yet resolvable as
# an image on the target) -- this rung is the cross-build only, not vms-597's
# fix. Portable C already used on x86_64/aarch64; no new design, just the
# same static-link/Decision-A cross-build pattern the existing vax image
...
#          vmsqueue.a
#   1.  compile vms_sysgen.c for elf32-vax.
#   2.  SYSGEN.EXE LINK PROOF: link a real vax--netbsdelf ELF32 executable
...
#       clean vax--netbsdelf link of SYSGEN.EXE.
#
# Teeth (CROSSCOMPILE_NEGCTL=1): compile a deliberately-broken copy of
# vms_sysgen.c and assert the cross-compile REJECTS it, mirroring
...
# --- common compile flags for vms_sysgen.c ----------------------------------
CFLAGS_COMMON="--sysroot=$SYSROOT -O2 -Wall -Wextra \
    ...
    -I$VMSFS_INCLUDE -I$LIBVMSSYS"

# --- teeth check -------------------------------------------------------------
if [ "${CROSSCOMPILE_NEGCTL:-}" = "1" ]; then
    bad="$OUT/sysgen_bad.c"
    { cat "$TOOLS/vms_sysgen.c"; printf '\nthis is deliberately invalid C @@@ ;\n'; } > "$bad"
    # shellcheck disable=SC2086
    if "$CC" $CFLAGS_COMMON -c "$bad" -o /dev/null 2>/dev/null; then
        echo "FAIL (negctl): a deliberately-broken SYSGEN.EXE TU COMPILED -- the cross-compile check has NO TEETH"
        exit 1
    fi
    echo "PASS (negctl): a deliberately-broken SYSGEN.EXE TU fails the elf32-vax cross-compile, as it must"
    exit 0
fi
...
# --- proof 1: cross-compile vms_sysgen.c ------------------------------------
echo "=== proof 1: cross-compile vms_sysgen.c ==="
# shellcheck disable=SC2086
"$CC" $CFLAGS_COMMON -c "$TOOLS/vms_sysgen.c" -o "$OUT/vms_sysgen.o"
echo "--- object arch check (must be VAX / ELF32 LSB) ---"
"$TARGET-objdump" -f "$OUT/vms_sysgen.o" | grep -Ei 'file format|architecture'
echo

# --- proof 2: link a real vax--netbsdelf SYSGEN.EXE over the whole stack ---
echo "=== proof 2: link a real vax--netbsdelf SYSGEN.EXE ==="
...
"$CC" --sysroot="$SYSROOT" \
    ...
    -Wl,--end-group \
    -lpthread -lm -latomic -o "$OUT/SYSGEN.EXE"
echo "--- linked executable ---"
file "$OUT/SYSGEN.EXE"
"$TARGET-readelf" -h "$OUT/SYSGEN.EXE" | grep -Ei 'Class|Data|Machine|Type'

HDR="$("$TARGET-readelf" -h "$OUT/SYSGEN.EXE")"
echo "$HDR" | grep -qiE 'Class:[[:space:]]+ELF32' \
    || { echo "FAIL: SYSGEN.EXE is not ELFCLASS32 (VAX is 32-bit)"; exit 1; }
echo "$HDR" | grep -qiF 'Digital VAX' \
    || { echo "FAIL: SYSGEN.EXE Machine is not Digital VAX"; exit 1; }

echo
echo "=== ALL PROOFS PASSED: SYSGEN.EXE builds and links for $TARGET ==="
