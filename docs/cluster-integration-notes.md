...
### E14. The port cannot ROUTE the 58- and 94-content SCS classes upward (raised by FC-P2.2)
...
### E15. `scs_sysap_ops.closed` IS design §3.2.5's `disconnected()` (raised by FC-P2.2)
...
### E16. FC-P2.3 must GROW FC-P2.2's SDIR seed, not add a second registry (raised by FC-P2.2)
...
### E17. SCSFLOWCUSH is not in `struct vms_cluster_params` (raised by FC-P2.2)
...
ruling on top of FC-P1.3 (no collision left — P1.3 integrated first, as this
entry required). **FC-P2.2 landed the SCS half**: `scs_fsm_vc_down(f, peer,
reason)` walks that SB's CDT queue (ch. 2's Path Block queue, which is why the
SB holds `cdt_head`) and closes every CDT with `SCS_CLOSE_PATHLOST`, discards
the ledgers, fails every Credit Wait entry with path-lost through the SYSAP's
`send_failed`, and calls `closed()`. Its R1 test
(`tests/cluster/host/test_scs_fsm_vcdown.c`) asserts each clause on an SB
carrying four connections at once, plus the hardest one: **zero frames were put
on the wire during or after the break**. FC-P2.4's glue only has to point
`pe_upper_ops.vc_down` at it and map the reason to `SS$_PATHLOST`.
