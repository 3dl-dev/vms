...
# The honest cross-node statuses. 0x000008F8 = 2296 = SS$_UNSUPPORTED (reached
# the real executive, rung-1 does not grant). 0x00000A78 = 2680 = SS$_NOSUCHDEV
# (fail-honest: no executive reached). H3's win is the FORMER, never the LATTER.
...
# THE MILESTONE ASSERTION: B reached the real executive (2296), NOT the missing-
# executive fail-honest status (2680).
if [ "$B_DISPATCH_STATUS" = "$STATUS_NOSUCHDEV" ]; then
    ...
    echo "  FAIL: node B dispatch returned '${B_DISPATCH_STATUS:-<none>}' -- expected ${STATUS_UNSUPPORTED} (2296 SS\$_UNSUPPORTED)."
    FAIL=1
fi
...
echo "=========================================="
if [ "$FAIL" = 0 ]; then
    echo "  DLM HARNESS H3 PASSED: B dispatched cross-node \$ENQ to real executive rc=2296 (not 2680)"
    echo "  A_VAXCLMEMBER=1 B_VAXCLMEMBER=1  A_SENT_ENQ=1  B_RECEIVED_ENQ=1  B_DISPATCH_STATUS=${STATUS_UNSUPPORTED}"
    ...
    echo "  /dev/vms executive (vms_lock_dlm_xnode_dispatch). B's status is 2296"
    echo "  (SS\$_UNSUPPORTED) -- the \$ENQ REACHED the mastering node's real executive and"
    echo "  was HONESTLY declined at rung 1, NOT 2680 (SS\$_NOSUCHDEV, which is what a"
    echo "  missing executive returns). The A->B->A round-trip completed. The rung-2 GRANT"
    echo "  (-> SS\$_NORMAL) is H4 (vms-e8f1); H3's milestone is REACHED-not-granted."
    echo "=========================================="
    exit 0
else
