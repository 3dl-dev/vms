...
| **AST + common event flags** `src/kernel/vms_ast.c`, `vms_eflag.c` | **KEEP** | Solid async foundation; the lock manager already delivers blocking ASTs through it (SIGUSR1 → per-mode queues). Common event-flag clusters 2-3 are already cross-process shared state — the conceptual seed for cluster-wide events. |
...
| **Process/AST layer** `src/vmsprocess/*` | **KEEP** | Reuse for delivering async cluster state transitions and lock grants. `$SETCLUEVT`/`$SET_SYSTEM_EVENT` need real implementation atop the (new) connection manager. |
