#!/bin/sh
# build-proctab-vax.sh - produce the two elf32-vax artifacts the P4-proctab
# runtime proof (rd vms-2e0, parent vms-945e, epic vms-8e8) loads on real
...
#                    (build-devvms-vax.sh) loads, and P4-E
#                    (build-eflag-vax.sh) loads -- same 10 TUs, same
#                    -O2 -fno-pic loadable-module build, so this is the SAME
#                    module image, just rebuilt here so this proof owns its
#                    own cached artifact (mirrors how P4-E's eflag-artifacts
#                    is independent of P4-B's devvms-artifacts).
#   2. vmsproctab  - the process-table guest tool
#                    (tests/netbsd/guest/vmsproctab.c), reaching /dev/vms
#                    THROUGH the NetBSD transport seam
#                    (kif_transport_netbsd.c), statically linked. This is the
#                    vax analogue of the NetBSD/amd64 P4-A tool build, and the
#                    SAME source build-proctab-tool-vax.sh (the fast per-PR
#                    link gate) proves builds+links -- this script
#                    additionally builds vms.kmod, which needs the pinned
#                    NetBSD kernel headers ($NBSRC) the per-PR gate
#                    deliberately does not require.
#
# WHY -O2 AND NOT B1's FLAGS. Same rationale as build-eflag-vax.sh /
# build-devvms-vax.sh: B1 is a width-audit GATE that permits unresolved
...
#   OUT     output dir for vms.kmod + vmsproctab. Default /out.
#
...
OUT="${OUT:-/out}"
# Remove ONLY this script's own outputs -- NOT the whole dir: run-proctab.sh
# shares OUT (proctab-artifacts) with the cached custom kernel (netbsd-OVMX),
# and wiping the dir here would delete that cache and force a ~40-min
# build.sh rerun.
mkdir -p "$OUT"; rm -f "$OUT"/vms.kmod "$OUT"/vmsproctab "$OUT"/*.o 2>/dev/null || true
...
# build-devvms-vax.sh use -- -O2 (kernel default) so the resource-hash inline
# uses link, -fno-pic so the object carries ABSOLUTE (R_VAX_32) relocations,
...
# containers + the SHARED executive facility sources (vms_proctab.c is the one
# this proof exercises; the rest ride along because they are one module image).
...
echo "=== build the process-table guest tool (static elf32-vax) ==="
...
"$OBJDUMP" -f "$OUT/vmsproctab" | grep -qiF 'file format elf32-vax' || { echo "FAIL: vmsproctab not elf32-vax"; exit 1; }
echo "  OK: vmsproctab (static elf32-vax)"
echo

echo "=== ARTIFACTS ==="
ls -l "$OUT/vms.kmod" "$OUT/vmsproctab"
echo "=== build-proctab-vax.sh: DONE (both elf32-vax artifacts ready for SIMH) ==="
