...
int main(void)
{
    ...
    if (sd == 0) {
         ...
         * 127.0.0.1). socket()/bind()/listen() dispatch to the executive. */
    ...
    CHECK(wait_for_sshd(50),
          "the WRAPPED sshd bound BGn: to a real host port and accepts inbound connections");
