...
/* cluster_sha256.c - FIPS 180-4 SHA-256, host test support (FC-P0.6). */
...
void cluster_sha256_hex(const uint8_t *data, size_t len, char *hex)
{
	...
	for (size_t j = rem + 1; j < tail_len - 8; j++)
		tail[j] = 0;
	for (size_t j = 0; j < 8; j++)
		tail[tail_len - 1 - j] = (uint8_t)((bits >> (8 * j)) & 0xffu);
	sha256_block(h, tail);
	if (tail_len == 128)
	...
	for (i = 0; i < 8; i++) {
		for (size_t j = 0; j < 4; j++) {
			uint8_t byte = (uint8_t)((h[i] >> (24 - 8 * j)) & 0xffu);

			hex[i * 8 + j * 2]     = digits[byte >> 4];
			hex[i * 8 + j * 2 + 1] = digits[byte & 0x0fu];
		}
	}
	hex[64] = '\0';
}
