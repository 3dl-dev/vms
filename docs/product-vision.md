...
| Q2 | Kernel modules vs pure userspace? | Kernel executive — VMS-specific semantics live in `vms.ko` | Access control, AST delivery, event flags, locking, and the Files-11 ODS-2 ACP live in the executive; userspace reaches them through `/dev/vms` and **fails honestly** (`SS$_NOSUCHDEV`) when the executive is absent — never a silent per-process fake (INV-6, Rule 9) |
| Q3 | How is OVMX run? | **One runtime** — the real-kernel / QEMU path where `vms.ko` provides the executive. Docker/podman are BUILD and TEST tooling only, never a runtime | The Docker *runtime* layer is dead (operator ruling 2026-07-28, Rule 9): a container has no `/dev/vms`, so "running OVMX in Docker" would necessarily be a fake. Build tooling stays containerized (`Dockerfile.bootable`, `src/kernel/Dockerfile`, `tests/qemu/Dockerfile`) |
...
Freestanding syscall layer (x86_64, aarch64, Alpha, VAX), VMS system services + RTL, process management (PCBs/ASTs/event flags/access modes), logical name manager, VMS filesystem library, RMS (sequential/relative/indexed), DCL shell, the kernel executive (`vms.ko` — the ODS-2 ACP now lives here; the standalone `vmsfs.ko` was retired), bootable distro (real-kernel/QEMU runtime; Docker/podman for build+test only), multi-user SSH with SYSUAF auth. Current status: `docs/release-roadmap-to-1.0.md` (roadmap of record, generated from rd + git tags) and `rd`. `tracking/roadmap.md` is pre-pivot history, not a live source.
